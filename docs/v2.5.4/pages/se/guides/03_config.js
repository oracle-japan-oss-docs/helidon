...
<tbody>
<tr>
<td class=""><span class="merged" id="all.1hKNxI.15" title="原文 : A Helidon {upper-case-flavor} Application">Helidon {upper-case-flavor} アプリケーション</span></td>
<td class=""><span class="merged" id="all.3jMlMY.15" title="原文 : You can use your own application or use the Helidon {upper-case-flavor} Quickstart to create a sample application.">独自のアプリケーションを使用するか、<a href="https://helidon.io/docs/v2/#/{lower-case-flavor}/guides/02_quickstart" id="" target="_blank" >「Helidon {upper-case-flavor} クイックスタート」</a>を使用してサンプル・アプリケーションを作成できます。</span></td>
</tr>
<tr>
<td class=""><span class="merged" id="all.3UjXGj.16" title="原文 : Java&#160;SE&#160;11 (Open&#160;JDK&#160;11)"><a href="https://www.oracle.com/technetwork/java/javase/downloads" id="" target="_blank" >Java&#160;SE&#160;11</a> (<a href="http://jdk.java.net" id="" target="_blank" >Open&#160;JDK&#160;11</a>)</span></td>
<td class=""><span class="merged" id="all.3e6V0X.16" title="原文 : Helidon requires Java 11+.">HelidonにはJava 11以上が必要です。</span></td>
</tr>
<tr>
<td class=""><span class="merged" id="all.1nyTbh.18" title="原文 : Maven 3.6.1+"><a href="https://maven.apache.org/download.cgi" id="" target="_blank" >Maven 3.6.1+</a></span></td>
<td class=""><span class="merged" id="all.3woDk0.16" title="原文 : Helidon requires Maven 3.6.1+.">Helidonには、Maven 3.6.1+が必要です。</span></td>
</tr>
<tr>
<td class=""><span class="merged" id="all.1HtX9I.17"  title="原文:: Docker 18.09+"><a href="https://docs.docker.com/install/" id="" target="_blank" >Docker 18.09+</a></span></td>
...
<tr>
<td class=""><span class="merged" id="all.48aNQe.17"  title="原文:: Kubectl 1.16.5+"><a href="https://kubernetes.io/docs/tasks/tools/install-kubectl/" id="" target="_blank" >Kubectl 1.16.5+</a></span></td>
<td class=""><span class="merged" id="all.1tjPJQ.16" title="原文 : If you want to deploy to Kubernetes, you need kubectl and a Kubernetes cluster (you can install one on your desktop).">Kubernetesにデプロイする場合は、<code>kubectl</code>およびKubernetesクラスタ(<router-link to="/about/05_kubernetes">「デスクトップにインストール」</router-link>も可)が必要です。</span></td>
</tr>
</tbody>
</table>
</div>
...
<div class="section">
<p><span class="merged" id="all.3XQAFB.spl1" title="原文 : Helidon provides a very flexible and comprehensive configuration system, offering you many application configuration choices.">Helidonは、非常に柔軟で包括的な構成システムを提供し、多くのアプリケーション構成を選択できます。</span> <span class="merged" id="all.3XQAFB.spl2" title="原文 : You can include configuration data from a variety of sources using different formats, like JSON and YAML.">JSONやYAMLなどの様々な形式を使用して、様々なソースからの構成データを含めることができます。</span> <span class="merged" id="all.3XQAFB.spl3" title="原文 : Furthermore, you can customize the precedence of sources and make them optional or mandatory.">さらに、ソースの優先順位をカスタマイズして、オプションまたは必須にすることもできます。</span> <span class="merged" id="all.3XQAFB.spl4" title="原文 : This guide introduces Helidon SE configuration and demonstrates the fundamental concepts using several examples.">このガイドでは、Helidon SE構成を紹介し、いくつかの例を使用して基本概念を示します。</span> <span class="merged" id="all.3XQAFB.spl5" title="原文 : Refer to Helidon Config for the full configuration concepts documentation.">構成の概念の詳細は、<router-link to="/se/config/01_introduction">「Helidon構成」</router-link>を参照してください。</span> </p>
...
<p class="admonition-inline"><span class="merged" id="all.3BEtQY.spl1" title="原文 : Because environment variable names are restricted to alphanumeric characters and underscore, Helidon adds aliases to the environment configuration source, allowing entries with dotted and/or hyphenated keys to be overriden.">環境変数名は英数字およびアンダースコアに制限されているため、Helidonでは、環境構成ソースに別名が追加され、ドット付きキーまたはハイフン付きキー(あるいはその両方)を持つエントリをオーバーライドできます。</span> <span class="merged" id="all.3BEtQY.spl2" title="原文 : For example, this mapping allows an environment variable named &quot;APP_GREETING&quot; to override an entry key named &quot;app.greeting&quot;.">たとえば、このマッピングを使用すると、"APP_GREETING"という環境変数で"app.greeting"というエントリ・キーをオーバーライドできます。</span> <span class="merged" id="all.3BEtQY.spl3" title="原文 : In the same way, an environment variable named &quot;APP_dash_GREETING&quot; will map to &quot;app-greeting&quot;.">同様に、"APP_dash_GREETING"という名前の環境変数が"app-greeting"にマップされます。</span> <span class="merged" id="all.3BEtQY.spl4" title="原文 : See Advanced Config for more information.">詳細は、<router-link to="/se/config/06_advanced-configuration">「拡張構成」</router-link>を参照してください。</span> </p>
</div>
...
<p><span class="merged" id="all.2WRQCO.spl1" title="原文 : You can also define custom sources, such as Git, and use them in your Helidon application.">Gitなどのカスタム・ソースを定義して、Helidonアプリケーションで使用することもできます。</span> <span class="merged" id="all.2WRQCO.spl2" title="原文 : See Advanced Config for more information.">詳細は、<router-link to="/se/config/06_advanced-configuration">「拡張構成」</router-link>を参照してください。</span> </p>
...
<h3 id="_configuration_profiles"><span class="merged" id="all.3IXrHG.1"  title="原文:: Configuration Profiles">構成プロファイル</span></h3>
<div class="section">
...
<div class="section">
<p><span class="merged" id="all.238fgP.spl1" title="原文 : You have used Helidon to customize configuration behavior from your code using the Config and Config.Builder classes.">Helidonを使用して、<code>Config</code>および<code>Config.Builder</code>クラスを使用したコードからの構成動作をカスタマイズしました。</span> <span class="merged" id="all.238fgP.spl2" title="原文 : As discussed previously, Helidon reads configuration from a config source, which uses a config parser to translate the source into an in-memory tree which represents the configuration&rsquo;s structure and values.">前述のように、Helidonは構成ソースから構成を読み取り、構成パーサーを使用して、構成の構造と値を表すメモリー内ツリーにソースを変換します。</span> <span class="merged" id="all.238fgP.spl3" title="原文 : Helidon offers a variety of methods to access in-memory configuration.">Helidonには、インメモリー構成にアクセスするための様々なメソッドがあります。</span> <span class="merged" id="all.238fgP.spl4" title="原文 : These can be categorized as key access or tree navigation.">これらは、<em>「キー・アクセス」</em>または<em>「ツリー・ナビゲーション」</em>に分類できます。</span> <span class="merged" id="all.238fgP.spl5" title="原文 : You have been using key access for all of the examples to this point.">この時点までのすべての例で、<em>「キー・アクセス」</em>を使用してきました。</span> <span class="merged" id="all.238fgP.spl6" title="原文 : For example app.greeting is accessing the greeting child node of the app parent node.">たとえば、<code>app.greeting</code>が<code>app</code>親ノードの<code>greeting</code>子ノードにアクセスしているとします。</span> <span class="merged" id="all.238fgP.spl7" title="原文 : There are many options for access this data using navigation methods as described in Hierarchical Config and Advanced Config."><router-link to="/se/config/03_hierarchical-features">「階層構成」</router-link>および<router-link to="/se/config/06_advanced-configuration">「拡張構成」</router-link>で説明されているように、ナビゲーション・メソッドを使用してこのデータにアクセスするためのオプションは多数あります。</span> </p>
...
<div class="section">
<p><span class="merged" id="all.1x5nxV.spl1" title="原文 : Even though in-memory config trees are immutable, the config system internally records configuration source metadata that allows it to watch sources for changes.">インメモリー構成ツリーは不変ですが、構成システムは、ソースの変更を監視できる構成ソース・メタデータを内部的に記録します。</span> <span class="merged" id="all.1x5nxV.spl2" title="原文 : Your application listens for updates to the underlying config sources and reacts to the changes.">アプリケーションは、基礎となる構成ソースの更新をリスニングし、変更に対応します。</span> <span class="merged" id="all.1x5nxV.spl3" title="原文 : See Config Mutability Support for a full discussion on this topic.">このトピックの詳細は、<router-link to="/se/config/05_mutability-support">「構成の可変性のサポート」</router-link>を参照してください。</span> <span class="merged" id="all.1x5nxV.spl4" title="原文 : The following example demonstrates how to listen and react to configuration changes.">次の例では、構成の変更をリスニングして対応する方法を示します。</span> </p>
...
<div class="section">
<p><span class="merged" id="all.2XP2UH.spl1" title="原文 : This guide has demonstrated how to use basic Helidon configuration features.">このガイドでは、基本的なHelidon構成機能の使用方法について説明しました。</span> <span class="merged" id="all.2XP2UH.spl2" title="原文 : The full configuration documentation, starting with the introduction section at Helidon Config has much more information including the following:"><router-link to="/se/config/01_introduction">「Helidon構成」</router-link>の概要セクションから始まる完全な構成ドキュメントには、次のような多くの情報が含まれています:</span> </p>
...
<li>
<p><span class="merged" id="all.2pceCn.1"  title="原文:: Extensions">拡張</span></p>
...
<li>
<p><span class="merged" id="all.3TYb55.1"  title="原文:: Property Mapping">プロパティ・マッピング</span></p>

</li>
<li>
<p><span class="merged" id="all.faGbn.1" title="原文 : Mutability Support">可変性のサポート</span></p>
