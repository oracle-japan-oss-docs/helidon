...
<h2 id="_introduction"><span class="merged" id="all.4LJMHk.16"  title="原文:: Introduction">導入</span></h2>
<div class="section">
...
<h2 id="_what_you_need"><span class="merged" id="all.27ZcMH.22"  title="原文:: What You Need">必要事項</span></h2>
<div class="section">
...
<tbody>
<tr>
<td class=""><span class="merged" id="all.1hKNxI.19" title="原文 : A Helidon {upper-case-flavor} Application">Helidon {upper-case-flavor} アプリケーション</span></td>
<td class=""><span class="merged" id="all.3jMlMY.19" title="原文 : You can use your own application or use the Helidon {upper-case-flavor} Quickstart to create a sample application.">独自のアプリケーションを使用するか、<a href="https://helidon.io/docs/v2/#/{lower-case-flavor}/guides/02_quickstart" id="" target="_blank" >「Helidon {upper-case-flavor} クイックスタート」</a>を使用してサンプル・アプリケーションを作成できます。</span></td>
</tr>
<tr>
<td class=""><span class="merged" id="all.3UjXGj.21" title="原文 : Java&#160;SE&#160;11 (Open&#160;JDK&#160;11)"><a href="https://www.oracle.com/technetwork/java/javase/downloads" id="" target="_blank" >Java&#160;SE&#160;11</a> (<a href="http://jdk.java.net" id="" target="_blank" >Open&#160;JDK&#160;11</a>)</span></td>
<td class=""><span class="merged" id="all.3e6V0X.21" title="原文 : Helidon requires Java 11+.">HelidonにはJava 11以上が必要です。</span></td>
</tr>
<tr>
<td class=""><span class="merged" id="all.1nyTbh.23" title="原文 : Maven 3.6.1+"><a href="https://maven.apache.org/download.cgi" id="" target="_blank" >Maven 3.6.1+</a></span></td>
<td class=""><span class="merged" id="all.3woDk0.21" title="原文 : Helidon requires Maven 3.6.1+.">Helidonには、Maven 3.6.1+が必要です。</span></td>
</tr>
<tr>
<td class=""><span class="merged" id="all.1HtX9I.22"  title="原文:: Docker 18.09+"><a href="https://docs.docker.com/install/" id="" target="_blank" >Docker 18.09+</a></span></td>
...
<tr>
<td class=""><span class="merged" id="all.48aNQe.22"  title="原文:: Kubectl 1.16.5+"><a href="https://kubernetes.io/docs/tasks/tools/install-kubectl/" id="" target="_blank" >Kubectl 1.16.5+</a></span></td>
<td class=""><span class="merged" id="all.1tjPJQ.21" title="原文 : If you want to deploy to Kubernetes, you need kubectl and a Kubernetes cluster (you can install one on your desktop).">Kubernetesにデプロイする場合は、<code>kubectl</code>およびKubernetesクラスタ(<router-link to="/about/05_kubernetes">「デスクトップにインストール」</router-link>も可)が必要です。</span></td>
</tr>
<tr>
<td class=""><span class="merged" id="all.3SaplK.1" title="原文 : GraalVM CE 21.0.0"><a href="https://github.com/graalvm/graalvm-ce-builds/releases/tag/vm-21.0.0" id="" target="_blank" >GraalVM CE 21.0.0</a></span></td>
...
<p><span class="merged" id="all.pf2id.1" title="原文 : And verify:">さらに、次のことを確認します:</span></p>
...
<p><span class="merged" id="all.4ciRtm" title="原文 : For more information about the Quickstart application and other enpoints it supports see the Helidon SE Quickstart Guide.">Quickstartアプリケーションおよびそれがサポートするその他のエンドポイントの詳細は、<router-link to="/se/guides/02_quickstart">「Helidon SEクイックスタート・ガイド」</router-link>を参照してください。</span></p>
...
<p ><span class="merged" id="all.3E5t1v.1.spl1" title="原文 : This uses the helidon-maven-plugin to perform the native compilation using your installed copy of GraalVM.">これは、<code>helidon-maven-plugin</code>を使用し、インストールしたGraalVMのコピーを使用してネイティブ・コンパイルを実行します。</span> <span class="merged" id="all.3E5t1v.1.spl2" title="原文 : It might take a while to complete.">完了するまでに時間がかかる場合があります。</span> </p>
</div>
...
<p><span class="merged" id="all.2NHOSR.1.spl1" title="原文 : Yep, it starts fast.">はい、すぐに開始します。</span> <span class="merged" id="all.2NHOSR.1.spl2" title="原文 : You can exercise the application&rsquo;s endpoints as before.">以前と同様に、アプリケーションのエンドポイントを実行できます。</span> </p>
...
<p><span class="merged" id="all.RH4Mw.1.spl1" title="原文 : Again, it starts fast.">ここでも、高速で起動します。</span> <span class="merged" id="all.RH4Mw.1.spl2" title="原文 : You can exercise the application&rsquo;s endpoints as before.">以前と同様に、アプリケーションのエンドポイントを実行できます。</span> </p>
...
<p><span class="merged" id="all.4M6S3D.1" title="原文 : That said, native images do have some limitations, and for long running applications where startup and footprint are less of a priority, the Java SE HotSpot VM might be more appropriate.">つまり、ネイティブ・イメージにはいくつかの<a href="https://github.com/oracle/graal/blob/master/substratevm/Limitations.md" id="" target="_blank" >「制限事項」</a>があり、起動とフットプリントの優先度が低い長時間実行アプリケーションでは、Java SE HotSpot VMの方が適している場合があります。</span></p>

<p><span class="merged" id="all.23MatH" title="原文 : For information about creating custom Java runtime images see Custom Runtime Images with jlink">カスタムJavaランタイム・イメージの作成の詳細は、<router-link to="/se/guides/37_jlink_image">「<code>jlink</code>を使用したカスタム・ランタイム・イメージ」</router-link>を参照してください</span></p>
