...
<p class="admonition-inline"><span class="merged" id="all.1tnirK.spl1" title="原文 : Helidon-provided endpoints for /metrics do their work synchronously, using the same thread on which the request arrived via Netty."><code>/metrics</code>のHelidon-providedエンドポイントは、リクエストがNetty経由で到着したスレッドと同じスレッドを使用して、同期的に作業を行います。</span> <span class="merged" id="all.1tnirK.spl2" title="原文 : To prevent performance degradation, avoid including long-running code that can be invoked by these handlers while Helidon is responding to the metric.">パフォーマンスの低下を防ぐため、Helidonがメトリックに応答している間に、これらのハンドラによって起動できる長時間実行コードを含めないでください。</span> <span class="merged" id="all.1tnirK.spl3" title="原文 :  For example, if you implement your own application-specific metric types, you will write logic to format the JSON and OpenMetrics output for those metric types."><br>たとえば、独自のアプリケーション固有のメトリック・タイプを実装する場合、それらのメトリック・タイプのJSONおよびOpenMetrics出力をフォーマットするロジックを記述します。</span> <span class="merged" id="all.1tnirK.spl4" title="原文 : Helidon invokes this formatting logic whenever a client accesses the /metrics endpoints, so make that formatting code as efficient as possible.">Helidonは、クライアントが /metricsエンドポイントにアクセスすると常にこの書式設定ロジックを起動するため、可能なかぎり書式設定コードを効率的にします。</span> </p>
</div>
</div>
