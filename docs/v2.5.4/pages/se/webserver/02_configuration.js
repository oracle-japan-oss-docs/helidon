...
<tr>
<td class=""><span class="merged" id="all.4EF1pq" title="原文 : backpressure-buffer-size"><strong><code>backpressure-buffer-size</code></strong></span></td>
<td class=""><span class="merged" id="all.15f2Ma.2"  title="原文:: long">long</span></td>
<td class=""><span class="merged" id="all.1xioaP"  title="原文: 5242880"><code>5242880</code></span></td>
<td class=""><span class="merged" id="all.3ol0yG.spl1" title="原文 : Set a maximum length of the unflushed response data sending buffer can keep without applying backpressure.">バック・プレスを適用せずに、フラッシュされていないレスポンス・データ送信バッファが保持できる最大長を設定します。</span> <span class="merged" id="all.3ol0yG.spl2" title="原文 : Depends on backpressure-policy what happens if max buffer size is reached.">最大バッファ・サイズに達した場合の処理は、<code>backpressure-policy</code>によって異なります。</span> </td><td class=""> <span class="merged" id="all.1uKlR7" title="原文 : Default is 5*1024*1024 - 5Mb">デフォルトは<code>5*1024*1024</code> - 5Mbです</span></td>
</tr>
<tr>
<td class=""><span class="merged" id="all.3U3ZB3" title="原文 : backpressure-policy"><strong><code>backpressure-policy</code></strong></span></td>
...
<td class=""><span class="merged" id="all.49aFgZ" title="原文 : Sets the strategy for applying backpressure to the reactive stream of response data.">レスポンス・データのリアクティブ・ストリームにバックプレッシャを適用するための戦略を設定します。</span></td><td class=""><span class="merged" id="all.26lIvS" title="原文 : * LINEAR - Data chunks are requested one-by-one after previous data chunk has been written to Netty&rsquo;s buffer, when backpressure-buffer-size watermark is reached, new chunks are not requested until buffer size decrease under the watermark value. * PREFETCH - After first data chunk arrives, expected number of chunks needed to fill the buffer up to watermark is calculated and requested. * AUTO_FLUSH - Data are requested one-by-one, in case buffer reaches watermark, no other data is requested and extra flush is initiated. * UNBOUNDED - No backpressure is applied, Long.MAX_VALUE(unbounded) is requested from upstream.">*線形 - データ・チャンクは、以前のデータ・チャンクがNettyバッファに書き込まれた後に1回ずつリクエストされます。<code>backpressure-buffer-size</code>ウォーターマークに達すると、バッファ・サイズがウォーターマーク値で減少するまで、新しいチャンクはリクエストされません。* PREFETCH - 最初のデータ・チャンクが到着すると、バッファを水位標まで埋めるために必要なチャンクの数が計算され、リクエストされます。* AUTO_FLUSH - データは1つずつリクエストされます。バッファがウォーターマークに達した場合、他のデータはリクエストされず、余分なフラッシュが開始されます。* UNBOUNDED - バック・プレッシャは適用されず、Long.MAX_VALUE(unbounded)はアップストリームからリクエストされます。</span></td><td class=""> <span class="merged" id="all.1CJ5TY" title="原文 : Default is LINEAR">デフォルトは<code>LINEAR</code>です</span></td>
</tr>
<tr>
