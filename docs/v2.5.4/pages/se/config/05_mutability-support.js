...
<dl>
<dt slot=title><span class="merged" id="all.faGbn" title="原文 : Mutability Support">可変性のサポート</span></dt>
...
<div class="section">
<p><span class="merged" id="all.4b7V8c.spl1" title="原文 : When the application creates a config source, it can set up change detection for that source.">アプリケーションは、構成ソースを作成するときに、そのソースの変更検出を設定できます。</span> <span class="merged" id="all.4b7V8c.spl2" title="原文 : This is called polling in the Helidon API but specific change detection algorithms might not use actual polling.">これはHelidon APIでは<em>「ポーリング」</em>と呼ばれますが、特定の変更検出アルゴリズムでは実際のポーリングが使用されない場合があります。</span> <span class="merged" id="all.4b7V8c.spl3" title="原文 : You choose a specific PollingStrategy for each config source you want to monitor.">モニターする構成ソースごとに特定の<a href="./apidocs/io.helidon.config/io/helidon/config/spi/PollingStrategy.html" id="" target="_blank" ><code>PollingStrategy</code></a>を選択します。</span> <span class="merged" id="all.4b7V8c.spl4" title="原文 : See the section on polling strategies in the config extensions doc page for more information.">詳細は、構成拡張ドキュメント・ページの<router-link :to="{path: '/se/config/07_extensions', hash: '#Config-SPI-PollingStrategy'}">「ポーリング計画」</router-link>に関する項を参照してください。</span> </p>
