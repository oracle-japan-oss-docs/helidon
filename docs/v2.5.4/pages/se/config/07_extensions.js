...
<dl>
<dt slot=title><span class="merged" id="all.2pceCn"  title="原文:: Extensions">拡張</span></dt>
...
<p><span class="merged" id="all.47erAJ.spl1" title="原文 : The config system introduction explains the design of the config system and how its parts work together to read and parse config data, convert it to Java types, fine-tune the look-up of config data, and reload and reprocess data when it changes."><router-link to="/se/config/01_introduction">config system introduction</router-link>では、構成システムの設計、および構成データの読取りと解析、構成データのJavaタイプへの変換、構成データの参照の微調整、および変更時のデータのリロードと再処理のために構成システムがどのように連携するかについて説明します。</span> <span class="merged" id="all.47erAJ.spl2" title="原文 : Config extensions provided by the application modify and expand the way the config system performs these steps.">アプリケーションによって提供される<em>「構成拡張」</em>は、これらのステップを構成システムが実行する方法を変更して展開します。</span> </p>
</dd>
</dl>
</v-card-text>
</v-card>
