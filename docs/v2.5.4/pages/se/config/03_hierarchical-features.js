...
<div class="section">
<p><span class="merged" id="all.3Wl7uk.spl1" title="原文 : The following example is in HOCON (human-optimized config object notation) format.">次の例は、<a href="https://github.com/lightbend/config/blob/master/HOCON.md" id="" target="_blank" >HOCON</a> (人間が最適化した構成オブジェクト表記)形式です。</span> <span class="merged" id="all.3Wl7uk.spl2" title="原文 : The config system supports HOCON as an extension module.">構成システムでは、HOCONが<router-link :to="{path: '/se/config/08_supported-formats', hash: '#Config-ModuleHocon'}">「拡張モジュール」</router-link>としてサポートされます。</span> </p>
...
<p><span class="merged" id="all.1I8K4K.spl1" title="原文 : For example, the application.properties from the introduction section contains several settings prefixed with web such as web.page-size.">たとえば、概要セクションの<router-link :to="{path: '/se/config/01_introduction', hash: '#create-simple-config-props'}"><code>application.properties</code></router-link>には、<code>web.page-size</code>など、<code>web</code>のプレフィクスが付いたいくつかの設定が含まれています。</span> <span class="merged" id="all.1I8K4K.spl2" title="原文 : Perhaps in another config source the same information might be stored as server.web.page-size:">別の構成ソースに、同じ情報が<code>server.web.page-size</code>として格納されている場合があります:</span> </p>
