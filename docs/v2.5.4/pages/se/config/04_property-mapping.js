...
<dl>
<dt slot=title><span class="merged" id="all.3TYb55"  title="原文:: Property Mapping">プロパティ・マッピング</span></dt>
...
<p><span class="merged" id="all.1FpXZ2.spl1" title="原文 : The next example, and later ones below showing complex type mapping, use the example application.properties configuration from the config introduction.">次の例および以降の例では、複合型マッピングを示します。構成の概要の<router-link :to="{path: '/se/config/01_introduction', hash: '#app-properties'}"><code>application.properties</code></router-link>構成の例を使用してください。</span> <span class="merged" id="all.1FpXZ2.spl2" title="原文 : Part of that example includes this line:">この例の一部には、次の行が含まれています:</span> </p>
...
<div class="section">
<p><span class="merged" id="all.sL24w.spl1" title="原文 : The hierarchical features section describes the tree structure used to represent config data."><router-link to="/se/config/03_hierarchical-features">「階層機能」</router-link>セクションでは、構成データを表すために使用されるツリー構造について説明します。</span> <span class="merged" id="all.sL24w.spl2" title="原文 : The config system can map subtrees of a config tree to complex Java types.">構成システムは、構成ツリーのサブツリーを複雑なJavaタイプにマップできます。</span> </p>
