...
<div class="section">
<p><span class="merged" id="all.F7N64" title="原文 : If your application uses the default configuration, then the config system automatically sets up the config sources for you, as described in the config introduction.">アプリケーションでデフォルト構成が使用されている場合は、<router-link :to="{path: '/se/config/01_introduction', hash: '#config-sources-default-config'}">「構成の概要」</router-link>の説明に従って、構成システムによって構成ソースが自動的に設定されます。</span></p>
...
<p><span class="merged" id="all.RUGaN.spl1" title="原文 : You can also extend the config system to handle other types of sources by implementing the ConfigSource interface."><a href="./apidocs/io.helidon.config/io/helidon/config/spi/ConfigSource.html" id="" target="_blank" ><code>ConfigSource</code></a>インタフェースを実装することで、他のタイプのソースを処理するように構成システムを拡張することもできます。</span> <span class="merged" id="all.RUGaN.spl2" title="原文 : See the extensions documentation for complete information.">詳細は、<router-link to="/se/config/07_extensions">「拡張機能」</router-link>のドキュメントを参照してください。</span> </p>

<p><span class="merged" id="all.3mddF8" title="原文 : See the advanced topics page for further information on some more involved aspects of config sources.">構成ソースのより関連性の高い側面の詳細は、<router-link to="/se/config/06_advanced-configuration">「高度なトピック」</router-link>のページを参照してください。</span></p>

</div>

<h2 id="parsers"><span class="merged" id="all.3oq9tm" title="原文 : Config Parsers">構成パーサー</span></h2>
<div class="section">
<p><span class="merged" id="all.1BGvRj.spl1" title="原文 : When it reads configuration text from sources, the config system uses config parsers to translate that text into the in-memory data structures representing that configuration.">ソースから構成テキストを読み取る場合、構成システムは構成パーサーを使用して、そのテキストをその構成を表すメモリー内データ構造に変換します。</span> <span class="merged" id="all.1BGvRj.spl2" title="原文 : The config system includes several built-in parsers, such as for the Java properties, YAML, JSON, and HOCON formats.">構成システムには、Javaプロパティ、YAML、JSON、HOCON形式などの組込みパーサーがいくつか含まれています。</span> <span class="merged" id="all.1BGvRj.spl3" title="原文 : See this section in the introduction for how to change your pom.xml to make parsers for those formats available to your application."><code>pom.xml</code>を変更して、これらのフォーマットのパーサーをアプリケーションで使用できるようにする方法は、<router-link :to="{path: '/se/config/01_introduction', hash: '#built-in-formats'}">「概要のこの項」</router-link>を参照してください。</span> <span class="merged" id="all.1BGvRj.spl4" title="原文 : Then your application can invoke the config builder&rsquo;s addParser method so that builder will use the parsers you choose.">その後、選択したパーサーがビルダーで使用されるように、アプリケーションで<a href="./apidocs/io.helidon.config/io/helidon/config/Config.Builder.html#addParser-io.helidon.config.spi.ConfigParser-" id="" target="_blank" >「構成ビルダー<code>addParser</code>」</a>メソッドを起動できます。</span> </p>
...
<p><span class="merged" id="all.2gNcdx.spl1" title="原文 : Even so, the config system allows your application to learn when such underlying changes in the data occur and respond accordingly.">そのような基礎となるデータの変更がいつ発生し、それに応じて応答できるかは、構成システムでもわかります。</span> <span class="merged" id="all.2gNcdx.spl2" title="原文 : The mutability section explains this in detail, and the PollingStrategies JavaDoc describes the built-in implementations.">これについては<router-link to="/se/config/05_mutability-support">「可変性」</router-link>の項で詳しく説明し、組込みの実装については<a href="./apidocs/io.helidon.config/io/helidon/config/PollingStrategies.html" id="" target="_blank" ><code>PollingStrategies</code></a> JavaDocで説明します。</span> <span class="merged" id="all.2gNcdx.spl3" title="原文 : You can, of course, write your own by implementing the PollingStrategy interface.">もちろん、<a href="./apidocs/io.helidon.config/io/helidon/config/spi/PollingStrategy.html" id="" target="_blank" ><code>PollingStrategy</code></a>インタフェースを実装することで独自のものを記述できます。</span> <span class="merged" id="all.2gNcdx.spl4" title="原文 : On a config source builder invoke pollingStrategy with an instance of your custom strategy and then invoke build to create the ConfigSource.">構成ソース・ビルダーで、カスタム戦略のインスタンスを使用して<code>pollingStrategy</code>を起動し、<code>build</code>を起動して<code>ConfigSource</code>を作成します。</span> </p>
