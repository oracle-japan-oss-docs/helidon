...
<h2 id="_profile_options"><span class="merged" id="all.4effQH"  title="原文:: Profile options">プロファイル・オプション</span></h2>
<div class="section">
...
<ol style="margin-left: 15px;">
<li>
<span class="merged" id="all.1bK0XE" title="原文 : Use a config source with a profile specific name"><router-link @click.native="this.scrollFix('#Profile-Source')" to="#Profile-Source">「プロファイル固有名」</router-link>で構成ソースの使用</span>
...
<h2 id="Profile-Source"><span class="merged" id="all.2ffT0X" title="原文 : Profile Config Sources">プロファイル構成ソース</span></h2>
<div class="section">
...
<p><span class="merged" id="all.3FIpz2" title="原文 : Let&rsquo;s consider the selected profile is dev, and we have yaml configuration support on classpath; config will look for the following sources (in this order):">選択したプロファイルは<code>dev</code>であり、クラスパスで<code>yaml</code>構成サポートがあるとします。構成では次のソースが検索されます(この順序):</span></p>
...
<p><span class="merged" id="all.2rj6oS" title="原文 : Let&rsquo;s consider the selected profile is dev, and we have yaml configuration support on classpath; config will look for the following profiles (in this order):">選択したプロファイルは<code>dev</code>であり、クラスパスで<code>yaml</code>構成サポートがあるとします。構成では次のプロファイルが(この順序で)検索されます:</span></p>
...
<p><span class="merged" id="all.4Nd9a9.spl1" title="原文 : If any of these files is discovered, it would be used to set up the configuration.">これらのファイルが見つかった場合は、構成の設定に使用されます。</span> <span class="merged" id="all.4Nd9a9.spl2" title="原文 : In case none is found, the config falls back to profile specific config sources.">見つからない場合、構成は<router-link @click.native="this.scrollFix('#Profile Config Sources')" to="#Profile Config Sources">「プロファイル固有の構成ソース」</router-link>に戻ります。</span> </p>

<p><span class="merged" id="all.3nUva2" title="原文 : The structure of the file is described below in profile file format.">ファイルの構造については、後述する<router-link @click.native="this.scrollFix('#Profile File Format')" to="#Profile File Format">「プロファイル・ファイル形式」</router-link>を参照してください。</span></p>
...
<th><span class="merged" id="all.3APT35.1"  title="原文:: Use">使用</span></th>
...
<p><span class="merged" id="all.17w9PS.spl1" title="原文 : Except for the system-properties and environment-variables types, the profile properties section for a source can also specify any optional settings for the corresponding config source type."><code>system-properties</code>および<code>environment-variables</code>型を除き、ソースのプロファイル<code>properties</code>セクションは、対応する構成ソース・オプションの設定を指定することもできます。</span> <span class="merged" id="all.17w9PS.spl2" title="原文 : The JavaDoc for the related config source type builders lists the supported properties for each type.">関連する構成ソース・タイプ・ビルダーのJavaDocには、各タイプでサポートされるプロパティがリストされています。</span> <span class="merged" id="all.17w9PS.spl3" title="原文 : (For example, FileConfigSource.FileBuilder.)">(たとえば、<a href="./apidocs/io.helidon.config/io/helidon/config/internal/FileConfigSource.FileBuilder.html" id="" target="_blank" ><code>FileConfigSource.FileBuilder</code></a>。)</span> </p>
...
<td class=""><span class="merged" id="all.4VkyxK" title="原文 : initial-delay-millis - delay between the start of the watcher and first check for changes"><code>initial-delay-millis</code> - ウォッチャの開始から変更を確認するための遅延</span></td>
</tr>
</tbody>
</table>
</div>
...
<th><span class="merged" id="all.JUOSC.10"  title="原文:: Usage">使用方法</span></th>
...
<p><span class="merged" id="all.fcyj7" title="原文 : See RetryPolicy, ChangeWatcher, and PollingStrategy JavaDoc sections."><a href="./apidocs/io.helidon.config/io/helidon/config/spi/RetryPolicy.html" id="" target="_blank" ><code>RetryPolicy</code></a>、<a href="./apidocs/io.helidon.config/io/helidon/config/spi/RetryPolicy.html" id="" target="_blank" ><code>ChangeWatcher</code></a>および<a href="./apidocs/io.helidon.config/io/helidon/config/spi/PollingStrategy.html" id="" target="_blank" ><code>PollingStrategy</code></a> JavaDocの各項を参照してください。</span></p>
