...
<p><span class="merged" id="all.4QLFGg" title="原文 : Helidon MP includes a complete OpenAPI example based on the MP quick-start sample app.">Helidon MPには、MPクイックスタート・サンプル・アプリケーションに基づく<a href="https://github.com/oracle/helidon/tree/2.5.4/examples/microprofile/openapi-basic" id="" target="_blank" >「完全なOpenAPIの例」</a>が含まれています。</span></p>
...
<p><span class="merged" id="all.451Pwe.spl1" title="原文 : This excerpt shows only a few annotations for illustration.">この抜粋は、例示のためにいくつかの注釈のみを示しています。</span> <span class="merged" id="all.451Pwe.spl2" title="原文 : The Helidon MP OpenAPI example illustrates more, and the MicroProfile OpenAPI spec describes them all."><a href="https://github.com/oracle/helidon/tree/2.5.4/examples/microprofile/openapi-basic" id="" target="_blank" >「Helidon MP OpenAPIの例」</a>ではさらに説明し、<a href="https://github.com/eclipse/microprofile-open-api/blob/master/spec/src/main/asciidoc/microprofile-openapi-spec.adoc" id="" target="_blank" >「MicroProfile OpenAPI仕様」</a>ではそれらすべてについて説明します。</span> </p>
...
<div class="section">
<p><span class="merged" id="all.2E3STj.spl1" title="原文 : Beyond the two config properties that denote the model reader and filter, Helidon MP OpenAPI supports a number of other mandated settings.">モデル・リーダーとフィルタを示す2つの構成プロパティ以外に、Helidon MP OpenAPIでは、他の多数の必須設定がサポートされています。</span> <span class="merged" id="all.2E3STj.spl2" title="原文 : These are described in the configuration section of the MicroProfile OpenAPI spec.">これらは、MicroProfile OpenAPI仕様の<a href="https://github.com/eclipse/microprofile-open-api/blob/master/spec/src/main/asciidoc/microprofile-openapi-spec.adoc#configuration" id="" target="_blank" >「構成セクション」</a>で説明されています。</span> </p>

<p><span class="merged" id="all.CwgmE" title="原文 : Helidon MP also supports additional properties specific to Helidon.">Helidon MPでは、Helidon固有の追加プロパティもサポートされます。</span></p>

<div class="block-title"><span class="merged" id="all.398FKU" title="原文 : Helidon-specific OpenAPI Config Properties"><span>Helidon-specific OpenAPI構成プロパティ</span></span></div>
...
<tbody>
<tr>
<td class=""><span class="merged" id="all.2AKve9"  title="原文: openapi.web-context"><code>openapi.web-context</code></span></td>
<td class=""><span class="merged" id="all.1btmrv" title="原文 : Path which serves the OpenAPI document (defaults to /openapi)">OpenAPIドキュメントを提供するパス(デフォルトは<code>/openapi</code>)</span></td>
</tr>
<tr>
<td class=""><span class="merged" id="all.2uHMSE"  title="原文: openapi.static-file"><code>openapi.static-file</code></span></td>
<td class=""><span class="merged" id="all.meaou" title="原文 : Full path to the static OpenAPI file (defaults to META-INF/openapi.yml, META-INF/openapi.yaml, or META-INF/openapi.json)">静的OpenAPIファイルへのフルパス(デフォルトは<code>META-INF/openapi.yml</code>、<code>META-INF/openapi.yaml</code>または<code>META-INF/openapi.json</code>)</span></td>
</tr>
</tbody>
</table>
</div>
<p><span class="merged" id="all.1XY8YW" title="原文 : Assign these settings in META-INF/microprofile-config.properties."><code>META-INF/microprofile-config.properties</code>でこれらの設定を割り当てます。</span></p>
