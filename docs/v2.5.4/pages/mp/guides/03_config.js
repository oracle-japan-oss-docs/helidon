...
<tbody>
<tr>
<td class=""><span class="merged" id="all.1hKNxI.1" title="原文 : A Helidon {upper-case-flavor} Application">Helidon {upper-case-flavor} アプリケーション</span></td>
<td class=""><span class="merged" id="all.3jMlMY.1" title="原文 : You can use your own application or use the Helidon {upper-case-flavor} Quickstart to create a sample application.">独自のアプリケーションを使用するか、<a href="https://helidon.io/docs/v2/#/{lower-case-flavor}/guides/02_quickstart" id="" target="_blank" >「Helidon {upper-case-flavor} クイックスタート」</a>を使用してサンプル・アプリケーションを作成できます。</span></td>
</tr>
<tr>
<td class=""><span class="merged" id="all.3UjXGj.1" title="原文 : Java&#160;SE&#160;11 (Open&#160;JDK&#160;11)"><a href="https://www.oracle.com/technetwork/java/javase/downloads" id="" target="_blank" >Java&#160;SE&#160;11</a> (<a href="http://jdk.java.net" id="" target="_blank" >Open&#160;JDK&#160;11</a>)</span></td>
<td class=""><span class="merged" id="all.3e6V0X.1" title="原文 : Helidon requires Java 11+.">HelidonにはJava 11以上が必要です。</span></td>
</tr>
<tr>
<td class=""><span class="merged" id="all.1nyTbh.3" title="原文 : Maven 3.6.1+"><a href="https://maven.apache.org/download.cgi" id="" target="_blank" >Maven 3.6.1+</a></span></td>
<td class=""><span class="merged" id="all.3woDk0.1" title="原文 : Helidon requires Maven 3.6.1+.">Helidonには、Maven 3.6.1+が必要です。</span></td>
</tr>
<tr>
<td class=""><span class="merged" id="all.1HtX9I.2"  title="原文:: Docker 18.09+"><a href="https://docs.docker.com/install/" id="" target="_blank" >Docker 18.09+</a></span></td>
...
<tr>
<td class=""><span class="merged" id="all.48aNQe.2"  title="原文:: Kubectl 1.16.5+"><a href="https://kubernetes.io/docs/tasks/tools/install-kubectl/" id="" target="_blank" >Kubectl 1.16.5+</a></span></td>
<td class=""><span class="merged" id="all.1tjPJQ.1" title="原文 : If you want to deploy to Kubernetes, you need kubectl and a Kubernetes cluster (you can install one on your desktop).">Kubernetesにデプロイする場合は、<code>kubectl</code>およびKubernetesクラスタ(<router-link to="/about/05_kubernetes">「デスクトップにインストール」</router-link>も可)が必要です。</span></td>
</tr>
</tbody>
</table>
</div>
...
<div class="section">
<p><span class="merged" id="all.1D5fgR.spl1" title="原文 : Helidon provides a very flexible and comprehensive configuration system, offering you many application configuration choices.">Helidonは、非常に柔軟で包括的な構成システムを提供し、多くのアプリケーション構成を選択できます。</span> <span class="merged" id="all.1D5fgR.spl2" title="原文 : You can include configuration data from a variety of sources using different formats, like JSON and YAML.">JSONやYAMLなどの様々な形式を使用して、様々なソースからの構成データを含めることができます。</span> <span class="merged" id="all.1D5fgR.spl3" title="原文 : Furthermore, you can customize the precedence of sources and make them optional or mandatory.">さらに、ソースの優先順位をカスタマイズして、オプションまたは必須にすることもできます。</span> <span class="merged" id="all.1D5fgR.spl4" title="原文 : This guide introduces Helidon MP configuration and demonstrates the fundamental concepts using several examples.">このガイドでは、Helidon MP構成について説明し、いくつかの例を使用して基本概念を示します。</span> <span class="merged" id="all.1D5fgR.spl5" title="原文 : Refer to Helidon Config for the full configuration concepts documentation.">構成の概念の詳細は、<router-link to="/se/config/01_introduction">「Helidon構成」</router-link>を参照してください。</span> </p>
...
<p class="admonition-inline"><span class="merged" id="all.2uqHBq.spl1" title="原文 : Because environment variable names are restricted to alphanumeric characters and underscores, Helidon adds aliases to the environment configuration source, allowing entries with dotted and/or hyphenated keys to be overriden.">環境変数名は英数字およびアンダースコアに制限されるため、Helidonは環境構成ソースに別名を追加し、ドットまたはハイフン(あるいはその両方)のキーを持つエントリを上書きできます。</span> <span class="merged" id="all.2uqHBq.spl2" title="原文 : For example, this mapping allows an environment variable named &quot;APP_GREETING&quot; to override an entry key named &quot;app.greeting&quot;.">たとえば、このマッピングを使用すると、"APP_GREETING"という環境変数で"app.greeting"というエントリ・キーをオーバーライドできます。</span> <span class="merged" id="all.2uqHBq.spl3" title="原文 : In the same way, an environment variable named &quot;APP_dash_GREETING&quot; will map to &quot;app-greeting&quot;.">同様に、"APP_dash_GREETING"という名前の環境変数が"app-greeting"にマップされます。</span> <span class="merged" id="all.2uqHBq.spl4" title="原文 : See Advanced Configuration for more information.">詳細は、<router-link to="/se/config/06_advanced-configuration">「詳細構成」</router-link>を参照してください。</span> </p>
</div>
...
<div class="section">
<p><span class="merged" id="all.2FqwyO.spl1" title="原文 : Helidon offers a variety of methods to access in-memory configuration.">Helidonには、インメモリー構成にアクセスするための様々なメソッドがあります。</span> <span class="merged" id="all.2FqwyO.spl2" title="原文 : These can be categorized as key access or tree navigation.">これらは、<em>「キー・アクセス」</em>または<em>「ツリー・ナビゲーション」</em>に分類できます。</span> <span class="merged" id="all.2FqwyO.spl3" title="原文 : You have been using key access for all of the examples to this point.">この時点までのすべての例で、<em>「キー・アクセス」</em>を使用してきました。</span> <span class="merged" id="all.2FqwyO.spl4" title="原文 : For example app.greeting is accessing the greeting child node of the app parent node.">たとえば、<code>app.greeting</code>が<code>app</code>親ノードの<code>greeting</code>子ノードにアクセスしているとします。</span> <span class="merged" id="all.2FqwyO.spl5" title="原文 : There are many options for access this data using navigation methods as described in Hierarchical Configuration and Advanced Configuration."><router-link to="/se/config/03_hierarchical-features">「階層構成」</router-link>および<router-link to="/se/config/06_advanced-configuration">「詳細構成」</router-link>で説明されているように、ナビゲーション・メソッドを使用してこのデータにアクセスするためのオプションは多数あります。</span> <span class="merged" id="all.2FqwyO.spl6" title="原文 : This simple example below demonstrates how to access a child node as a detached configuration sub-tree.">次の簡単な例では、デタッチされた構成サブツリーとして子ノードにアクセスする方法を示します。</span> </p>
...
<div class="section">
<p><span class="merged" id="all.LOEDp.spl1" title="原文 : This guide has demonstrated how to use basic Helidon configuration features.">このガイドでは、基本的なHelidon構成機能の使用方法について説明しました。</span> <span class="merged" id="all.LOEDp.spl2" title="原文 : For more information about using the advanced Helidon configuration features, including mutability support and extensions, see Helidon Configuration.">可変性のサポートや拡張など、高度なHelidon構成機能の使用方法の詳細は、<router-link to="/se/config/01_introduction">「Helidon構成」</router-link>を参照してください。</span> </p>
