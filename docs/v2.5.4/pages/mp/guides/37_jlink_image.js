...
<div class="section">
<p><span class="merged" id="all.2qcB0R.spl1" title="原文 : JDK 9 introduced the jlink command that supports assembling a set of modules and their dependencies into a custom runtime image.">JDK 9では、一連のモジュールとその依存関係のカスタム・ランタイム・イメージへのアセンブルをサポートする<a href="https://docs.oracle.com/en/java/javase/11/tools/jlink.html" id="" target="_blank" ><code>jlink</code></a>コマンドが導入されました。</span> <span class="merged" id="all.2qcB0R.spl2" title="原文 : The helidon-maven-plugin has support for easily creating a custom runtime image for your Helidon application resulting in a smaller, better performing runtime."><code>helidon-maven-plugin</code>では、Helidonアプリケーションのカスタム・ランタイム・イメージの簡単な作成がサポートされているため、実行時間が小さくなり、パフォーマンスが向上します。</span> </p>
...
<tbody>
<tr>
<td class=""><span class="merged" id="all.1hKNxI.10" title="原文 : A Helidon {upper-case-flavor} Application">Helidon {upper-case-flavor} アプリケーション</span></td>
<td class=""><span class="merged" id="all.3jMlMY.10" title="原文 : You can use your own application or use the Helidon {upper-case-flavor} Quickstart to create a sample application.">独自のアプリケーションを使用するか、<a href="https://helidon.io/docs/v2/#/{lower-case-flavor}/guides/02_quickstart" id="" target="_blank" >「Helidon {upper-case-flavor} クイックスタート」</a>を使用してサンプル・アプリケーションを作成できます。</span></td>
</tr>
<tr>
<td class=""><span class="merged" id="all.3UjXGj.11" title="原文 : Java&#160;SE&#160;11 (Open&#160;JDK&#160;11)"><a href="https://www.oracle.com/technetwork/java/javase/downloads" id="" target="_blank" >Java&#160;SE&#160;11</a> (<a href="http://jdk.java.net" id="" target="_blank" >Open&#160;JDK&#160;11</a>)</span></td>
<td class=""><span class="merged" id="all.3e6V0X.11" title="原文 : Helidon requires Java 11+.">HelidonにはJava 11以上が必要です。</span></td>
</tr>
<tr>
<td class=""><span class="merged" id="all.1nyTbh.13" title="原文 : Maven 3.6.1+"><a href="https://maven.apache.org/download.cgi" id="" target="_blank" >Maven 3.6.1+</a></span></td>
<td class=""><span class="merged" id="all.3woDk0.11" title="原文 : Helidon requires Maven 3.6.1+.">Helidonには、Maven 3.6.1+が必要です。</span></td>
</tr>
<tr>
<td class=""><span class="merged" id="all.1HtX9I.12"  title="原文:: Docker 18.09+"><a href="https://docs.docker.com/install/" id="" target="_blank" >Docker 18.09+</a></span></td>
...
<tr>
<td class=""><span class="merged" id="all.48aNQe.12"  title="原文:: Kubectl 1.16.5+"><a href="https://kubernetes.io/docs/tasks/tools/install-kubectl/" id="" target="_blank" >Kubectl 1.16.5+</a></span></td>
<td class=""><span class="merged" id="all.1tjPJQ.11" title="原文 : If you want to deploy to Kubernetes, you need kubectl and a Kubernetes cluster (you can install one on your desktop).">Kubernetesにデプロイする場合は、<code>kubectl</code>およびKubernetesクラスタ(<router-link to="/about/05_kubernetes">「デスクトップにインストール」</router-link>も可)が必要です。</span></td>
</tr>
</tbody>
</table>
</div>
...
<div class="section">
<p><span class="merged" id="all.2Ic8Sa" title="原文 : As noted in the prerequisites above, JDK 11 or newer is required.">前述の前提条件に示すように、JDK 11以降が必要です。</span></p>
...
<p ><span class="merged" id="all.23C1fx.spl1" title="原文 : RPM based distributions provide *.jmod files in separate java-*-openjdk-jmods packages."><a href="https://en.wikipedia.org/wiki/List_of_Linux_distributions#RPM-based" id="" target="_blank" >「RPMベース」</a>ディストリビューションでは、<code>*.jmod</code>ファイルが個別の<code>java-*-openjdk-jmods</code>パッケージで提供されます。</span> <span class="merged" id="all.23C1fx.spl2" title="原文 : Debian based distributions provide *.jmod files only in the openjdk-*-jdk-headless packages."><a href="https://en.wikipedia.org/wiki/List_of_Linux_distributions#Debian-based" id="" target="_blank" >「Debianベース」</a>ディストリビューションは、<code>openjdk-*-jdk-headless</code>パッケージでのみ<code>*.jmod</code>ファイルを提供します。</span> </p>
</div>
</div>
...
<p><span class="merged" id="all.3DbUPR" title="原文 : For more information about the Quickstart application and other enpoints it supports see the Helidon MP quickstart Guide.">Quickstartアプリケーションおよびそれがサポートするその他のエンドポイントの詳細は、<router-link to="/mp/guides/02_quickstart">「Helidon MPクイックスタート・ガイド」</router-link>を参照してください。</span></p>
...
<p><span class="merged" id="all.4P1eEO" title="原文 : If you&rsquo;d rather have a smaller image size (with a slightly increased startup time) you can skip the creation of the CDS archive by executing your build like this:">(起動時間を少し長くして)イメージ・サイズを小さくする場合は、次のようにビルドを実行してCDSアーカイブの作成をスキップできます:</span></p>
...
<p><span class="merged" id="all.38WD5I" title="原文 : You can exercise the application&rsquo;s endpoints as before.">以前と同様に、アプリケーションのエンドポイントを実行できます。</span></p>
...
<p><span class="merged" id="all.dhYqv" title="原文 : For cases where absolute minimal startup time and image size are required, then consider using GraalVM Native Images.">起動時間とイメージの絶対最小サイズが必要な場合は、<router-link to="/mp/guides/36_graalnative">「GraalVMネイティブ・イメージ」</router-link>の使用を検討してください。</span></p>
