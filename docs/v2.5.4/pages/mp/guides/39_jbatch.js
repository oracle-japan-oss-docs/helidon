...
<tbody>
<tr>
<td class=""><span class="merged" id="all.1hKNxI.13" title="原文 : A Helidon {upper-case-flavor} Application">Helidon {upper-case-flavor} アプリケーション</span></td>
<td class=""><span class="merged" id="all.3jMlMY.13" title="原文 : You can use your own application or use the Helidon {upper-case-flavor} Quickstart to create a sample application.">独自のアプリケーションを使用するか、<a href="https://helidon.io/docs/v2/#/{lower-case-flavor}/guides/02_quickstart" id="" target="_blank" >「Helidon {upper-case-flavor} クイックスタート」</a>を使用してサンプル・アプリケーションを作成できます。</span></td>
</tr>
<tr>
<td class=""><span class="merged" id="all.3UjXGj.14" title="原文 : Java&#160;SE&#160;11 (Open&#160;JDK&#160;11)"><a href="https://www.oracle.com/technetwork/java/javase/downloads" id="" target="_blank" >Java&#160;SE&#160;11</a> (<a href="http://jdk.java.net" id="" target="_blank" >Open&#160;JDK&#160;11</a>)</span></td>
<td class=""><span class="merged" id="all.3e6V0X.14" title="原文 : Helidon requires Java 11+.">HelidonにはJava 11以上が必要です。</span></td>
</tr>
<tr>
<td class=""><span class="merged" id="all.1nyTbh.16" title="原文 : Maven 3.6.1+"><a href="https://maven.apache.org/download.cgi" id="" target="_blank" >Maven 3.6.1+</a></span></td>
<td class=""><span class="merged" id="all.3woDk0.14" title="原文 : Helidon requires Maven 3.6.1+.">Helidonには、Maven 3.6.1+が必要です。</span></td>
</tr>
<tr>
<td class=""><span class="merged" id="all.1HtX9I.15"  title="原文:: Docker 18.09+"><a href="https://docs.docker.com/install/" id="" target="_blank" >Docker 18.09+</a></span></td>
...
<tr>
<td class=""><span class="merged" id="all.48aNQe.15"  title="原文:: Kubectl 1.16.5+"><a href="https://kubernetes.io/docs/tasks/tools/install-kubectl/" id="" target="_blank" >Kubectl 1.16.5+</a></span></td>
<td class=""><span class="merged" id="all.1tjPJQ.14" title="原文 : If you want to deploy to Kubernetes, you need kubectl and a Kubernetes cluster (you can install one on your desktop).">Kubernetesにデプロイする場合は、<code>kubectl</code>およびKubernetesクラスタ(<router-link to="/about/05_kubernetes">「デスクトップにインストール」</router-link>も可)が必要です。</span></td>
</tr>
</tbody>
</table>
</div>
...
<p class="admonition-inline"><span class="merged" id="all.2r9fRN" title="原文 : This guide assumes you are familiar with the Jakarta Batch project specification from the Ecplipse Foundation project site.">このガイドでは、Ecplipse Foundationプロジェクト・サイトの<a href="https://projects.eclipse.org/projects/ee4j.batch" id="" target="_blank" >「Jakartaバッチ・プロジェクト仕様」</a>について理解していることを前提としています。</span></p>
</div>
</div>

<h2 id="_dependencies"><span class="merged" id="all.4R2wMP.1"  title="原文:: Dependencies">依存関係</span></h2>
<div class="section">
...
<div class="section">
<p><span class="merged" id="all.13FC2b.spl1" title="原文 : In this demonstration you will first create sample input and output records and then the following jobs: MyItemReader, MyItemProcessor and MyItemWriter.">このデモンストレーションでは、最初にサンプル入力レコードと出力レコードを作成し、次に次のジョブを作成: <code>MyItemReader</code>、<code>MyItemProcessor</code>および<code>MyItemWriter</code>。</span> <span class="merged" id="all.13FC2b.spl2" title="原文 : Finally you will create MyBatchlet to demonstrate all possible usages of JBatch.">最後に、<code>MyBatchlet</code>を作成して、JBatchの使用可能なすべての内容を示します。</span> </p>


<h4 id="_1_create_a_unit_of_input_information"><span class="merged" id="all.2mViVA" title="原文 : 1. Create a unit of input information">1. 入力情報の単位の作成</span></h4>
<div class="section">
<markup
lang="java"
title="MyInputRecord"
...
<h4 id="_6_create_mybatchlet_to_extentd_abstractbatchlet"><span class="merged" id="all.JwDyH" title="原文 : 6. Create MyBatchlet to extentd AbstractBatchlet">6.エクステント<code>AbstractBatchlet</code>への<code>MyBatchlet</code>の作成</span></h4>
<div class="section">
...
<p><span class="merged" id="all.l8lyt.spl1" title="原文 : Helidon specifies to JBatch that it should run in Standalone (SE) mode.">Helidonは、スタンドアロン(SE)モードで実行する必要があるJBatchを指定します。</span> <span class="merged" id="all.l8lyt.spl2" title="原文 : It will also register the HelidonExecutorServiceProvider which is actually relatively small.">また、実際には比較的小さい<code>HelidonExecutorServiceProvider</code>も登録されます。</span> <span class="merged" id="all.l8lyt.spl3" title="原文 : For our example we need something really small, like a FixedTheadPool with 2 threads.">この例では、2つのスレッドを持つ<code>FixedTheadPool</code>など、非常に小さいものが必要です。</span> <span class="merged" id="all.l8lyt.spl4" title="原文 : This provider is used to tell our JBatch engine exactly which ExecutorSevice to use.">このプロバイダは、使用するExecutorSeviceを正確にJBatchエンジンに通知するために使用されます。</span> </p>
...
<div class="admonition note">
<p class="admonition-inline"><span class="merged" id="all.4M9URW" title="原文 : In this example the job ID is 1, but make sure that you enter your specfic job ID in the string.">この例では、ジョブIDは1ですが、文字列にスペック・ジョブIDを入力してください。</span></p>
</div>
