...
<div class="section">
<p><span class="merged" id="all.2DrVNQ" title="原文 : The config system represents configuration in memory using three types of nodes, each a different interface defined within the ConfigNode interface.">configシステムは、<a href="./apidocs/io.helidon.config/io/helidon/config/spi/ConfigNode.html" id="" target="_blank" ><code>ConfigNode</code></a>インタフェース内で定義された異なるインタフェースを持つ3つのタイプのノードを使用してメモリー内の構成を表します。</span></p>
...
<div class="section">
<p><span class="merged" id="all.5jIF8.spl1" title="原文 : The following example is in HOCON (human-optimized config object notation) format.">次の例は、<a href="https://github.com/lightbend/config/blob/master/HOCON.md" id="" target="_blank" >HOCON</a> (人間が最適化した構成オブジェクト表記)形式です。</span> <span class="merged" id="all.5jIF8.spl2" title="原文 : The config system supports HOCON as an extension module.">構成システムでは、HOCONが<router-link @click.native="this.scrollFix('#Config-ModuleHocon')" to="#Config-ModuleHocon">「拡張モジュール」</router-link>としてサポートされます。</span> </p>

<markup
lang="hocon"
title="HOCON <code>application.conf</code>ファイル"
>app {
    greeting = "Hello"
    page-size = 20
    basic-range = [ -20, 20 ]
...
<v-card> <v-card-text class="overflow-y-hidden" style="text-align:center"> <img alt="ロードされた構成ノード構造" src="./images/config/application_conf-nodes.png" /> </v-card-text> </v-card> <v-card flat color="grey lighten-3" class="card__example"> <v-card-text><p><span class="merged" id="all.3Wy1iS.1"  title="原文:: Notes">ノート</span></p>
...
<p><span class="merged" id="all.2SkX0l" title="原文 : The Config object exposes methods to return the name, key, and type of the node."><code>Config</code>オブジェクトは、ノードの<a href="./apidocs/io.helidon.config/io/helidon/config/Config.html#name--" id="" target="_blank" ><code>name</code></a>、<a href="./apidocs/io.helidon.config/io/helidon/config/Config.html#key--" id="" target="_blank" ><code>key</code></a>および<a href="./apidocs/io.helidon.config/io/helidon/config/Config.html#type--" id="" target="_blank" ><code>type</code></a>を返すメソッドを公開します。</span></p>
...
<p><span class="merged" id="all.1qIrbM" title="原文 : All of the following lines retrieve the same Config node.">次の行はすべて、同じ<code>Config</code>ノードを取得します。</span></p>
...
<td class=""><span class="merged" id="all.18ghG8" title="原文 : Returns the ConfigValue with the correct type that has all methods of Optional and a few additional ones - see ConfigValue interface."><code>Optional</code>のすべてのメソッドといくつかの追加メソッドを持つ正しいタイプの<code>ConfigValue</code>を返します - <a href="./apidocs/io.helidon.config/io/helidon/config/ConfigValue.html" id="" target="_blank" ><code>ConfigValue</code></a>インタフェースを参照してください。</span></td>
</tr>
</tbody>
</table>
</div>
...
<markup
lang="java"
title="<em>「オブジェクト」</em>ノードの子ノードの名前のリスト"
...
<markup
lang="java"
title="<em>「リスト」</em>ノードの下のサブツリーを移動"
...
<markup
lang="java"
title="<em>root</em> (<em>オブジェクト</em>)ノードを横断し、<code>data</code>サブツリー全体をスキップ"
...
<p><span class="merged" id="all.d4lZA.spl1" title="原文 : For example, the application.properties from the introduction section contains several settings prefixed with web such as web.page-size.">たとえば、概要セクションの<router-link @click.native="this.scrollFix('#create-simple-config-props')" to="#create-simple-config-props"><code>application.properties</code></router-link>には、<code>web.page-size</code>など、<code>web</code>のプレフィクスが付いたいくつかの設定が含まれています。</span> <span class="merged" id="all.d4lZA.spl2" title="原文 : Perhaps in another config source the same information might be stored as server.web.page-size:">別の構成ソースに、同じ情報が<code>server.web.page-size</code>として格納されている場合があります:</span> </p>
...
<p><span class="merged" id="all.3KXoyg.spl1" title="原文 : One easy way to do this is to detach a subtree from a larger config tree.">これを簡単に行うには、より大きな構成ツリーからのサブツリーを<em>「デタッチ」</em>します。</span> <span class="merged" id="all.3KXoyg.spl2" title="原文 : When your application invokes the Config.detach method it gets back a copy of the config node but with no parent.">アプリケーションが<a href="./apidocs/io.helidon.config/io/helidon/config/Config.html#detach--" id="" target="_blank" ><code>Config.detach</code></a>メソッドを呼び出すと、親を持たない構成ノードの<em>「コピー」</em>が返されます。</span> <span class="merged" id="all.3KXoyg.spl3" title="原文 : The copy and the original node both point to the same objects for their child nodes (if any).">コピーと元のノードの両方が、子ノード(存在する場合)の同じオブジェクトを指しています。</span> <span class="merged" id="all.3KXoyg.spl4" title="原文 : The original node is unchanged.">元のノードは変更されません。</span> </p>
