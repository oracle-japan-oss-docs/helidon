...
<h3 id="_your_first_config_application"><span class="merged" id="all.3fN8Aw" title="原文 : Your First Config Application">最初の構成アプリケーション</span></h3>
<div class="section">
<p><span class="merged" id="all.113ORH" title="原文 : An easy way to start with the Config API is to follow these four steps:"><a href="./apidocs/io.helidon.config/io/helidon/config/Config.html" id="" target="_blank" >Config</a> APIを開始する簡単な方法は、次の4つのステップに従います:</span></p>
...
<li>
<span class="merged" id="all.2yDtmJ" title="原文 : revise your module-info.java to refer to config (if you are using Java 9+)"><router-link @click.native="this.scrollFix('#update-module-info')" to="#update-module-info">configを参照するように<code>module-info.java</code>を変更します(Java 9+を使用している場合)</router-link></span>

</li>
<li>
<span class="merged" id="all.1kWoCH" title="原文 : create a simple config properties file"><router-link @click.native="this.scrollFix('#create-simple-config-props')" to="#create-simple-config-props">単純な構成プロパティ・ファイルの作成</router-link></span>

</li>
<li>
<span class="merged" id="all.2K5iat" title="原文 : retrieve and use the default Config from your app"><router-link @click.native="this.scrollFix('#Config-Basics-DefaultConfig')" to="#Config-Basics-DefaultConfig">アプリケーションからのデフォルトの<code>Config</code>の取得および使用</router-link></span>
...
<h4 id="update-module-info"><span class="merged" id="all.1W5vmn" title="原文 : Update module-info.java"><code>module-info.java</code>の更新</span></h4>
<div class="section">
<p><span class="merged" id="all.CBgze" title="原文 : If you are using Java 11 then create or update the module-info.java file for your application:">Java 11を使用している場合は、アプリケーションの<code>module-info.java</code>ファイルを作成または更新します:</span></p>
...
<h4 id="create-simple-config-props"><span class="merged" id="all.3NNFv0" title="原文 : Create simple Config Properties File">単純な構成プロパティ・ファイルの作成</span></h4>
<div class="section">
<markup

title="<code>src/main/resources/application.properties</code>構成ファイルの例"
>greeting = Hello
...
<h4 id="Config-Basics-DefaultConfig"><span class="merged" id="all.48CPC5" title="原文 : Write Code using the Default Config">デフォルト構成を使用したコードの記述</span></h4>
<div class="section">
<markup
lang="java"
title="Javaからのデフォルト<code>Config</code>の作成および使用"
...
<ul class="colist">
<li data-value="1"><span class="merged" id="all.2mJOPS" title="原文 : Import Config."><code>Config</code>のインポート。</span></li>
<li data-value="2"><span class="merged" id="all.32hPxu" title="原文 : Create the root of the Config tree from the default sources.">デフォルト・ソースから<code>Config</code>ツリーのルートを作成します。</span></li>
<li data-value="3"><span class="merged" id="all.33Arhy" title="原文 : Retrieve various values by their dotted names and decode them as the appropriate Java types, providing default values if the property is missing.">ドット表記名を使用して様々な値を取得し、適切なJavaタイプとしてデコードし、プロパティがない場合はデフォルト値を指定します。</span></li>
<li data-value="4"><span class="merged" id="all.2hBtIM" title="原文 : Retrieve the value (and fail with a runtime exception if missing)">値を取得します(値がない場合はランタイム例外で失敗します)</span></li>
</ul>
<p><span class="merged" id="all.4NIrpF" title="原文 : When you build and run your project, the output will look like this:">プロジェクトをビルドして実行すると、出力は次のようになります:</span></p>
...
<tbody>
<tr>
<td class=""><span class="merged" id="all.Fz2xo.1"  title="原文: application.yaml"><code>application.yaml</code></span></td>
...
<li>
<p><span class="merged" id="all.3nV5da" title="原文 : accessing the data once loaded, and">ロード後のデータへのアクセス</span></p>
...
<li>
<p><span class="merged" id="all.2incH0" title="原文 : creating a meta-configuration file on the runtime classpath or file system to control how the config system prepares the default configuration.">ランタイム・クラスパスまたはファイル・システムに<router-link @click.native="this.scrollFix('#Config-Advanced-metaconfig')" to="#Config-Advanced-metaconfig">meta-configuration</router-link>ファイルを作成して、構成システムがデフォルト構成を準備する方法を制御します。</span></p>
...
<td class=""><span class="merged" id="all.277sxr" title="原文 : Config sources, meta-configuration"><router-link @click.native="this.scrollFix('#config/02_config-sources.adoc')" to="#config/02_config-sources.adoc">「構成ソース」</router-link>, <router-link @click.native="this.scrollFix('#metaconfig')" to="#metaconfig">meta-configuration</router-link></span></td>
</tr>
<tr>
<td class=""><span class="merged" id="all.b4eLP" title="原文 : What format config data is expressed in">構成データの表現形式</span></td>
<td class=""><span class="merged" id="all.tfLtT" title="原文 : Config parsers, supported formats"><router-link @click.native="this.scrollFix('#parsers')" to="#parsers">「構成パーサー」</router-link>, <router-link @click.native="this.scrollFix('#config/08_supported-formats.adoc')" to="#config/08_supported-formats.adoc">「サポートされる形式」</router-link></span></td>
</tr>
<tr>
...
<td class=""><span class="merged" id="all.4BMnyL" title="原文 : Filters and overrides"><router-link @click.native="this.scrollFix('#filters-and-overrides')" to="#filters-and-overrides">フィルタおよびオーバーライド</router-link></span></td>
</tr>
<tr>
<td class=""><span class="merged" id="all.3WHjPi" title="原文 : What happens when config data changes">構成データが変更された場合の処理</span></td>
<td class=""><span class="merged" id="all.2pLtQZ" title="原文 : Config polling"><router-link @click.native="this.scrollFix('#polling')" to="#polling">構成ポーリング</router-link></span></td>
</tr>
<tr>
...
<td class=""><span class="merged" id="all.44hemG" title="原文 : Config mappers"><router-link @click.native="this.scrollFix('#config/04_property-mapping.adoc')" to="#config/04_property-mapping.adoc">構成マッパー</router-link></span></td>
</tr>
<tr>
<td class=""><span class="merged" id="all.DFNVK" title="原文 : How to navigate config trees">構成ツリーのナビゲート方法</span></td>
<td class=""><span class="merged" id="all.pIFwE" title="原文 : Navigation"><router-link @click.native="this.scrollFix('#config/03_hierarchical-features.adoc')" to="#config/03_hierarchical-features.adoc">ナビゲーション</router-link></span></td>
</tr>
</tbody>
</table>
</div>
...
<td class=""><span class="merged" id="all.f1n7z" title="原文 : Extensions"><router-link @click.native="this.scrollFix('#config/07_extensions.adoc')" to="#config/07_extensions.adoc">Extensions</router-link></span></td>
</tr>
</tbody>
</table>
</div>
