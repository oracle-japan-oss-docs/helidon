...
<markup
lang="java"
title="<code>module-info.java</code>でのYAML依存関係の構成"
>module myModule {
...
<markup
lang="java"
title="YAMLパーサーの指定 - ソースにファイル・タイプがありません"
...
<markup
lang="java"
title="パーサー・サービスが無効になっているためYAMLパーサーが指定されました"
...
<markup
lang="java"
title="HOCONパーサーの指定 - ソースにファイル・タイプがありません"
>Config config = Config.create(classpath("my-config")                                     <span class="conum" data-value="1" />
...
<markup
lang="java"
title="パーサー・サービスが無効なため、HOCONパーサーが指定されました"
...
<div class="section">
<p><span class="merged" id="all.3XllOW.spl1" title="原文 : The config system can load information about config sources from meta-configuration rather than requiring your application to construct the builder.">構成システムは、アプリケーションでビルダーを構築する必要なく、<router-link @click.native="this.scrollFix('#Config-Advanced-Config-MetaConfig')" to="#Config-Advanced-Config-MetaConfig">meta-configuration</router-link>から構成ソースに関する情報をロードできます。</span> <span class="merged" id="all.3XllOW.spl2" title="原文 : To read meta-configuration from an Etcd source set the following required properties for the source:">Etcdソースからメタ構成を読み取るには、ソースに次の必須プロパティを設定します:</span> </p>
...
<markup
lang="YAML"
title="etcdソースのメタ構成<code>config-meta-etcd.yaml</code>"
>sources:
...
<p><span class="merged" id="all.lX39o.spl1" title="原文 : You can also implemention your own polling strategy by implementing PollingStrategy."><a href="./apidocs/io.helidon.config/io/helidon/config/spi/PollingStrategy.html" id="" target="_blank" ><code>PollingStrategy</code></a>を実装することで、独自のポーリング戦略を実装することもできます。</span> <span class="merged" id="all.lX39o.spl2" title="原文 : See the mutability support and polling strategy discussions."><router-link @click.native="this.scrollFix('#config/05_mutability-support.adoc')" to="#config/05_mutability-support.adoc">「可変性のサポート」</router-link>および<router-link @click.native="this.scrollFix('#Config-SPI-PollingStrategy')" to="#Config-SPI-PollingStrategy">「ポーリング計画」</router-link>のディスカッションを参照してください。</span> </p>
...
<markup
lang="YAML"
title="gitソースのメタ構成<code>config-meta-git.yaml</code>"
>sources:
