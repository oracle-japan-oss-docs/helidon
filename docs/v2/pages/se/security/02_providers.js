...
<td class=""><span class="merged" id="all.1wtIb3.1" title="原文 : Issuer of token - each JWT is validated to check the issuer">トークンの発行者 - 各JWTは、発行者をチェックするために検証されます</span></td>
</tr>
<tr>
<td class=""><span class="merged" id="all.4WSwQo.1"  title="原文: audience"><code>audience</code></span></td>
<td class="">&#160;</td>
<td class=""><span class="merged" id="all.3aGKE2.1" title="原文 : Audience of a token - each JWT is validated to check the audience">トークンの対象者 - 各JWTは、オーディエンスをチェックするために検証されます</span></td>
</tr>
<tr>
...
<td class=""><span class="merged" id="all.42DdBS.12"  title="原文: false"><code>false</code></span></td>
...
<td class=""><span class="merged" id="all.4bhk53.1" title="原文 : When true - validate against jwk defined by &quot;sign-jwk&quot;, when false validate JWT through OIDC Server endpoint &quot;validation-endpoint-uri&quot;">trueの場合 - "sign-jwk"で定義されたjwkに対して検証し、falseの場合はOIDCサーバー・エンドポイント"validation-endpoint-uri"を介してJWTを検証</span></td>
</tr>
<tr>
<td class=""><span class="merged" id="all.8wfMM.1"  title="原文: sign-jwk.resource"><code>sign-jwk.resource</code></span></td>
<td class=""><span class="merged" id="all.fgGdF.1" title="原文 : &quot;jwks-uri&quot; in OIDC metadata, or identity-uri/admin/v1/SigningCert/jwk if not available, only needed when jwt validation is done by us">OIDCメタデータの"jwks-uri"、または使用できない場合は<code>identity-uri/admin/v1/SigningCert/jwk</code>(jwt検証が行われる場合のみ必要)</span></td>
<td class=""><span class="merged" id="all.1YuYeB.1.spl1" title="原文 : A resource pointing to JWK with public keys of signing certificates used to validate JWT.">JWTの検証に使用される署名証明書の公開キーを持つJWKを指すリソース。</span> <span class="merged" id="all.1YuYeB.1.spl2" title="原文 : See Resource.create(io.helidon.config.Config)">Resource.create(io.helidon.config.Config)を参照</span> </td>
</tr>
<tr>
...
<li>
<span class="merged" id="all.3wWz4X.1" title="原文 : client-id, client-secret, and identityUri are validated - these must provide values"><code>client-id</code>、<code>client-secret</code>および<code>identityUri</code>が検証されます - これらは値を指定する必要があります</span>
...
<li>
<span class="merged" id="all.4V3YQL.1" title="原文 : We validate the JWT signature either against local JWK or against the identity server&rsquo;s introspection endpoint depending on configuration">JWTシグネチャは、構成に応じて、ローカルJWKまたはアイデンティティ・サーバーのイントロスペクション・エンドポイントのいずれかに対して検証されます</span>

</li>
<li>
<span class="merged" id="all.3Rdm1A.1" title="原文 : We validate the issuer and audience of the token if it matches the configured values">トークンの発行者とオーディエンスが構成済の値と一致するかどうかを検証</span>
...
<li>
<span class="merged" id="all.4WPiBt.1" title="原文 : We validate that we have sufficient scopes to proceed, and return 403 if not">続行するのに十分なスコープがあることを検証し、ない場合は<code>403</code>を返します</span>
...
<td class=""><span class="merged" id="all.4KM3rs.4.spl1" title="原文 : If set to true, failure to authenticate will return ABSTAIN result instead of FAILURE."><code>true</code>に設定した場合、認証に失敗すると、<code>FAILURE</code>ではなく<code>ABSTAIN</code>の結果が返されます。</span> <span class="merged" id="all.4KM3rs.4.spl2" title="原文 : This is an important distinction when more than one provider is used">これは、複数のプロバイダを使用する場合の重要な違いです</span> </td>
</tr>
<tr>
...
<td class=""><span class="merged" id="all.2Qehz5.4"  title="原文: none"><code>none</code></span></td>
...
<td class=""><span class="merged" id="all.4KM3rs.5.spl1" title="原文 : If set to true, failure to authenticate will return ABSTAIN result instead of FAILURE."><code>true</code>に設定した場合、認証に失敗すると、<code>FAILURE</code>ではなく<code>ABSTAIN</code>の結果が返されます。</span> <span class="merged" id="all.4KM3rs.5.spl2" title="原文 : This is an important distinction when more than one provider is used">これは、複数のプロバイダを使用する場合の重要な違いです</span> </td>
</tr>
<tr>
...
<td class=""><span class="merged" id="all.34BiJ2.1"  title="原文: [SIGNATURE,AUTHORIZATION]"><code>[SIGNATURE,AUTHORIZATION]</code></span></td>
<td class=""><span class="merged" id="all.lTc0e.1" title="原文 : Headers to look for inbound signatures and to store outbound signatures">インバウンド・シグネチャを検索し、アウトバウンド・シグネチャを保存するためのヘッダー</span></td>
</tr>
<tr>
...
<td class=""><span class="merged" id="all.PuOzo.1" title="原文 : &quot;Unvalidated&quot; means: an attribute is defined, but there is no validator available for it">「未検証」とは、: 属性が定義されていますが、使用可能なバリデータがありません</span></td>
</tr>
<tr>
...
<td class=""><span class="merged" id="all.2kIejh.1" title="原文 : &quot;None validated&quot; means: there was not a single attribute that was validated">「検証なし」は、: 検証された単一の属性がありませんでした</span></td>
</tr>
</tbody>
</table>
</div>
...
<div class="section">
<p><span class="merged" id="all.nxxf6.1" title="原文 : ABAC uses available validators and validates them against attributes of the authenticated user.">ABACは、使用可能なバリデータを使用して、認証されたユーザーの属性に対して検証します。</span></p>
...
<ol style="margin-left: 15px;">
<li>
<span class="merged" id="all.VE4Of.1" title="原文 : true &amp; true: Will fail if any attribute is not validated and if any has failed validation"><code>true</code> &amp; <code>true</code>: 検証されていない属性がある場合、および検証に失敗した属性がある場合は失敗</span>
...
<li>
<span class="merged" id="all.49BjyI.1" title="原文 : false &amp; false: Will fail if there is any attribute that failed validation, Will NOT fail if there are no failed validation or if there are NONE validated"><code>false</code> &amp; <code>false</code>: 検証に失敗した属性がある場合は失敗し、検証に失敗しなかった場合またはNONEが検証されている場合は失敗しません</span>
...
<p><span class="merged" id="all.11tgoV.1.spl1" title="原文 : This provider checks that all defined ABAC validators are validated.">このプロバイダは、定義されているすべてのABACバリデータが検証されていることを確認します。</span> <span class="merged" id="all.11tgoV.1.spl2" title="原文 : If there is a definition for a validator that is not checked, the request is denied (depending on configuration as mentioned above).">チェックされていないバリデータの定義がある場合、(前述の構成に応じて)リクエストは拒否されます。</span> </p>
...
<td class=""><span class="merged" id="all.4KM3rs.6.spl1" title="原文 : If set to true, failure to authenticate will return ABSTAIN result instead of FAILURE."><code>true</code>に設定した場合、認証に失敗すると、<code>FAILURE</code>ではなく<code>ABSTAIN</code>の結果が返されます。</span> <span class="merged" id="all.4KM3rs.6.spl2" title="原文 : This is an important distinction when more than one provider is used">これは、複数のプロバイダを使用する場合の重要な違いです</span> </td>
</tr>
<tr>
...
<ol style="margin-left: 15px;">
<li>
<span class="merged" id="all.1XFk0r.1" title="原文 : Validate if it timed out locally">ローカルでタイムアウトしたかどうかを検証</span>
...
<td class="">&#160;</td>
<td class=""><span class="merged" id="all.2IbXDf.1.spl1" title="原文 : Configuration of the JWK to obtain key(s) to validate signatures of inbound token.">インバウンド・トークンのシグネチャを検証するキーを取得するためのJWKの構成。</span> <span class="merged" id="all.2IbXDf.1.spl2" title="原文 : The JWK should contain public keys.">JWKには公開キーを含める必要があります。</span> <span class="merged" id="all.2IbXDf.1.spl3" title="原文 : This may be: jwk.resource.path, jwk.resource.resource-path, jwk.resource.url, jwk.resource.content-plain (actual JSON string), jwk.resource.content (base64)">次のようになります: jwk.resource.path、jwk.resource.resource-path、jwk.resource.url、jwk.resource.content-plan (実際のJSON文字列)、jwk.resource.content (base64)</span> </td>
</tr>
<tr>
...
<td class="">&#160;</td>
<td class=""><span class="merged" id="all.212d4a.1" title="原文 : When we issue a new token, this is the issuer to be placed into it (validated by target service)">新しいトークンを発行すると、これがそこに配置される発行者になります(ターゲット・サービスによって検証されます)</span></td>
</tr>
<tr>
...
<td class="">&#160;</td>
<td class=""><span class="merged" id="all.2X9JJW.1" title="原文 : Audience this key is generated for (e.g. http://www.example.org/api/myService) - validated by the other service">このキーが生成されるオーディエンス(<a href="http://www.example.org/api/myService" id="" target="_blank" >http://www.example.org/api/myService</a>など) - 他のサービスによって検証されました</span></td>
</tr>
<tr>
