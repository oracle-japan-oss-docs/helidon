...
<div class="section">
<p><span class="merged" id="all.2KudlH.spl1" title="原文 : Use the StaticContentSupport class to serve files and classpath resources."><code>StaticContentSupport</code>クラスを使用して、ファイルおよびクラスパス・リソースを提供します。</span> <span class="merged" id="all.2KudlH.spl2" title="原文 : StaticContentSupport can be created for any readable directory or classpath context root and registered on a path in Routing."><code>StaticContentSupport</code>は、読取り可能な任意のディレクトリまたはクラスパス・コンテキスト・ルートに対して作成し、<code>Routing</code>のパスに登録できます。</span> </p>
