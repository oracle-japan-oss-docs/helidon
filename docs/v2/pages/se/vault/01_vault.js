...
<dl>
<dt slot=title><span class="merged" id="all.19StqX.1"  title="原文:: Vault">Vault</span></dt>
<dd slot="desc"><p><span class="merged" id="all.4eTCPn.1.spl1" title="原文 : HashiCorp Vault is a commonly used Vault in many microservices.">HashiCorp Vaultは、多くのマイクロサービスでよく使用されているVaultです。</span> <span class="merged" id="all.4eTCPn.1.spl2" title="原文 : The APIs are REST-based and Helidon implements them using reactive client.">APIはRESTベースであり、Helidonはリアクティブ・クライアントを使用して実装します。</span> </p>
...
<h3 id="_experimental"><span class="merged" id="all.3qlK5J.10"  title="原文:: Experimental">試作</span></h3>
<div class="section">
<div class="admonition warning">
<p class="admonition-inline"><span class="merged" id="all.1nUBj2.1.spl1" title="原文 : Helidon Vault support is still experimental and not intended for production use.">Helidon Vaultのサポートは引き続き試験的であり、本番での使用を目的としていません。</span> <span class="merged" id="all.1nUBj2.1.spl2" title="原文 : APIs and features have not yet been fully tested and are subject to change.">APIおよび機能はまだ完全にテストされておらず、変更される可能性があります。</span> </p>
</div>
</div>

<h3 id="_sys_operations"><span class="merged" id="all.4DPBwG.1" title="原文 : Sys Operations">システム操作</span></h3>
<div class="section">
<p><span class="merged" id="all.2qYYw9.1.spl1" title="原文 : Each of these features is implemented as a separate module, with the Vault class binding them together.">これらの各機能は個別のモジュールとして実装され、Vaultクラスによって結合されます。</span> <span class="merged" id="all.2qYYw9.1.spl2" title="原文 : In Helidon MP, with injection, this binding is done automatically, and you can simply inject your favorite secret engine.">Helidon MPでは、インジェクションを使用すると、このバインディングが自動的に行われ、お気に入りのシークレット・エンジンを単にインジェクトできます。</span> </p>
...
<h3 id="_modules"><span class="merged" id="all.47VrhL.1"  title="原文:: Modules">モジュール</span></h3>
<div class="section">
<p><span class="merged" id="all.4WLHW2.1" title="原文 : The following is a list of maven coordinates of all Vault modules available:">次に、使用可能なすべてのVaultモジュールのmaven座標のリストを示します:</span></p>
...
<p><span class="merged" id="all.4G30eL.1" title="原文 : Configuration to connect to Vault.">Vaultに接続するための構成。</span></p>

<ol style="margin-left: 15px;">
<li>
<span class="merged" id="all.3Nwkr.1" title="原文 : Authenticating using Token:">トークンを使用した認証:</span>
...
<ol style="margin-left: 15px;">
<li>
<span class="merged" id="all.LF0dL.1" title="原文 : Authenticating using AppRole:">AppRoleを使用した認証:</span>
...
<ol style="margin-left: 15px;">
<li>
<span class="merged" id="all.28Gyws.1" title="原文 : Authenticating using Kubernetes:">Kubernetesを使用した認証:</span>
...
<p><span class="merged" id="all.1rvXh6" title="原文 : Code to set up Vault and obtain a specific secret engine:">Vaultを設定し、特定のシークレット・エンジンを取得するコード:</span></p>
...
<p><span class="merged" id="all.4S28TI" title="原文 : Code to obtain a specific authentication method:">特定の認証方式を取得するコード:</span></p>
...
<p><span class="merged" id="all.4Egt6t" title="原文 : Similar code can be used for any authentication method available:">同様のコードを、使用可能な任意の認証方式に使用できます:</span></p>

<ul class="ulist">
<li>
<p><span class="merged" id="all.4IpHOH" title="原文 : AppRoleAuthRx - AppRole authentication method (management operations)">AppRoleAuthRx - AppRole認証方式(管理操作)</span></p>

</li>
<li>
<p><span class="merged" id="all.2VkCCU" title="原文 : K8sAuthRx - Kubernetes authentication method (management operations)">K8sAuthRx - Kubernetes認証方式(管理操作)</span></p>

</li>
<li>
<p><span class="merged" id="all.4N02cZ" title="原文 : TokenAuthRx - Token authentication method (management operations)">TokenAuthRx - トークン認証方式(管理操作)</span></p>
...
<h2 id="_usage_with_webserver"><span class="merged" id="all.1AmitT" title="原文 : Usage with WebServer">WebServerでの使用方法</span></h2>
<div class="section">
...
<h2 id="_local_testing"><span class="merged" id="all.3HPxA2.1" title="原文 : Local testing">ローカル・テスト</span></h2>
<div class="section">
...
<p><span class="merged" id="all.1XeBdv.1.spl1" title="原文 : This will create a Vault docker image, run it in background and open it on localhost:8200 with a custom root token my-token, using name vault.">これにより、Vault dockerイメージが作成され、バックグラウンドで実行され、名前vaultを使用して、カスタム・ルート・トークンmy-tokenでlocalhost:8200で開きます。</span> <span class="merged" id="all.1XeBdv.1.spl2" title="原文 : This is of course only suitable for local testing, as the root token has too many rights, but it can be easily used with the examples below.">ルート・トークンの権限が多すぎるため、これはローカル・テストにのみ適していますが、次の例で簡単に使用できます。</span> </p>
