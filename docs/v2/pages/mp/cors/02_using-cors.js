...
<div class="section">
<p><span class="merged" id="all.3j8QoM" title="原文 : Adding CORS behavior to your Helidon MP application involves two simple changes to your application code:">CORS動作をHelidon MPアプリケーションに追加するには、アプリケーション・コードを次のように簡単に変更する必要があります:</span></p>

<ol style="margin-left: 15px;">
<li>
<span class="merged" id="all.3jQzDg.spl1" title="原文 : For each resource and subresource in each resource class, make sure you have a Java method annotated with @OPTIONS and with the correct @Path.">各リソース・クラスのリソースおよびサブリソースごとに、<code>@OPTIONS</code>および適切な<code>@Path</code>の注釈が付いたJavaメソッドがあることを確認します。</span> <span class="merged" id="all.3jQzDg.spl2" title="原文 : Create these methods for each resource if you do not already have them.">これらのメソッドをまだ持っていない場合は、リソースごとに作成します。</span> 

</li>
<li>
<span class="merged" id="all.QZlj2" title="原文 : To each of those methods, add a @CrossOrigin annotation that describes the cross-origin sharing you want for that resource.">これらの各メソッドに、そのリソースに必要なクロス・オリジン共有を記述する<code>@CrossOrigin</code>注釈を追加します。</span>
...
<div class="section">
<p><span class="merged" id="all.4KwIIe.spl1" title="原文 : In the Helidon MP Quickstart application you can change the greeting by sending a PUT request to the /greet/greeting resource."><a href="https://github.com/oracle/helidon/tree/2.2.1-SNAPSHOT/examples/quickstarts/helidon-quickstart-mp" id="" target="_blank" >「Helidon MP Quickstartアプリケーション」</a>では、<code>PUT</code>リクエストを<code>/greet/greeting</code>リソースに送信することで、応答メッセージを変更できます。</span> <span class="merged" id="all.4KwIIe.spl2" title="原文 : The example below extends the Helidon MP QuickStart application (the greeting app) to:">次の例では、Helidon MP QuickStartアプリケーション(greetingアプリケーション)を次のように拡張します:</span> </p>

<ul class="ulist">
<li>
<p><span class="merged" id="all.19rL3Z" title="原文 : Permit unrestricted sharing of the resource that returns greetings, and">グリーティングを返すリソースの無制限共有を許可</span></p>
...
<li>
<p><span class="merged" id="all.2b3Fwa" title="原文 : See the Helidon CORS support in action by building and running the CORS example."><a href="https://github.com/oracle/helidon/tree/2.2.1-SNAPSHOT/examples/microprofile/cors" id="" target="_blank" >「CORSの例」</a>をビルドして実行し、動作中のHelidon CORSサポートを参照してください。</span></p>
