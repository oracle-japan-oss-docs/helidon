...
<tbody>
<tr>
<td class=""><span class="merged" id="all.10cPjD.4" title="原文 : About 10 minutes">約10分</span></td>
</tr>
<tr>
<td class=""><span class="merged" id="all.27GIMH.12" title="原文 : Helidon Prerequisites"><router-link to="/about/03_prerequisites">Helidonの前提条件</router-link></span></td>
</tr>
</tbody>
</table>
</div>
...
<div class="section">
<p><span class="merged" id="all.24u4wa" title="原文 : JDK 11 or newer is required.">JDK 11以降が必要です。</span></p>
...
<h2 id="_generate_the_project"><span class="merged" id="all.xQmwm.2" title="原文 : Generate The Project">プロジェクトの生成</span></h2>
<div class="section">
...
<p ><span class="merged" id="all.3BIrNM.1.spl1" title="原文 : This does a full build inside the Docker container.">これにより、Dockerコンテナ内で完全なビルドが実行されます。</span> <span class="merged" id="all.3BIrNM.1.spl2" title="原文 : The first time you run it, it will take a while because it is downloading all of the Maven dependencies and caching them in a Docker layer.">初回実行時には、すべてのMaven依存性をダウンロードしてDockerレイヤーにキャッシュするため、しばらく時間がかかります。</span> <span class="merged" id="all.3BIrNM.1.spl3" title="原文 : Subsequent builds will be much faster as long as you don&rsquo;t change the pom.xml file."><code>pom.xml</code>ファイルを変更しないかぎり、後続のビルドははるかに高速になります。</span> <span class="merged" id="all.3BIrNM.1.spl4" title="原文 : If the pom is modified then the dependencies will be re-downloaded.">POMが変更されると、依存性が再ダウンロードされます。</span> </p>
</div>
...
<h2 id="_when_should_you_use_a_custom_runtime_image"><span class="merged" id="all.2D2u1c" title="原文 : When should you use a Custom Runtime Image?">カスタム・ランタイム・イメージはどのような場合に使用しますか。</span></h2>
<div class="section">
