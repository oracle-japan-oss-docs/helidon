...
<dl>
<dt slot=title><span class="merged" id="all.8l1BP" title="原文 : Helidon MP Metrics Guide">Helidon MPメトリック・ガイド</span></dt>
<dd slot="desc"><p><span class="merged" id="all.11MWoi" title="原文 : This guide describes how to create a sample MicroProfile (MP) project that can be used to run some basic examples using both built-in and custom metrics with Helidon MP.">このガイドでは、Helidon MPで組込みメトリックとカスタム・メトリックの両方を使用していくつかの基本的な例を実行するために使用できるサンプルMicroProfile (MP)プロジェクトの作成方法について説明します。</span></p>
</dd>
</dl>
</v-card-text>
</v-card>
...
<h3 id="_using_the_built_in_metrics"><span class="merged" id="all.43ESSC" title="原文 : Using the built-in metrics">組込みメトリックの使用</span></h3>
<div class="section">
<p><span class="merged" id="all.lNNDM.spl1" title="原文 : Helidon provides three scopes of metrics: base, vendor, and application.">Helidonには、3つのメトリック・スコープが用意されています: ベース、ベンダーおよびアプリケーション。</span> <span class="merged" id="all.lNNDM.spl2" title="原文 : Helidon automatically provides built-in base and vendor metrics.">Helidonには、基本メトリックとベンダー・メトリックが自動的に組み込まれています。</span> <span class="merged" id="all.lNNDM.spl3" title="原文 : Applications can use these metrics without additional configuration or code changes.">アプリケーションでは、追加の構成またはコードを変更せずに、これらのメトリックを使用できます。</span> <span class="merged" id="all.lNNDM.spl4" title="原文 : Here are the metric endpoints:">メトリック・エンドポイントは次のとおりです:</span> </p>

<ol style="margin-left: 15px;">
<li>
<span class="merged" id="all.4HMomk" title="原文 : /metrics/base - Base metrics data as specified by the MicroProfile Metrics specification."><code>/metrics/base</code> - MicroProfileメトリック仕様で指定されているベース・メトリック・データ。</span>

</li>
<li>
<span class="merged" id="all.3CsSgu" title="原文 : /metrics/vendor - Helidon-specific metrics data."><code>/metrics/vendor</code> - Helidon固有のメトリック・データ。</span>

</li>
<li>
<span class="merged" id="all.3A3wVX" title="原文 : /metrics/application - Application-specific metrics data."><code>/metrics/application</code> - アプリケーション固有のメトリック・データ。</span>
...
<p><span class="merged" id="all.4c7fA5.spl1" title="原文 : The following example will demonstrate how to use the built-in metrics.">次の例では、組込みメトリックの使用方法を示します。</span> <span class="merged" id="all.4c7fA5.spl2" title="原文 : All examples are executed from the root directory of your project (helidon-quickstart-mp).">すべての例は、プロジェクトのルート・ディレクトリ(helidon-quickstart-mp)から実行されます。</span> </p>
...
<h4 id="_controlling_rest_request_metrics"><span class="merged" id="all.r28S3" title="原文 : Controlling REST.request metrics"><code>REST.request</code>メトリックの制御</span></h4>
<div class="section">
<p><span class="merged" id="all.1BUPZE.spl1" title="原文 : Helidon implements the optional family of metrics, all with the name REST.request, as described in the MicroProfile Metrics specification.">Helidonは、<a href="https://download.eclipse.org/microprofile/microprofile-metrics-2.3/microprofile-metrics-spec-2.3.html#_optional_rest" id="" target="_blank" >「MicroProfileメトリック仕様」</a>で説明されているように、オプションのメトリック・ファミリをすべて<code>REST.request</code>という名前で実装します。</span> <span class="merged" id="all.1BUPZE.spl2" title="原文 : Each instance is a SimpleTimer with tags class and method identifying exactly which REST endpoint Java method that instance measures.">各インスタンスは、インスタンスが測定するRESTエンドポイントJavaメソッドを正確に識別するタグ<code>class</code>および<code>method</code>を含む<code>SimpleTimer</code>です。</span> </p>
...
<h3 id="_metrics_metadata"><span class="merged" id="all.1eMrtK" title="原文 : Metrics metadata">メトリック・メタデータ</span></h3>
<div class="section">
...
<h3 id="_application_specific_metrics_data"><span class="merged" id="all.2BGaic" title="原文 : Application-specific metrics data">アプリケーション固有のメトリック・データ</span></h3>
<div class="section">
<p><span class="merged" id="all.3xfFYk.spl1" title="原文 : You can create application-specific metrics and integrate them with Helidon using CDI.">CDIを使用して、アプリケーション固有のメトリックを作成し、Helidonと統合できます。</span> <span class="merged" id="all.3xfFYk.spl2" title="原文 : To add a new metric, simply annotate the JAX-RS resource with one of the metric annotations.">新しいメトリックを追加するには、JAX-RSリソースにメトリック注釈のいずれかを付けます。</span> <span class="merged" id="all.3xfFYk.spl3" title="原文 : Metrics can be injected at the class, method, and field-levels.">メトリックは、クラス、メソッドおよびフィールド・レベルでインジェクトできます。</span> <span class="merged" id="all.3xfFYk.spl4" title="原文 : This document shows examples of all three.">このドキュメントでは、3つすべての例を示します。</span> </p>

<p><span class="merged" id="all.1yHQZS.spl1" title="原文 : Helidon will automatically create and register annotated application metrics and store them in the application MetricRegistry, which also contains the metric metadata.">Helidonでは、注釈付きアプリケーション・メトリックが自動的に作成および登録され、アプリケーション<code>MetricRegistry</code>に格納され、そこでメトリック・メタデータも含まれます。</span> <span class="merged" id="all.1yHQZS.spl2" title="原文 : The metrics will exist for the lifetime of the application.">メトリックは、アプリケーションの存続期間中存在します。</span> <span class="merged" id="all.1yHQZS.spl3" title="原文 : Each metric annotation has mandatory and optional fields.">各メトリック注釈には、必須フィールドとオプション・フィールドがあります。</span> <span class="merged" id="all.1yHQZS.spl4" title="原文 : The name field, for example, is optional.">たとえば、名前フィールドはオプションです。</span> </p>


<h4 id="_method_level_metrics"><span class="merged" id="all.3cgkpB" title="原文 : Method level metrics">メソッド・レベルのメトリック</span></h4>
<div class="section">
<p><span class="merged" id="all.ocHd6" title="原文 : There are four metrics that you can use by annotating a method:">メソッドに注釈を付けると、次の4つのメトリックを使用できます:</span></p>
...
<h4 id="_additional_method_level_metrics"><span class="merged" id="all.2Lc6Pb" title="原文 : Additional method-level metrics">追加のメソッド・レベル・メトリック</span></h4>
<div class="section">
<p><span class="merged" id="all.1yJT5Q.spl1" title="原文 : The @Timed, @Metered, and @SimplyTimed annotations can also be used with a method.">メソッドでは、<code>@Timed</code>、<code>@Metered</code>および<code>@SimplyTimed</code>注釈も使用できます。</span> <span class="merged" id="all.1yJT5Q.spl2" title="原文 : For the following example. you can just annotate the same method with @Metered and @Timed.">次の例では、<code>@Metered</code>と<code>@Timed</code>で同じメソッドに注釈を付けることができます。</span> <span class="merged" id="all.1yJT5Q.spl3" title="原文 : These metrics collect significant information about the measured methods, but at a cost of some overhead and more complicated output.">これらのメトリックは、測定されたメソッドに関する重要な情報を収集しますが、オーバーヘッドやより複雑な出力が発生します。</span> <span class="merged" id="all.1yJT5Q.spl4" title="原文 : Use @SimplyTimed in cases where capturing the invocation count and the total elapsed time spent in a block of code is sufficient.">呼出し数およびコード・ブロックで費やされた合計経過時間を取得するだけで十分な場合は、<code>@SimplyTimed</code>を使用します。</span> </p>

<p><span class="merged" id="all.3kX4nx" title="原文 : Note that when using multiple annotations on a method, you must give the metrics different names as shown below.">メソッドで複数の注釈を使用する場合は、次に示すようにメトリックに異なる名前を付ける必要があります。</span></p>
...
<h4 id="_reusing_metrics"><span class="merged" id="all.2m7b4x" title="原文 : Reusing metrics">メトリックの再利用</span></h4>
<div class="section">
...
<h4 id="_class_level_metrics"><span class="merged" id="all.1d7jHO" title="原文 : Class-level metrics">クラス・レベル・メトリック</span></h4>
<div class="section">
<p><span class="merged" id="all.3PPnOX.spl1" title="原文 : You can collect metrics at the class-level to aggregate data from all methods in that class using the same metric.">クラス・レベルでメトリックを収集し、同じメトリックを使用してそのクラスのすべてのメソッドからデータを集計できます。</span> <span class="merged" id="all.3PPnOX.spl2" title="原文 : The following example introduces a metric to count all card queries.">次の例では、すべてのカード問合せをカウントするメトリックを導入します。</span> <span class="merged" id="all.3PPnOX.spl3" title="原文 : In the following example, the method-level metrics are not needed to aggregate the counts, but they are left in the example to demonstrate the combined output of all three metrics.">次の例では、カウントの集計にメソッド・レベル・メトリックは必要ありませんが、3つのメトリックすべての結合された出力を示すために例に残されています。</span> </p>
...
<ul class="colist">
<li data-value="1"><span class="merged" id="all.1PZkzO.spl1" title="原文 : The totalCards count is a total of all the method-level Counter metrics."><code>totalCards</code>数は、すべてのメソッド・レベルの<code>Counter</code>メトリックの合計です。</span> <span class="merged" id="all.1PZkzO.spl2" title="原文 : Class level metric names are always fully qualified.">クラス・レベル・メトリック名は常に完全修飾されます。</span> </li>
</ul>
</div>

<h4 id="_field_level_metrics"><span class="merged" id="all.1JUeht" title="原文 : Field-level metrics">フィールド・レベルのメトリック</span></h4>
<div class="section">
<p><span class="merged" id="all.3IwyWN.spl1" title="原文 : Field level metrics can be injected into managed objects, but they need to be updated by the application code.">フィールド・レベルのメトリックは管理対象オブジェクトにインジェクトできますが、アプリケーション・コードで更新する必要があります。</span> <span class="merged" id="all.3IwyWN.spl2" title="原文 : This annotation can be used on fields of type Meter, Timer, Counter, and Histogram.">この注釈は、<code>Meter</code>, <code>Timer</code>, <code>Counter</code>および<code>Histogram</code>型のフィールドで使用できます。</span> </p>
...
<div class="section">
<p><span class="merged" id="all.2TLOUS.spl1" title="原文 : The metrics you have tested so far are updated in response to an application REST request, i.e GET /cards.">これまでにテストしたメトリックは、アプリケーションのRESTリクエスト(GET <code>/cards</code>)に応じて更新されます。</span> <span class="merged" id="all.2TLOUS.spl2" title="原文 : These metrics can be declared in a request scoped class and Helidon will store the metric in the MetricRegistry, so the value persists across requests.">これらのメトリックはリクエスト・スコープのクラスで宣言でき、Helidonはメトリックを<code>MetricRegistry</code>に格納するため、値はリクエスト間で保持されます。</span> <span class="merged" id="all.2TLOUS.spl3" title="原文 : When GET /metrics/application is invoked, Helidon will return the current value of the metric stored in the MetricRegistry.">GET <code>/metrics/application</code>が起動されると、Helidonは<code>MetricRegistry</code>に格納されているメトリックの現在の値を返します。</span> <span class="merged" id="all.2TLOUS.spl4" title="原文 : The Gauge metric is different from all the other metrics."><code>Gauge</code>メトリックは、他のすべてのメトリックとは異なります。</span> <span class="merged" id="all.2TLOUS.spl5" title="原文 : The application must provide a getter to return the gauge value in an application scoped class.">アプリケーションは、アプリケーション・スコープのクラスのゲージ値を返すgetterを提供する必要があります。</span> <span class="merged" id="all.2TLOUS.spl6" title="原文 : When GET /metrics/application is invoked, Helidon will call the Gauge getter, store that value in the MetricsRegistry, and return it as part of the metrics response payload.">GET <code>/metrics/application</code>が起動されると、Helidonは<code>Gauge</code> getterをコールし、その値を<code>MetricsRegistry</code>に格納して、メトリック・レスポンス・ペイロードの一部として返します。</span> <span class="merged" id="all.2TLOUS.spl7" title="原文 : So, the Gauge metric value is updated real-time, in response to the get metrics request.">そのため、<code>Gauge</code>メトリック値は、メトリック・リクエストの取得に応じてリアルタイムで更新されます。</span> </p>
...
<li data-value="4"><span class="merged" id="all.38FnMS" title="原文 : Return the application appUpTimeSeconds metric, which will be included in the application metrics.">アプリケーション・メトリックに含まれるアプリケーション<code>appUpTimeSeconds</code>メトリックを返します。</span></li>
</ul>
<markup
lang="java"
title="次のコードを使用して<code>GreetingCards</code>クラスを更新し、メトリック出力を簡略化します:"
>package io.helidon.examples.quickstart.mp;
...
<markup
lang="bash"
title="アプリケーションをビルドして実行し、アプリケーション・メトリック・エンドポイントを起動します:"
>curl -H "Accept: application/json"  http://localhost:8080/metrics/application</markup>
...
<markup
lang="bash"
title="ポート<code>30116</code>を使用してメトリック・エンドポイントを検証します。実際のポートは異なる可能性があります:"
...
<div class="section">
<p><span class="merged" id="all.1pN5mg" title="原文 : This guide demonstrated how to use metrics in a Helidon MP application using various combinations of metrics and scopes.">このガイドでは、メトリックとスコープの様々な組合せを使用して、Helidon MPアプリケーションでメトリックを使用する方法について説明しました。</span></p>

<ul class="ulist">
<li>
<p><span class="merged" id="all.3Mkxqz" title="原文 : Access metrics for all three scopes: base, vendor, and application">3つのスコープすべてのアクセス・メトリック: ベース、ベンダーおよびアプリケーション</span></p>

</li>
<li>
<p><span class="merged" id="all.2uQ0mO" title="原文 : Configure application metrics at the class, method, and field-level">クラス・レベル、メソッド・レベルおよびフィールド・レベルでのアプリケーション・メトリックの構成</span></p>

</li>
<li>
<p><span class="merged" id="all.1cNgkv" title="原文 : Integrate Helidon metrics with Kubernetes and Prometheus">HelidonメトリックとKubernetesおよびPrometheusの統合</span></p>
