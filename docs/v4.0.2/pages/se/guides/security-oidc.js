...
<dd slot="desc"><p><span class="merged" id="all.7lHFr" title="原文 : This guide describes how to setup Keycloak and Helidon to secure your application with OIDC security provider.">このガイドでは、OIDCセキュリティ・プロバイダでアプリケーションを保護するためにKeycloakおよびHelidonを設定する方法について説明します。</span></p>
</dd>
</dl>
</v-card-text>
</v-card>
...
<div class="section">
<p><span class="merged" id="all.3yeiO0.5" title="原文 : For this 20 minute tutorial, you will need the following:">この20分間のチュートリアルでは、次のものが必要です:</span></p>
...
<tbody>
<tr>
<td class=""><span class="merged" id="all.3EWh8P.7" title="原文 : A Helidon SE Application">Helidon SEアプリケーション</span></td>
<td class=""><span class="merged" id="all.3f8L7i.7" title="原文 : You can use your own application or use the Helidon SE Quickstart to create a sample application.">独自のアプリケーションを使用するか、<router-link to="/se/guides/quickstart">「Helidon SEクイックスタート」</router-link>を使用してサンプル・アプリケーションを作成できます。</span></td>
</tr>
<tr>
<td class=""><span class="merged" id="all.17vEm3.22" title="原文 : Java&#160;SE&#160;21 (Open&#160;JDK&#160;21)"><a href="https://www.oracle.com/technetwork/java/javase/downloads" target="_blank">Java SE 21</a> (<a href="http://jdk.java.net" target="_blank">JDK 21を開く</a>)</span></td>
<td class=""><span class="merged" id="all.3uVV7D.22" title="原文 : Helidon requires Java 21+.">HelidonにはJava 21+が必要です。</span></td>
</tr>
<tr>
<td class=""><span class="merged" id="all.UWTyd.22" title="原文 : Maven 3.8+"><a href="https://maven.apache.org/download.cgi" target="_blank">Maven 3.8+</a></span></td>
<td class=""><span class="merged" id="all.47RJEf.22" title="原文 : Helidon requires Maven 3.8+.">HelidonにはMaven 3.8+が必要です。</span></td>
</tr>
<tr>
<td class=""><span class="merged" id="all.3ND1mN.22"  title="原文:: Docker 18.09+"><a href="https://docs.docker.com/install/" target="_blank">Docker 18.09+</a></span></td>
<td class=""><span class="merged" id="all.aX1BT.22" title="原文 : You need Docker if you want to build and deploy Docker containers.">Dockerコンテナをビルドおよびデプロイする場合は、Dockerが必要です。</span></td>
</tr>
<tr>
<td class=""><span class="merged" id="all.4Lo77h.22"  title="原文:: Kubectl 1.16.5+"><a href="https://kubernetes.io/docs/tasks/tools/install-kubectl/" target="_blank">Kubectl 1.16.5+</a></span></td>
<td class=""><span class="merged" id="all.1JBSOB.22" title="原文 : If you want to deploy to Kubernetes, you need kubectl and a Kubernetes cluster (you can install one on your desktop.">Kubernetesにデプロイする場合は、<code>kubectl</code>およびKubernetesクラスタが必要です (<router-link to="/about/kubernetes">「デスクトップにインストール」</router-link>できます)。</span></td>
</tr>
</tbody>
</table>
</div>
...
<li>
<p><span class="merged" id="all.3fNMnT" title="原文 : Keycloak Installation"><router-link @click.native="this.scrollFix('#Keycloak-Installation')" to="#Keycloak-Installation">Keycloakのインストール</router-link></span></p>

</li>
<li>
<p><span class="merged" id="all.4PsMK0" title="原文 : Setup Keycloak"><router-link @click.native="this.scrollFix('#Setup-Keycloak')" to="#Setup-Keycloak">Keycloakの設定</router-link></span></p>

</li>
<li>
<p><span class="merged" id="all.5uZEs" title="原文 : Setup Helidon"><router-link @click.native="this.scrollFix('#Setup-Helidon')" to="#Setup-Helidon">Helidonの設定</router-link></span></p>

</li>
<li>
<p><span class="merged" id="all.2SaM0C" title="原文 : Test Keycloak process with Postman"><router-link @click.native="this.scrollFix('#Test-Keycloak-process-with-Postman')" to="#Test-Keycloak-process-with-Postman">PostmanによるKeycloakプロセスのテスト</router-link></span></p>

</li>
<li>
<p><span class="merged" id="all.pF2yp" title="原文 : Restrict access to a specific role"><router-link @click.native="this.scrollFix('#Restrict-access-to-a-specific-role')" to="#Restrict-access-to-a-specific-role">特定のロールへのアクセスの制限</router-link></span></p>
...
<h2 id="_introduction"><span class="merged" id="all.4LJMHk.17"  title="原文:: Introduction">導入</span></h2>
<div class="section">
...
<h2 id="Keycloak-Installation"><span class="merged" id="all.47XsKo" title="原文 : Keycloak Installation">Keycloakのインストール</span></h2>
<div class="section">

<h3 id="_on_docker"><span class="merged" id="all.36l17N.1" title="原文 : On Docker">Dockerの場合</span></h3>
<div class="section">
...
<p><span class="merged" id="all.33kbPQ.1" title="原文 : To verify that Keycloak is running correctly, go to the admin console : http://localhost:8080/auth/admin Log in using the username and password mentioned above: admin.">Keycloakが正しく実行されていることを確認するには、管理コンソールに移動 : <a class="bare" href="http://localhost:8080/auth/admin" target="_blank">http://localhost:8080/auth/admin</a>前述のユーザー名とパスワードを使用してログイン : <code>admin</code>。</span></p>
...
<h3 id="_on_jdk"><span class="merged" id="all.3YAmaV.1" title="原文 : On JDK">JDKの場合</span></h3>
<div class="section">
<p><span class="merged" id="all.DbOGr.1.spl1" title="原文 : Download the last version of Keycloak from Keycloak website : https://www.keycloak.org/downloads In the table Server choose Standalone server distribution.">Keycloak webサイトから最後のバージョンのKeycloakをダウンロード : <a class="bare" href="https://www.keycloak.org/downloads" target="_blank">https://www.keycloak.org/downloads</a>表のサーバーで、スタンドアロン・サーバー・ディストリビューションを選択します。</span> <span class="merged" id="all.DbOGr.1.spl2" title="原文 : ZIP or Tar format are available, click on either to download Keycloak.">ZIPまたはTar形式が使用可能です。いずれかをクリックしてKeycloakをダウンロードしてください。</span> </p>
...
<h2 id="_start_keycloak"><span class="merged" id="all.1Wa2Zu.1" title="原文 : Start Keycloak">Keycloakの起動</span></h2>
<div class="section">
...
<h3 id="_create_an_admin_user"><span class="merged" id="all.2lwsBL.1" title="原文 : Create an Admin User">管理ユーザーの作成</span></h3>
<div class="section">
<p><span class="merged" id="all.3mLZYh.1.spl1" title="原文 : You need to create an admin user because it does not come by default when installing Keycloak.">管理ユーザーは、Keycloakのインストール時にデフォルトでは提供されないため、作成する必要があります。</span> <span class="merged" id="all.3mLZYh.1.spl2" title="原文 : To do this, open http://localhost:8080/auth in your favorite browser.">これを行うには、お気に入りのブラウザで<a class="bare" href="http://localhost:8080/auth" target="_blank">http://localhost:8080/auth</a>を開きます。</span> </p>
...
<h2 id="Setup-Keycloak"><span class="merged" id="all.2c2blg" title="原文 : Setup Keycloak">Keycloakの設定</span></h2>
<div class="section">
<p><span class="merged" id="all.ShyHc" title="原文 : To setup Keycloak properly, go to the admin console: http://localhost:8080/auth/admin">Keycloakを正しく設定するには、管理コンソールに移動 : <a class="bare" href="http://localhost:8080/auth/admin" target="_blank">http://localhost:8080/auth/admin</a></span></p>
...
<h3 id="_create_a_realm"><span class="merged" id="all.3oYief.1"  title="原文:: Create a Realm">レルムの作成</span></h3>
<div class="section">
...
<li>
<span class="merged" id="all.2Lp4ms.1" title="原文 : Open Keycloak admin console http://localhost:8080/auth/admin.">Keycloak管理コンソール<a class="bare" href="http://localhost:8080/auth/admin" target="_blank">http://localhost:8080/auth/admin</a>を開きます。</span>
...
<h3 id="_create_a_user"><span class="merged" id="all.obRkg.1"  title="原文:: Create a User">ユーザーの作成</span></h3>
<div class="section">
...
<ol style="margin-left: 15px;">
<li>
<span class="merged" id="all.4RPgHl.1" title="原文 : Open the Keycloak admin console: http://localhost:8080/auth/admin">Keycloak管理コンソールを開く : <a class="bare" href="http://localhost:8080/auth/admin" target="_blank">http://localhost:8080/auth/admin</a></span>
...
<li>
<span class="merged" id="all.3axGYC.1" title="原文 : Click Save"><code>Save</code>をクリック</span>
...
<ol style="margin-left: 15px;">
<li>
<span class="merged" id="all.1jflBZ" title="原文 : Open the Keycloak account console: http://localhost:8080/auth/realms/myRealm/account.">Keycloakアカウント・コンソールを開く : <code><a class="bare" href="http://localhost:8080/auth/realms/myRealm/account" target="_blank">http://localhost:8080/auth/realms/myRealm/account</a></code>。</span>
...
<h3 id="_create_a_client"><span class="merged" id="all.4esYak.1" title="原文 : Create a Client">クライアントの作成</span></h3>
<div class="section">
<p><span class="merged" id="all.3bbsNV.1" title="原文 : To create your first client:">最初のクライアントを作成するには:</span></p>

<ol style="margin-left: 15px;">
<li>
<span class="merged" id="all.Vm4eb.1" title="原文 : Open the Keycloak admin console: http://localhost:8080/auth/admin.">Keycloak管理コンソールを開く : <a class="bare" href="http://localhost:8080/auth/admin" target="_blank">http://localhost:8080/auth/admin</a>。</span>
...
<li>
<span class="merged" id="all.4Y7MZT.1" title="原文 : Update Valid Redirect URIs : http://localhost:7987/*"><code>Valid Redirect URIs</code>の更新 : <a class="bare" href="http://localhost:7987/*" target="_blank">http://localhost:7987/*</a></span>
...
<p><span class="merged" id="all.2xcx8v.spl1" title="原文 : Keycloak is now configured and ready.">Keycloakが構成され、準備ができました。</span> <span class="merged" id="all.2xcx8v.spl2" title="原文 : Keep keycloak running on your terminal and open a new tab to setup Helidon.">端末でkeycloakを実行したままにし、新しいタブを開いてHelidonを設定します。</span> </p>
...
<h2 id="Setup-Helidon"><span class="merged" id="all.20wYMc" title="原文 : Setup Helidon">Helidonの設定</span></h2>
<div class="section">
<p><span class="merged" id="all.4KU55G.spl1" title="原文 : Use the Helidon SE Maven archetype to create a simple project.">Helidon SE Maven原型を使用して、単純なプロジェクトを作成します。</span> <span class="merged" id="all.4KU55G.spl2" title="原文 : It will be used as an example to show how to setup Helidon.">Helidonの設定方法を示す例として使用されます。</span> <span class="merged" id="all.4KU55G.spl3" title="原文 : Replace 4.0.2 by the latest helidon version."><code>4.0.2</code>を最新のhelidonバージョンに置き換えます。</span> <span class="merged" id="all.4KU55G.spl4" title="原文 : It will download the quickstart project into the current directory.">クイックスタート・プロジェクトが現在のディレクトリにダウンロードされます。</span> </p>
...
<h3 id="_update_project_dependencies"><span class="merged" id="all.1vG31d.1" title="原文 : Update Project Dependencies">プロジェクトの依存関係の更新</span></h3>
<div class="section">
...
<h3 id="_add_oidc_security_properties"><span class="merged" id="all.2P0F4E.1" title="原文 : Add OIDC Security Properties">OIDCセキュリティ・プロパティの追加</span></h3>
<div class="section">
...
<li data-value="5"><span class="merged" id="all.4MWcgw" title="原文 : paths section defines the protected application’s path."><code>paths</code>セクションでは、保護されたアプリケーションのパスを定義します。</span></li>
</ul>
...
<h3 id="_configure_web_server"><span class="merged" id="all.3thdyx"  title="原文:: Configure Web Server">Webサーバーの構成</span></h3>
<div class="section">
<p><span class="merged" id="all.2LvOSr.spl1" title="原文 : Once the properties are added, the web server must be setup.">プロパティを追加したら、webサーバーを設定する必要があります。</span> <span class="merged" id="all.2LvOSr.spl2" title="原文 : The Main.routing method gather all configuration properties."><code>Main.routing</code>メソッドは、すべての構成プロパティを収集します。</span> </p>

<markup
lang="java"
title="Add the following to <code>Main.routing</code> method"
...
routing.addFeature(ContextFeature.create())
        .addFeature(SecurityFeature.create(security, config.get("security"))) <span class="conum" data-value="2" />
        .addFeature(OidcFeature.create(config))   <span class="conum" data-value="3" /></markup>
...
<li data-value="2"><span class="merged" id="all.3fKrBo" title="原文 : Register Helidon SecurityFeature instance using security instance and configuration.">セキュリティ・インスタンスおよび構成を使用して、Helidon <code>SecurityFeature</code>インスタンスを登録します。</span></li>
<li data-value="3"><span class="merged" id="all.10JC6y" title="原文 : Register Helidon OidcFeature instance.">Helidon <code>OidcFeature</code>インスタンスを登録します。</span></li>
</ul>

<p><span class="merged" id="all.4JLnLl" title="原文 : It is also needed to add the following lines to the module-info.java file:">また、次の行を<code>module-info.java</code>ファイルに追加する必要があります:</span></p>

<markup
lang="java"
title="Add security related module requirements"
>requires io.helidon.security;
requires io.helidon.security.providers.oidc;
requires io.helidon.webserver.security;
requires io.helidon.webserver.context;</markup>

<p><span class="merged" id="all.UgP3q.spl1" title="原文 : That code is extracting security properties from application.yaml into two steps.">このコードは、セキュリティ・プロパティをapplication.yamlから2つのステップに抽出しています。</span> <span class="merged" id="all.UgP3q.spl2" title="原文 : First the Security instance is used to bootstrap security, so the SecurityFeature instance can integrate security into Web Server.">まず、セキュリティ・インスタンスがセキュリティのブートストラップに使用されるため、SecurityFeatureインスタンスはセキュリティをWebサーバーに統合できます。</span> <span class="merged" id="all.UgP3q.spl3" title="原文 : Then, OidcFeature instance registers the endpoint to which OIDC redirects browser after a successful login.">次に、OidcFeatureインスタンスは、ログインの成功後にOIDCがブラウザをリダイレクトするエンドポイントを登録します。</span> </p>

<p><span class="merged" id="all.4Q6mK6" title="原文 : Helidon sample is now setup and ready.">Helidonサンプルが設定され、準備が整いました。</span></p>
...
<ol style="margin-left: 15px;">
<li>
<span class="merged" id="all.7Xc48.1" title="原文 : Open your favourite browser and try to access http://localhost:7987/greet/Michael.">お気に入りのブラウザを開き、<code><a class="bare" href="http://localhost:7987/greet/Michael" target="_blank">http://localhost:7987/greet/Michael</a></code>にアクセスしてみます。</span>
...
<li>
<span class="merged" id="all.4ZpsMN.1" title="原文 : Enter the following into URL : http://localhost:7987/greet.">URLに次のように入力 : <code><a class="bare" href="http://localhost:7987/greet" target="_blank">http://localhost:7987/greet</a></code>。</span>
...
<li>
<span class="merged" id="all.1Dqs7b.1" title="原文 : After successful log in, keycloak redirect you to the http://localhost:7987/greet endpoint and print Hello word.">ログインに成功すると、keycloakによって<code><a class="bare" href="http://localhost:7987/greet" target="_blank">http://localhost:7987/greet</a></code>エンドポイントにリダイレクトされ、Helloワードが出力されます。</span>
...
<h3 id="Test-Keycloak-process-with-Postman"><span class="merged" id="all.41tvNz" title="原文 : Test Keycloak Process with Postman">PostmanによるKeycloakプロセスのテスト</span></h3>
<div class="section">
...
<ol style="margin-left: 15px;">
<li>
<span class="merged" id="all.9TTcI.1" title="原文 : Open the Keycloak Console.">Keycloakコンソールを開きます。</span>
...
<p><span class="merged" id="all.1yLlsJ.1.spl1" title="原文 : For the first step, paste the following URL into your browser: http://localhost:8080/auth/realms/myRealm/protocol/openid-connect/auth?client_id=myClientID&amp;response_type=code.">最初のステップでは、次のURLをブラウザに貼り付けます : <code><a class="bare" href="http://localhost:8080/auth/realms/myRealm/protocol/openid-connect/auth?client_id=myClientID&amp;response_type=code" target="_blank">http://localhost:8080/auth/realms/myRealm/protocol/openid-connect/auth?client_id=myClientID&amp;response_type=code</a></code>。</span> <span class="merged" id="all.1yLlsJ.1.spl2" title="原文 : The first part of the url http:/../auth is the Keycloak endpoint to request an authorization code.">URL <code>http:/../auth</code>の最初の部分は、認可コードをリクエストするKeycloakエンドポイントです。</span> <span class="merged" id="all.1yLlsJ.1.spl3" title="原文 : Two query parameters are provided, the client id and the response type.">クライアントIDとレスポンス・タイプの2つの問合せパラメータが用意されています。</span> <span class="merged" id="all.1yLlsJ.1.spl4" title="原文 : Press enter and Keycloak responds with different URL containing a query parameter code.">Enterキーを押すと、問合せパラメータ<code>code</code>を含む別のURLでKeycloakが応答します。</span> <span class="merged" id="all.1yLlsJ.1.spl5" title="原文 : You successfully received the authorization code.">認可コードが正常に受信されました。</span> </p>

<p><span class="merged" id="all.vTCRi.1.spl1" title="原文 : In order to achieve the third step, we can use Postman to exchange the authorization code for tokens.">3番目のステップを達成するために、Postmanを使用してトークンに対する認可コードを交換できます。</span> <span class="merged" id="all.vTCRi.1.spl2" title="原文 : In Postman, select the Http POST method.">Postmanで、HTTP POSTメソッドを選択します。</span> <span class="merged" id="all.vTCRi.1.spl3" title="原文 : Keycloak endpoint to get token is the following: http://localhost:8080/auth/realms/myRealm/protocol/openid-connect/token.">トークンを取得するKeycloakエンドポイントは次のとおりです : <code><a class="bare" href="http://localhost:8080/auth/realms/myRealm/protocol/openid-connect/token" target="_blank">http://localhost:8080/auth/realms/myRealm/protocol/openid-connect/token</a></code>。</span> <span class="merged" id="all.vTCRi.1.spl4" title="原文 : In the body of the request, select x-www-form-urlencoded type.">リクエストの本文で、<code>x-www-form-urlencoded</code>タイプを選択します。</span> <span class="merged" id="all.vTCRi.1.spl5" title="原文 : Add the following data:">次のデータを追加します:</span> </p>
...
<div class="section">
<p><span class="merged" id="all.ewsoF.1.spl1" title="原文 : The Direct Access Grants flow is used by REST clients that want to request tokens on behalf of a user.">直接アクセス付与フローは、ユーザーにかわってトークンをリクエストするRESTクライアントで使用されます。</span> <span class="merged" id="all.ewsoF.1.spl2" title="原文 : To use Postman to make this request on behalf of myuser, select the GET method and enter this URL: http://localhost:7987/greet/.">Postmanを使用して<code>myuser</code>のかわりにこのリクエストを実行するには、GETメソッドを選択し、このURLを入力 : <code><a class="bare" href="http://localhost:7987/greet/" target="_blank">http://localhost:7987/greet/</a></code>。</span> <span class="merged" id="all.ewsoF.1.spl3" title="原文 : Under Authorization tab, select authorization type`OAuth 2.0`."><code>Authorization</code>タブで、認可タイプ`OAuth 2.0`を選択します。</span> <span class="merged" id="all.ewsoF.1.spl4" title="原文 : Under it, complete the sentence Add authorization data to with Request Headers, and complete the required fields.">下で、文<code>Add authorization data to</code>に<code>Request Headers</code>を入力し、必要なフィールドに入力します。</span> </p>

<p><span class="merged" id="all.2EmYiR.spl1" title="原文 : Note: Make sure your Helidon application is running.">ノート: Helidonアプリケーションが実行中であることを確認します。</span> <span class="merged" id="all.2EmYiR.spl2" title="原文 : If it is not, please start it.">そうでない場合は、開始してください。</span> </p>
...
<p><span class="merged" id="all.4Abqji" title="原文 : In the test folder open the application.yaml file: helidon-quickstart-se/src/test/resources/application.yaml">testフォルダで、application.yamlファイルを開きます: <code>helidon-quickstart-se/src/test/resources/application.yaml</code></span></p>
...
<p><span class="merged" id="all.1QiWFc" title="原文 : In the AbstractMainTest.java file, tests need to be modified to check the application security when accessing /greet path with a GET method."><code>AbstractMainTest.java</code>ファイルで、<code>GET</code>メソッドを使用して<code>/greet</code>パスにアクセスするときにアプリケーション・セキュリティをチェックするようにテストを変更する必要があります。</span></p>

<markup
lang="java"
title="Import the following class:"
>import io.helidon.http.Status;</markup>

<markup
lang="java"
title="Replace the first webclient call by this one into testRootRoute method:"
...
<markup
lang="java"
title="Add new check to the testRootRoute method:"
...
assertThat(jsonObject.getString("message"), is("Hello World!"));</markup>
...
<h4 id="Restrict-access-to-a-specific-role"><span class="merged" id="all.2aQryx.1" title="原文 : Restrict Access to a Specific Role">アクセスを特定のロールに制限</span></h4>
<div class="section">
...
<p><span class="merged" id="all.4LwIqr.spl1" title="原文 : Now, only Jack has access to secure endpoint as he has an admin role.">Jackのみが管理ロールを持つため、セキュアなエンドポイントにアクセスできるようになりました。</span> <span class="merged" id="all.4LwIqr.spl2" title="原文 : John, as a simple user, can not access it.">ジョンは単純なユーザーなので、アクセスできません。</span> <span class="merged" id="all.4LwIqr.spl3" title="原文 : Once it is done, go to the tests to check the application behavior.">完了したら、テストに移動してアプリケーションの動作を確認します。</span> <span class="merged" id="all.4LwIqr.spl4" title="原文 : The test from previous section is still passing as jack has access.">jackがアクセスできるため、前のセクションからのテストに合格しています。</span> </p>
