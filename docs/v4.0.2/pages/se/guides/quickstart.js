...
<tbody>
<tr>
<td class=""><span class="merged" id="all.3EWh8P.6" title="原文 : A Helidon SE Application">Helidon SEアプリケーション</span></td>
<td class=""><span class="merged" id="all.3f8L7i.6" title="原文 : You can use your own application or use the Helidon SE Quickstart to create a sample application.">独自のアプリケーションを使用するか、<router-link to="/se/guides/quickstart">「Helidon SEクイックスタート」</router-link>を使用してサンプル・アプリケーションを作成できます。</span></td>
</tr>
<tr>
<td class=""><span class="merged" id="all.17vEm3.21" title="原文 : Java&#160;SE&#160;21 (Open&#160;JDK&#160;21)"><a href="https://www.oracle.com/technetwork/java/javase/downloads" target="_blank">Java SE 21</a> (<a href="http://jdk.java.net" target="_blank">JDK 21を開く</a>)</span></td>
<td class=""><span class="merged" id="all.3uVV7D.21" title="原文 : Helidon requires Java 21+.">HelidonにはJava 21+が必要です。</span></td>
</tr>
<tr>
<td class=""><span class="merged" id="all.UWTyd.21" title="原文 : Maven 3.8+"><a href="https://maven.apache.org/download.cgi" target="_blank">Maven 3.8+</a></span></td>
<td class=""><span class="merged" id="all.47RJEf.21" title="原文 : Helidon requires Maven 3.8+.">HelidonにはMaven 3.8+が必要です。</span></td>
</tr>
<tr>
...
<p><span class="merged" id="all.1BP9lM.1.spl1" title="原文 : The example is a very simple &quot;Hello World&quot; greeting service.">この例は、非常に単純な「Hello World」サービスです。</span> <span class="merged" id="all.1BP9lM.1.spl2" title="原文 : It supports GET requests for generating a greeting message, and a PUT request for changing the greeting itself.">応答メッセージを生成するためのGETリクエストと、応答メッセージ自体を変更するためのPUTリクエストをサポートしています。</span> <span class="merged" id="all.1BP9lM.1.spl3" title="原文 : The response is encoded using JSON.">レスポンスはJSONを使用してエンコードされます。</span> <span class="merged" id="all.1BP9lM.1.spl4"  title="原文:: For example:">例えば:</span> </p>
...
lang="bash"
title="Health"
>curl -sv -X GET http://localhost:8080/observe/health</markup>

<p><span class="merged" id="all.OB0VA" title="原文 : Notice we use the -v option to curl so that you can see that the health endpoint returns 204 (No Content) by default.">ヘルス・エンドポイントがデフォルトで204 (コンテンツなし)を返すように、<code>-v</code>オプションを使用してcurlします。</span></p>

<markup
lang="bash"
title="Metrics in Prometheus Format"
>curl -s -X GET http://localhost:8080/observe/metrics</markup>

<markup
lang="bash"
title="Metrics in JSON Format"
>curl -H 'Accept: application/json' -X GET http://localhost:8080/observe/metrics</markup>
...
<div class="section">
<p><span class="merged" id="all.178sc6.2.spl1" title="原文 : If you don’t have access to a Kubernetes cluster, you can install one on your desktop.">Kubernetesクラスタにアクセスできない場合は、<router-link to="/about/kubernetes">「デスクトップにインストール」</router-link>を使用できます。</span> <span class="merged" id="all.178sc6.2.spl2" title="原文 : Then deploy the example:">次に、例をデプロイします:</span> </p>
...
<p><span class="merged" id="all.1C6LBc.1.spl1" title="原文 : Note the PORTs.">ポートを書き留めます。</span> <span class="merged" id="all.1C6LBc.1.spl2" title="原文 : You can now exercise the application as you did before but use the second port number (the NodePort) instead of 8080.">これで、以前と同様にアプリケーションを実行できますが、8080のかわりに2番目のポート番号(NodePort)を使用します。</span> <span class="merged" id="all.1C6LBc.1.spl3"  title="原文:: For example:">例えば:</span> </p>
...
<p><span class="merged" id="all.MVSYM.1" title="原文 : After you’re done, cleanup.">完了したら、クリーンアップします。</span></p>
...
<ul class="ulist">
<li>
<p><span class="merged" id="all.4dGDEm" title="原文 : GraalVM Native Images"><router-link to="/se/guides/graalnative">GraalVMネイティブ・イメージ</router-link></span></p>

</li>
<li>
<p><span class="merged" id="all.2aRNPx" title="原文 : Custom Runtime Images using jlink"><router-link to="/se/guides/jlink-image"><code>jlink</code>を使用したカスタム・ランタイム・イメージ</router-link></span></p>
...
<div class="section">
<p><span class="merged" id="all.1oDspe.1.spl1" title="原文 : With the Helidon CLI you can create additional types of Helidon applications and use the &quot;dev loop&quot; to do fast, iterative development.">Helidon CLIを使用すると、追加のタイプのHelidonアプリケーションを作成し、devループを使用して迅速かつ反復的に開発できます。</span> <span class="merged" id="all.1oDspe.1.spl2" title="原文 : Try it now."><router-link to="/about/cli">「今すぐ試してみる」</router-link>。</span> </p>
