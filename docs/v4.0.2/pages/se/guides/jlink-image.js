...
<dd slot="desc"><p><span class="merged" id="all.1X8uFJ.1" title="原文 : This guide describes how to build a custom runtime image for your Helidon application using Helidon’s support for the JDK’s jlink tool.">このガイドでは、HelidonによるJDKの<code>jlink</code>ツールのサポートを使用して、Helidonアプリケーションのカスタム・ランタイム・イメージをビルドする方法について説明します。</span></p>
</dd>
</dl>
</v-card-text>
</v-card>
...
<div class="section">
<p><span class="merged" id="all.1cUl5U.1.spl1" title="原文 : JDK 9 introduced the jlink command that supports assembling a set of modules and their dependencies into a custom runtime image.">JDK 9では、一連のモジュールとその依存関係のカスタム・ランタイム・イメージへのアセンブルをサポートする<a href="https://docs.oracle.com/en/java/javase/21/docs/specs/man/jlink.html" target="_blank"><code>jlink</code></a>コマンドが導入されました。</span> <span class="merged" id="all.1cUl5U.1.spl2" title="原文 : The helidon-maven-plugin has support for easily creating a custom runtime image for your Helidon application resulting in a smaller, better performing runtime."><code>helidon-maven-plugin</code>では、Helidonアプリケーションのカスタム・ランタイム・イメージの簡単な作成がサポートされているため、実行時間が小さくなり、パフォーマンスが向上します。</span> </p>
...
<div class="section">
<p><span class="merged" id="all.2vag3o.4" title="原文 : For this 10 minute tutorial, you will need the following:">この10分間のチュートリアルでは、次のものが必要です:</span></p>
...
<tbody>
<tr>
<td class=""><span class="merged" id="all.3EWh8P.4" title="原文 : A Helidon SE Application">Helidon SEアプリケーション</span></td>
<td class=""><span class="merged" id="all.3f8L7i.4" title="原文 : You can use your own application or use the Helidon SE Quickstart to create a sample application.">独自のアプリケーションを使用するか、<router-link to="/se/guides/quickstart">「Helidon SEクイックスタート」</router-link>を使用してサンプル・アプリケーションを作成できます。</span></td>
</tr>
<tr>
<td class=""><span class="merged" id="all.17vEm3.19" title="原文 : Java&#160;SE&#160;21 (Open&#160;JDK&#160;21)"><a href="https://www.oracle.com/technetwork/java/javase/downloads" target="_blank">Java SE 21</a> (<a href="http://jdk.java.net" target="_blank">JDK 21を開く</a>)</span></td>
<td class=""><span class="merged" id="all.3uVV7D.19" title="原文 : Helidon requires Java 21+.">HelidonにはJava 21+が必要です。</span></td>
</tr>
<tr>
<td class=""><span class="merged" id="all.UWTyd.19" title="原文 : Maven 3.8+"><a href="https://maven.apache.org/download.cgi" target="_blank">Maven 3.8+</a></span></td>
<td class=""><span class="merged" id="all.47RJEf.19" title="原文 : Helidon requires Maven 3.8+.">HelidonにはMaven 3.8+が必要です。</span></td>
</tr>
<tr>
...
<div class="section">
<p><span class="merged" id="all.gbRmG.1" title="原文 : As noted in the prerequisites above, JDK 21 or newer is required.">前述の前提条件で説明したように、JDK 21以降が必要です。</span></p>
...
<p ><span class="merged" id="all.4fJuAz.1.spl1" title="原文 : RPM based distributions provide *.jmod files in separate java-*-openjdk-jmods packages."><a href="https://en.wikipedia.org/wiki/List_of_Linux_distributions#RPM-based" target="_blank">「RPMベース」</a>ディストリビューションでは、<code>*.jmod</code>ファイルが個別の<code>java-*-openjdk-jmods</code>パッケージで提供されます。</span> <span class="merged" id="all.4fJuAz.1.spl2" title="原文 : Debian based distributions provide *.jmod files only in the openjdk-*-jdk-headless packages."><a href="https://en.wikipedia.org/wiki/List_of_Linux_distributions#Debian-based" target="_blank">「Debianベース」</a>ディストリビューションは、<code>openjdk-*-jdk-headless</code>パッケージでのみ<code>*.jmod</code>ファイルを提供します。</span> </p>
</div>

</div>


<h2 id="_generate_the_project"><span class="merged" id="all.3GySK3.4" title="原文 : Generate the Project">プロジェクトを生成</span></h2>
<div class="section">
...
<p><span class="merged" id="all.14GlBS" title="原文 : For more information about the Quickstart application and other endpoints it supports see the Helidon SE quickstart Guide.">Quickstartアプリケーションおよびサポートされているその他のエンドポイントの詳細は、<router-link to="/se/guides/quickstart">「Helidon SEクイックスタート・ガイド」</router-link>を参照してください。</span></p>
...
<h3 id="_local_build"><span class="merged" id="all.Zgu5Y.1" title="原文 : Local Build">ローカル・ビルド</span></h3>
<div class="section">
...
<p><span class="merged" id="all.4P1eEO.1" title="原文 : If you’d rather have a smaller image size (with a slightly increased startup time) you can skip the creation of the CDS archive by executing your build like this:">(起動時間を少し長くして)イメージ・サイズを小さくする場合は、次のようにビルドを実行してCDSアーカイブの作成をスキップできます:</span></p>
...
<p><span class="merged" id="all.38WD5I.1" title="原文 : You can exercise the application’s endpoints as before.">以前と同様に、アプリケーションのエンドポイントを実行できます。</span></p>
...
<h2 id="_using_custom_runtime_images"><span class="merged" id="all.nY3BX.1" title="原文 : Using Custom Runtime Images">カスタム・ランタイム・イメージの使用</span></h2>
<div class="section">
...
<p><span class="merged" id="all.1S7D5J" title="原文 : For cases where absolute minimal startup time and image size are required, then consider using GraalVM Native Images.">起動時間とイメージの絶対最小サイズが必要な場合は、<router-link to="/se/guides/graalnative">「GraalVMネイティブ・イメージ」</router-link>の使用を検討してください。</span></p>
