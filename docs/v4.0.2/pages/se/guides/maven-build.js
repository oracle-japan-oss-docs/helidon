...
<dd slot="desc"><p><span class="merged" id="all.2yZVmw.1" title="原文 : This guide describes Helidon’s support for Maven projects.">このガイドでは、HelidonでのMavenプロジェクトのサポートについて説明します。</span></p>
</dd>
</dl>
</v-card-text>
</v-card>
...
<div class="section">
<p><span class="merged" id="all.3Pz8wM.spl1" title="原文 : Helidon examples and projects generated using the Helidon Quickstart use a Helidon application POM as their parent."><router-link to="/se/guides/quickstart">「Helidonクイックスタート」</router-link>を使用して生成されたHelidonの例およびプロジェクトでは、親としてHelidonアプリケーションPOMを使用します。</span> <span class="merged" id="all.3Pz8wM.spl2" title="原文 : This parent POM provides the following:">この親POMは次のものを提供します:</span> </p>
...
<p><span class="merged" id="all.3RZXMt.spl1" title="原文 : If you want to use your own parent POM, then take a look at the standalone quickstart example.">独自の親POMを使用する場合は、<a href="https://github.com/oracle/helidon/tree/4.0.2/examples/quickstarts/helidon-standalone-quickstart-se" target="_blank">「スタンドアロン・クイックスタートの例」</a>を参照してください。</span> <span class="merged" id="all.3RZXMt.spl2" title="原文 : This example has a stand-alone POM that you can pattern your own application POM after.">この例には、独自のアプリケーションPOMを後からパターン化できるスタンドアロンPOMがあります。</span> </p>

<p><span class="merged" id="all.3HS3lM.1" title="原文 : For more details on Helidon application POMs see the Helidon’s Application POMS">HelidonアプリケーションPOMの詳細は、<a href="https://github.com/oracle/helidon/tree/4.0.2/docs-internal/application-pom.md" target="_blank">「HelidonアプリケーションPOMS」</a>を参照してください</span></p>

</div>


<h2 id="_dependency_management"><span class="merged" id="all.Udjxg.3"  title="原文:: Dependency Management"> 依存性管理</span></h2>
<div class="section">
...
<p><span class="merged" id="all.2Cmqrr.1.spl1" title="原文 : When you use a Helidon Application POM as your project’s parent pom, you inherit Helidon’s dependency management.">Helidon Application POMをプロジェクトの親POMとして使用する場合、Helidonの依存関係管理を継承します。</span> <span class="merged" id="all.2Cmqrr.1.spl2" title="原文 : If you have your own parent, then you can import Helidon dependency management like this:">独自の親がある場合は、次のようにHelidonの依存関係管理をインポートできます:</span> </p>
...
<ol style="margin-left: 15px;">
<li>
<span class="merged" id="all.2pt65P" title="原文 : jlink-image: Build a custom runtime Java image.">jlink-image: <router-link to="/se/guides/jlink-image">「カスタム・ランタイムJavaイメージ」</router-link>を構築します。</span>

</li>
<li>
<span class="merged" id="all.NkxCm.spl1" title="原文 : native-image: Build a GraalVM native image.">native-image: <router-link to="/se/guides/graalnative">「GraalVMネイティブ・イメージ」</router-link>を構築します。</span> <span class="merged" id="all.NkxCm.spl2" title="原文 : Note: this capability is now provided via the Maven plugin for GraalVM Native Image">ノート: この機能は、<a href="https://graalvm.github.io/native-build-tools/latest/maven-plugin.html" target="_blank">「GraalVMネイティブ・イメージのMavenプラグイン」</a>を介して提供されます</span> 

</li>
</ol>

<p><span class="merged" id="all.3yWng9.1" title="原文 : For full documentation of the plugin please see the Helidon Maven Plugin README.">プラグインの完全なドキュメントは、<a href="https://github.com/oracle/helidon-build-tools/tree/master/maven-plugins/helidon-maven-plugin" target="_blank">「Helidon MavenプラグインREADME」</a>を参照してください。</span></p>
...
<li>
<span class="merged" id="all.2kzwRk.1" title="原文 : Setting system properties in your project’s pom.xml">プロジェクトの<code>pom.xml</code>でのシステム・プロパティの設定</span>
...
<div class="section">
<p><span class="merged" id="all.1X72OQ.1" title="原文 : Or you can set the properties in your project’s pom.xml:">または、プロジェクトのpom.xmlでプロパティを設定できます:</span></p>
...
<div class="section">
<p><span class="merged" id="all.2tAvi3.1" title="原文 : For full control you can override the plugin’s configuration using pluginManagement:">完全に制御するには、<code>pluginManagement</code>を使用してプラグインの構成をオーバーライドします:</span></p>
