...
<ul class="ulist">
<li>
<p><span class="merged" id="all.1S1Ao6.42" title="原文 : Overview"><router-link @click.native="this.scrollFix('#_overview')" to="#_overview">概要</router-link></span></p>
...
<h2 id="_overview"><span class="merged" id="all.YrpRV.58"  title="原文:: Overview">概要</span></h2>
<div class="section">
...
<div class="section">
<p><span class="merged" id="all.2r1O4t" title="原文 : The config system represents configuration in memory using three types of nodes, each a different interface defined within the ConfigNode interface.">configシステムは、<a href="/apidocs/io.helidon.config/io/helidon/config/spi/ConfigNode.html" target="_blank"><code>ConfigNode</code></a>インタフェース内で定義された異なるインタフェースを持つ3つのタイプのノードを使用してメモリー内の構成を表します。</span></p>
...
<th><span class="merged" id="all.JUOSC.40"  title="原文:: Usage">使用</span></th>
</tr>
</thead>
<tbody>
<tr>
...
<p ><p><span class="merged" id="all.kr1We.spl1" title="原文 : To emphasize, the dot character (“.”) has special meaning as a name separator in keys.">強調するために、ドット文字(".")はキーの名前セパレータとして特別な意味を持ちます。</span> <span class="merged" id="all.kr1We.spl2" title="原文 : To include a dot as a character in a key escape it as “~1”.">ドットをキーに文字として含めるには、"~1"としてエスケープします。</span> <span class="merged" id="all.kr1We.spl3" title="原文 : To include a tilda escape it as “~0”.">ティルダを含めるには、"~0"としてエスケープします。</span> </p>
</p>
</div>
...
<li>
<span class="merged" id="all.2XVmjk" title="原文 : Each node’s key is the fully-qualified path using dotted names from the root to that node.">各ノードのキーは、ルートからそのノードへのドット表記名を使用した完全修飾パスです。</span>
...
<p><span class="merged" id="all.1Z4WA7" title="原文 : The Config object exposes methods to return the name, key, and type of the node."><code>Config</code>オブジェクトは、ノードの<a href="/apidocs/io.helidon.config/io/helidon/config/Config.html#name--" target="_blank"><code>name</code></a>、<a href="/apidocs/io.helidon.config/io/helidon/config/Config.html#key--" target="_blank"><code>key</code></a>および<a href="/apidocs/io.helidon.config/io/helidon/config/Config.html#type--" target="_blank"><code>type</code></a>を返すメソッドを公開します。</span></p>
...
<th><span class="merged" id="all.JUOSC.41"  title="原文:: Usage">使用</span></th>
</tr>
</thead>
<tbody>
<tr>
...
<tr>
<td class=""><span class="merged" id="all.2SYAnU.5"  title="原文: type"><code>type</code></span></td>
...
<th><span class="merged" id="all.JUOSC.42"  title="原文:: Usage">使用</span></th>
</tr>
</thead>
<tbody>
<tr>
...
<p><span class="merged" id="all.3TqTeB.spl1" title="原文 : You might want to write the web portion of your app to work with a config subtree with keys that are independent of the subtree’s position in a larger tree.">大きいツリーでのサブツリーの位置に依存しないキーを持つ構成サブツリーを操作するように、アプリケーションのweb部分を記述できます。</span> <span class="merged" id="all.3TqTeB.spl2" title="原文 : This would allow you to reuse the web portion of your application without change, regardless of which structure a config source used.">これにより、構成ソースで使用されている構造に関係なく、アプリケーションのweb部分を変更せずに再利用できます。</span> </p>

<p><span class="merged" id="all.4eAAFW.spl1" title="原文 : One easy way to do this is to detach a subtree from a larger config tree.">これを簡単に行うには、より大きな構成ツリーからのサブツリーを<em>「デタッチ」</em>します。</span> <span class="merged" id="all.4eAAFW.spl2" title="原文 : When your application invokes the Config.detach method it gets back a copy of the config node but with no parent.">アプリケーションが<a href="/apidocs/io.helidon.config/io/helidon/config/Config.html#detach--" target="_blank"><code>Config.detach</code></a>メソッドを呼び出すと、親を持たない構成ノードの<em>「コピー」</em>が返されます。</span> <span class="merged" id="all.4eAAFW.spl3" title="原文 : The copy and the original node both point to the same objects for their child nodes (if any).">コピーと元のノードの両方が、子ノード(存在する場合)の同じオブジェクトを指しています。</span> <span class="merged" id="all.4eAAFW.spl4" title="原文 : The original node is unchanged.">元のノードは変更されません。</span> </p>
