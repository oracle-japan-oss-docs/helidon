...
<h2 id="_contents"><span class="merged" id="all.1Q34Zl.42"  title="原文:: Contents">目次</span></h2>
<div class="section">
<ul class="ulist">
<li>
<p><span class="merged" id="all.1S1Ao6.41" title="原文 : Overview"><router-link @click.native="this.scrollFix('#_overview')" to="#_overview">概要</router-link></span></p>
...
<h2 id="_overview"><span class="merged" id="all.YrpRV.57"  title="原文:: Overview">概要</span></h2>
<div class="section">
...
<p><span class="merged" id="all.LXz3i.spl1" title="原文 : The config system would iterate through all ConfigSourceProvider implementations found through Java ServiceLoader based on their weight.">構成システムは、<a href="/apidocs/io.helidon.common/io/helidon/common/Weight.html" target="_blank">「重み」</a>に基づいて、Java <code>ServiceLoader</code>を介して検出されたすべての<code>ConfigSourceProvider</code>実装を反復します。</span> <span class="merged" id="all.LXz3i.spl2" title="原文 : First provider that returns true when supports(&quot;my-type&quot;) is called would be used, and an instance of a ConfigSource created using create(&quot;my-type&quot;, config), where config is located on the node of properties from config profile."><code>supports("my-type")</code>がコールされたときに<code>true</code>を返す最初のプロバイダが使用され、<code>create("my-type", config)</code>を使用して作成された<code>ConfigSource</code>のインスタンス(<code>config</code>は、構成プロファイルから<code>properties</code>のノードにあります)。</span> </p>
...
<div class="section">
<p><span class="merged" id="all.4cqcEL.spl1" title="原文 : The config system invokes extensions of a given type in priority order.">configシステムは、指定されたタイプの拡張を優先度順に呼び出します。</span> <span class="merged" id="all.4cqcEL.spl2" title="原文 : Developers can express the relative importance of an extension by annotating the service implementation class with @Weight.">開発者は、サービス実装クラスに<a href="/apidocs/io.helidon.common/io/helidon/common/Weight.html" target="_blank"><code>@Weight</code></a>注釈を付けることで、拡張の相対的な重要性を表すことができます。</span> <span class="merged" id="all.4cqcEL.spl3"  title="原文: The default value is 100.">デフォルト値は100です。</span> <span class="merged" id="all.4cqcEL.spl4" title="原文 : The higher the weight, the more important the extension is.">加重が高いほど、延長が重要になります。</span> </p>
...
<div class="section">
<p><span class="merged" id="all.1ycNvJ.spl1" title="原文 : The config system includes built-in support for several types of sources (for example, Java String, Readable, Properties, and Map objects - see ConfigSources).">構成システムには、複数のタイプのソースに対する組込みサポートが含まれています (たとえば、Java <code>String</code>, <code>Readable</code>, <code>Properties</code>オブジェクトや<code>Map</code>オブジェクト - <a href="/apidocs/io.helidon.config/io/helidon/config/ConfigSources.html" target="_blank"><code>ConfigSources</code></a>を参照)。</span> <span class="merged" id="all.1ycNvJ.spl2" title="原文 : Implement a ConfigSource to load raw configuration data from a type of source that the config system does not already support."><a href="/apidocs/io.helidon.config/io/helidon/config/spi/ConfigSource.html" target="_blank"><code>ConfigSource</code></a>を実装して、構成システムでまだサポートされていないタイプのソースからRAW構成データをロードします。</span> </p>
...
<div class="section">
<p><span class="merged" id="all.1WfhC0.spl1" title="原文 : The parsing step converts config data in some format into the corresponding in-memory representation of config ObjectNodes.">解析ステップでは、なんらかの形式の構成データを、対応するインメモリー表現の構成<code>ObjectNode</code>に変換します。</span> <span class="merged" id="all.1WfhC0.spl2" title="原文 : The config system can already parse several data formats (for example Java Properties, YAML, and HOCON).">構成システムでは、複数のデータ形式(Java <code>Properties</code>、YAML、HOCONなど)をすでに解析できます。</span> <span class="merged" id="all.1WfhC0.spl3" title="原文 : Implement the ConfigParser SPI to allow the config system to handle additional formats."><a href="/apidocs/io.helidon.config/io/helidon/config/spi/ConfigParser.html" target="_blank"><code>ConfigParser</code></a> SPIを実装して、構成システムが追加の形式を処理できるようにします。</span> </p>
...
<p><span class="merged" id="all.4WHDkI.spl1" title="原文 : ConfigParser accepts @Weight."><code>ConfigParser</code>は、<a href="/apidocs/io.helidon.common/io/helidon/common/Weight.html" target="_blank"><code>@Weight</code></a>を受け入れます。</span> <span class="merged" id="all.4WHDkI.spl2" title="原文 : See About Priority."><router-link @click.native="this.scrollFix('#priority-info')" to="#priority-info">「優先度について」</router-link>を参照してください。</span> </p>
...
<p><span class="merged" id="all.4DNaxZ.spl1" title="原文 : To furnish overrides to the config system, implement the OverrideSource SPI one or more times and pass instances of those implementations to the config builder’s overrides method.">構成システムにオーバーライドを提供するには、<a href="/apidocs/io.helidon.config/io/helidon/config/spi/OverrideSource.html" target="_blank"><code>OverrideSource</code></a> SPIを1回以上実装し、それらの実装のインスタンスを構成ビルダーの<a href="/apidocs/io.helidon.config/io/helidon/config/Config.Builder.html#overrides-java.util.function.Supplier-" target="_blank"><code>overrides</code></a>メソッドに渡します。</span> <span class="merged" id="all.4DNaxZ.spl2" title="原文 : The config system will apply the overrides returned from each OverrideSource to each config key requested from a Config that is based on that Config.Builder.">構成システムは、各<code>OverrideSource</code>から返されたオーバーライドを、その<code>Config.Builder</code>に基づく<code>Config</code>からリクエストされた各構成キーに適用します。</span> </p>

<p><span class="merged" id="all.9z7ow" title="原文 : To support custom override sources in config profiles, also implement the OverrideSourceProvider service loader SPI">構成プロファイルでカスタム・オーバーライド・ソースをサポートするには、<a href="/apidocs/io.helidon.config/io/helidon/config/spi/OverrideSourceProvider.html" target="_blank"><code>OverrideSourceProvider</code></a>サービス・ローダーSPIも実装</span></p>
...
<div class="section">
<p><span class="merged" id="all.fUYWP.spl1" title="原文 : Before returning a String from Config.value() the config system applies any filters set up on the Config.Builder used to create the config tree that contains the config node of interest."><code>Config.value()</code>から<code>String</code>を返す前に、構成システムは、目的の構成ノードを含む構成ツリーを作成するために使用される<code>Config.Builder</code>に設定されている<em>「フィルタ」</em>を適用します。</span> <span class="merged" id="all.fUYWP.spl2" title="原文 : The application provides filters as implementations of the ConfigFilter interface.">アプリケーションは、<a href="/apidocs/io.helidon.config/io/helidon/config/spi/ConfigFilter.html" target="_blank"><code>ConfigFilter</code></a>インタフェースの実装としてフィルタを提供します。</span> <span class="merged" id="all.fUYWP.spl3" title="原文 : Each filter is a function which accepts a Config.Key and an input String value and returns a String value the config system should use for that key going forward.">各フィルタは、<code>Config.Key</code>と入力<code>String</code>値を受け入れ、構成システムが今後そのキーに使用する<code>String</code>値を返す関数です。</span> <span class="merged" id="all.fUYWP.spl4" title="原文 : The filter can return the original value or return some other value.">フィルタは、元の値を返すことも、他の値を返すこともできます。</span> </p>

<p><span class="merged" id="all.3UeyTJ.spl1" title="原文 : The application registers filters and filter providers by passing ConfigFilter implementations to one of the config builder addFilter methods.">アプリケーションは、<code>ConfigFilter</code>実装をいずれかの構成ビルダー<a href="/apidocs/io.helidon.config/io/helidon/config/Config.Builder.html" target="_blank">「<code>addFilter</code>メソッド」</a>に渡すことで、フィルタおよびフィルタ・プロバイダを登録します。</span> <span class="merged" id="all.3UeyTJ.spl2" title="原文 : The config system also uses the Java service loader mechanism to load additional filters automatically, for all builders, using the service interface described in the following table.">また、構成システムでは、次の表で説明するサービス・インタフェースを使用して、すべてのビルダーに対して追加のフィルタを自動的にロードするJavaサービス・ローダー・メカニズムも使用します。</span> <span class="merged" id="all.3UeyTJ.spl3" title="原文 : Prevent a given builder from using the autoloaded filters by invoking the disableFilterServices method."><a href="/apidocs/io.helidon.config/io/helidon/config/Config.Builder.html#disableFilterServices--" target="_blank"><code>disableFilterServices</code></a>メソッドを起動して、特定のビルダーが自動ロードされたフィルタを使用しないようにします。</span> </p>
...
<th><span class="merged" id="all.JUOSC.38"  title="原文:: Usage">使用</span></th>
</tr>
</thead>
<tbody>
<tr>
<td class=""><span class="merged" id="all.NXzes" title="原文 : ConfigFilter"><a href="/apidocs/io.helidon.config/io/helidon/config/spi/ConfigFilter.html" target="_blank"><code>ConfigFilter</code></a></span></td><td class=""><span class="merged" id="all.2V0vcZ.spl1" title="原文 : Accepts @Weight."><a href="/apidocs/io.helidon.common/io/helidon/common/Weight.html" target="_blank"><code>@Weight</code></a>を受け入れます。</span> <span class="merged" id="all.2V0vcZ.spl2" title="原文 : See About Priority."><router-link @click.native="this.scrollFix('#priority-info')" to="#priority-info">「優先度について」</router-link>を参照してください。</span> </td>
...
<p><span class="merged" id="all.1sm7pH.spl1" title="原文 : Instead, implement the ConfigFilter.init(Config) method on the filter.">かわりに、フィルタに<code>ConfigFilter.init(Config)</code>メソッドを実装します。</span> <span class="merged" id="all.1sm7pH.spl2" title="原文 : The config system invokes the filters&apos; init methods according to the filters priority.">構成システムは、フィルタ<router-link @click.native="this.scrollFix('#priority-info')" to="#priority-info">「優先度」</router-link>に従ってフィルタの<code>init</code>メソッドを呼び出します。</span> </p>

<p><span class="merged" id="all.2WNONb.spl1" title="原文 : Recall that whenever any code invokes Config.get, the Config instance invokes the apply method of all registered filters.">コードが<code>Config.get</code>を起動するたびに、<code>Config</code>インスタンスは<em>「すべての」</em>登録済フィルタの<code>apply</code>メソッドを起動することに注意してください。</span> <span class="merged" id="all.2WNONb.spl2" title="原文 : By the time the application retrieves config this way the config system will have run the init method on all the filters.">アプリケーションがこの方法で構成を取得すると、構成システムはすべてのフィルタに対して<code>init</code>メソッドを実行します。</span> <span class="merged" id="all.2WNONb.spl3" title="原文 : But note that when a filter’s init method invokes Config.get, the init methods of lower-priority filters will not yet have run."><em>ただし、フィルタの<code>init</code>メソッドが<code>Config.get</code>を呼び出す場合、優先度の低いフィルタの<code>init</code>メソッドはまだ実行されていないことに注意してください。</em></span> </p>
...
<div class="section">
<p><span class="merged" id="all.WEcyP.spl1" title="原文 : The config system provides built-in mappings from String values to various Java types.">構成システムは、<code>String</code>値から様々なJavaタイプへの組込みマッピングを提供します。</span> <span class="merged" id="all.WEcyP.spl2" title="原文 : (See ConfigMappers.)">(<a href="/apidocs/io.helidon.config/io/helidon/config/ConfigMappers.html" target="_blank"><code>ConfigMappers</code></a>を参照。)</span> </p>

<p><span class="merged" id="all.swBFe" title="原文 : To handle mappings to other types the application can register custom mappers with the config system by implementing the ConfigMapperProvider SPI.">他のタイプへのマッピングを処理するために、アプリケーションでは、<a href="/apidocs/io.helidon.config/io/helidon/config/spi/ConfigMapperProvider.html" target="_blank"><code>ConfigMapperProvider</code></a> SPIを実装することで、カスタム・マッパーを構成システムに登録できます。</span></p>
...
<p><span class="merged" id="all.2HTjBt.spl1" title="原文 : Mapper providers accept @Weight.">マッパー・プロバイダは<code>@Weight</code>を受け入れます。</span> <span class="merged" id="all.2HTjBt.spl2" title="原文 : See About Priority."><router-link @click.native="this.scrollFix('#priority-info')" to="#priority-info">「優先度について」</router-link>を参照してください。</span> </p>
...
<p><span class="merged" id="all.kg908.spl1" title="原文 : A mapper provider can specify @Weight.">マッパー・プロバイダは、<a href="/apidocs/io.helidon.common/io/helidon/common/Weight.html" target="_blank"><code>@Weight</code></a>を指定できます。</span> <span class="merged" id="all.kg908.spl2" title="原文 : If no weight is explicitly assigned, the value of 100 is assumed.">重みが明示的に割り当てられていない場合、<code>100</code>の値が想定されます。</span> </p>
...
<div class="section">
<p><span class="merged" id="all.GJROq.spl1" title="原文 : Once it loads a Config tree from ConfigSource, the config system does not itself change the in-memory Config tree."><code>ConfigSource</code>から<code>Config</code>ツリーをロードすると、構成システム自体はインメモリー<code>Config</code>ツリーを変更しません。</span> <span class="merged" id="all.GJROq.spl2" title="原文 : Even so, the underlying data available via the tree’s ConfigSources can change.">その場合でも、ツリーの<code>ConfigSource</code>を介して使用可能な基礎となるデータは変更される可能性があります。</span> <span class="merged" id="all.GJROq.spl3" title="原文 : Implementations of PollingStrategy may trigger regular check whether a source has new data."><a href="/apidocs/io.helidon.config/io/helidon/config/spi/PollingStrategy.html" target="_blank"><code>PollingStrategy</code></a>の実装によって、ソースに新しいデータがあるかどうかの定期的なチェックがトリガーされる場合があります。</span> <span class="merged" id="all.GJROq.spl4" title="原文 : Implementation of ChangeWatcher may watch the underlying source for changes and trigger an update."><a href="/apidocs/io.helidon.config/io/helidon/config/spi/ChangeWatcher.html" target="_blank"><code>ChangeWatcher</code></a>の実装では、基礎となるソースの変更を監視し、更新をトリガーできます。</span> </p>
...
<div class="section">
<p><span class="merged" id="all.24ZhPe" title="原文 : The builder for each ConfigSource and OverrideSource accepts a RetryPolicy governing if and how the source should deal with failures loading the underlying data.">各<code>ConfigSource</code>および<code>OverrideSource</code>のビルダーは、基礎となるデータのロードの失敗をソースが処理するかどうか、およびどのように処理するかを制御する<a href="/apidocs/io.helidon.config/io/helidon/config/spi/RetryPolicy.html" target="_blank"><code>RetryPolicy</code></a>を受け入れます。</span></p>

<p><span class="merged" id="all.2q5Q4T.spl1" title="原文 : A retry policy accepts a function, the invocation of which the policy will govern according to its own implementation.">再試行ポリシーは、独自の実装に従ってポリシーが制御する関数を受け入れます。</span> <span class="merged" id="all.2q5Q4T.spl2" title="原文 : Applications can use the predefined policies in RetryPolicies, such as RetryPolicies.justCall which simply invokes the function without any retry.">アプリケーションでは、再試行せずにファンクションを起動するだけの<code>RetryPolicies.justCall</code>など、<a href="/apidocs/io.helidon.config/io/helidon/config/RetryPolicies.html" target="_blank"><code>RetryPolicies</code></a>の事前定義済ポリシーを使用できます。</span> <span class="merged" id="all.2q5Q4T.spl3" title="原文 : That class also exposes a builder for constructing a time-based retry policy, with several parameters:">このクラスは、時間ベースの再試行ポリシーをビルドするためのビルダーも公開します。パラメータは次のとおりです:</span> </p>
...
<th><span class="merged" id="all.JUOSC.39"  title="原文:: Usage">使用</span></th>
<th><span class="merged" id="all.oxso7.4"  title="原文:: Default">デフォルト</span></th>
</tr>
</thead>
<tbody>
<tr>
<td class=""><span class="merged" id="all.lGKge.4"  title="原文: delay"><code>delay</code></span></td>
