...
<div class="section">
<p><span class="merged" id="all.1UefFv" title="原文 : To enable Security add the following dependency to your project’s pom.xml (see Managing Dependencies).">セキュリティを有効にするには、プロジェクトの<code>pom.xml</code>に次の依存関係を追加します(<router-link to="/about/managing-dependencies">「依存関係の管理」</router-link>を参照)。</span></p>
...
<p><span class="merged" id="all.3yefc6" title="原文 : Security in Helidon MicroProfile is built on top of Jersey’s and can be enabled/disabled using the property security.jersey.enabled=[true|false].">Helidon MicroProfileのセキュリティはJerseyの上に構築され、<code>security.jersey.enabled=[true|false]</code>プロパティを使用して有効化/無効化できます。</span></p>
...
<p><span class="merged" id="all.3n9Ydw" title="原文 : These endpoints are all implemented using Helidon WebServer and as such can be protected only through Security integration with WebServer.">これらのエンドポイントはすべて、Helidon WebServerを使用して実装されるため、WebServerとのセキュリティ統合によってのみ保護できます。</span></p>
