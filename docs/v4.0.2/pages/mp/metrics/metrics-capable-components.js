...
<ul class="ulist">
<li>
<p><span class="merged" id="all.1S1Ao6.15" title="原文 : Overview"><router-link @click.native="this.scrollFix('#_overview')" to="#_overview">概要</router-link></span></p>
...
<h2 id="_overview"><span class="merged" id="all.YrpRV.15"  title="原文:: Overview">概要</span></h2>
<div class="section">
<p><span class="merged" id="all.3UP7Sq" title="原文 : This document explains Helidon MP metrics-capable components and applications and describes how to create and control them.">このドキュメントは、Helidon MPメトリクス対応のコンポーネントおよびアプリケーションについて説明し、それらを作成および制御する方法について説明します。</span></p>

<p><span class="merged" id="all.2Pc1LE" title="原文 : Think of Helidon metrics in several related but different parts:">Helidonメトリクスは、関連する複数の部分(異なる部分)で考えてください:</span></p>


<h3 id="_apis"><span class="merged" id="all.2TAXlO"  title="原文:: APIs">API</span></h3>
<div class="section">
<ul class="ulist">
<li>
<p><span class="merged" id="all.1szR3u" title="原文 : The Helidon metrics API">HelidonメトリクスAPI</span></p>
<p><span class="merged" id="all.pNO3s" title="原文 : This API allows your code to register, look-up, remove, and update metrics using the RegistryFactory, MetricRegistry, and individual metrics interfaces.">このAPIを使用すると、コードは、<code>RegistryFactory</code>、<code>MetricRegistry</code>および個々のメトリクス・インタフェースを使用してメトリクスを登録、参照、削除および更新できます。</span></p>

</li>
<li>
<p><span class="merged" id="all.1za6E0" title="原文 : The Helidon metrics REST service API">HelidonメトリクスRESTサービスAPI</span></p>
<p><span class="merged" id="all.36YWxb" title="原文 : This API allows your code to set up and respond to the /metrics endpoint so clients can retreive metrics information.">このAPIでは、クライアントがメトリクス情報を取得できるように、コードを設定して<code>/metrics</code>エンドポイントに応答できます。</span></p>
...
<h3 id="_implementations_of_the_apis"><span class="merged" id="all.NooOY" title="原文 : Implementations of the APIs">APIの実装</span></h3>
<div class="section">
<ul class="ulist">
<li>
<p><span class="merged" id="all.2gR1JS" title="原文 : Implementations of the Helidon metrics API.">HelidonメトリクスAPIの実装。</span></p>
<p><span class="merged" id="all.4W7cFx" title="原文 : Helidon provides two-​minimal and full-featured-​and selects which one to use at runtime, based on what components are present on the runtime path and whether metrics is configured to be enabled or disabled.">Helidonは、最小およびフル機能を2つ提供し、ランタイム・パスに存在するコンポーネントと、メトリクスが有効または無効に構成されているかどうかに基づいて、実行時に使用するコンポーネントを選択します。</span></p>

<p><span class="merged" id="all.1zN4LL" title="原文 : By default, Helidon MP services use the full-featured implementation.">デフォルトでは、Helidon MPサービスはフル機能実装を使用します。</span></p>

</li>
<li>
<p><span class="merged" id="all.4MyIl0" title="原文 : Implementations of the Helidon metrics REST service API.">HelidonメトリクスRESTサービスAPIの実装。</span></p>
<p><span class="merged" id="all.1UTu4l" title="原文 : Helidon provides two-​minimal and full-featured-​and selects which one to use at runtime.">Helidonは、最小およびフル機能を2つ提供し、実行時にどの機能を使用するかを選択します。</span></p>

<p><span class="merged" id="all.2Xl8Yr" title="原文 : By default, Helidon MP apps which use metrics use the full-featured metrics REST service by default.">デフォルトでは、メトリクスを使用するHelidon MPアプリケーションは、デフォルトでフル機能のメトリクスRESTサービスを使用します。</span></p>

</li>
</ul>

<p><span class="merged" id="all.Frgio.spl1" title="原文 : As you plan and write Helidon components and applications, you make some choices about exactly how your code will use metrics.">Helidonコンポーネントおよびアプリケーションを計画および記述するとき、コードでメトリクスを使用する方法について、いくつかの選択を行います。</span> <span class="merged" id="all.Frgio.spl2" title="原文 : This document gives some background information, describes each option and its effect, and provides some code examples.">このドキュメントでは、バックグラウンド情報を示し、各オプションとその効果について説明し、いくつかのコード例を提供します。</span> </p>
...
<h2 id="_usage"><span class="merged" id="all.JUOSC.8"  title="原文:: Usage">使用</span></h2>
<div class="section">
<p><span class="merged" id="all.4dOGVo" title="原文 : This section helps you decide how incorporate metrics into your software by describing the categories of metrics usage, explaining generally how Helidon implements metrics, and illustrating how to write the metrics-related code accordingly.">この項では、メトリクス使用のカテゴリを記述し、一般的にHelidonがメトリクスを実装する方法について説明し、メトリクス関連コードの記述方法を示すことで、ソフトウェアにメトリクスを組み込む方法を決定できます。</span></p>


<h3 id="_categorizing_metrics_usage"><span class="merged" id="all.1PB3e4" title="原文 : Categorizing Metrics Usage">メトリクス使用状況のカテゴリ化</span></h3>
<div class="section">
...
<p><span class="merged" id="all.4RRQRJ" title="原文 : Whenever possible, if your component or application uses metrics, then write it as metrics-capable code.">可能な場合は常に、コンポーネントまたはアプリケーションがメトリクスを使用する場合は、メトリクス対応のコードとして記述します。</span></p>

</div>


<h3 id="_understanding_the_two_metrics_implementations"><span class="merged" id="all.WjCl0" title="原文 : Understanding the Two Metrics Implementations">2つのメトリクスの実装の理解</span></h3>
<div class="section">
<p><span class="merged" id="all.2a2k8Z" title="原文 : Helidon provides two metrics implementations:">Helidonには、次の2つのメトリクス実装があります:</span></p>
...
<h3 id="_understanding_the_two_metrics_service_implementations"><span class="merged" id="all.v3uce" title="原文 : Understanding the Two Metrics Service Implementations">2つのメトリクス・サービス実装の理解</span></h3>
<div class="section">
...
<h3 id="_enabling_and_disabling_metrics"><span class="merged" id="all.4HJAOy" title="原文 : Enabling and Disabling Metrics">メトリクスの有効化および無効化</span></h3>
<div class="section">
...
<h3 id="_designing_and_writing_metrics_capable_applications_and_components"><span class="merged" id="all.1vymNA" title="原文 : Designing and Writing Metrics-capable Applications and Components">メトリクス対応のアプリケーションおよびコンポーネントの設計と記述</span></h3>
<div class="section">
...
<h4 id="_declaring_dependencies"><span class="merged" id="all.4bs1kZ" title="原文 : Declaring Dependencies">依存関係の宣言</span></h4>
<div class="section">
...
<h3 id="_writing_metrics_capable_code"><span class="merged" id="all.1gYHIB" title="原文 : Writing Metrics-capable Code">メトリクス対応コードの記述</span></h3>
<div class="section">
...
<li data-value="6"><span class="merged" id="all.4cAo8v" title="原文 : Provides easy access to the MetricRegistry which the component’s metrics code should use.">コンポーネントのメトリクス・コードを使用する<code>MetricRegistry</code>に簡単にアクセスできます。</span></li>
</ul>
...
<div class="section">
<p><span class="merged" id="all.3Bziqk" title="原文 : The following example shows how useful metrics-capable code can be in the context of building Docker images.">次の例は、Dockerイメージの作成のコンテキストで有用なメトリクス対応コードを使用する方法を示しています。</span></p>
...
<h3 id="_advantages_of_writing_metrics_capable_modules"><span class="merged" id="all.3VAwd" title="原文 : Advantages of Writing Metrics-capable Modules">メトリクス対応モジュールの記述の利点</span></h3>
<div class="section">
