...
<h2 id="_overview"><span class="merged" id="all.YrpRV.37"  title="原文:: Overview">概要</span></h2>
<div class="section">
<p><span class="merged" id="all.1NgwIa.spl1" title="原文 : Helidon has its own set of reactive operators that have no dependencies outside of the Helidon ecosystem.">Helidonには、Helidonエコシステムの外部に依存関係のない、独自のリアクティブ・オペレータのセットがあります。</span> <span class="merged" id="all.1NgwIa.spl2" title="原文 : These operators can be used with java.util.concurrent.Flow based reactive streams.">これらのオペレータは、<code>java.util.concurrent.Flow</code>ベースのリアクティブ・ストリームで使用できます。</span> </p>
...
<div class="section">
<p><span class="merged" id="all.4ccNwb" title="原文 : To enable Reactive Engine add the following dependency to your project’s pom.xml (see Managing Dependencies).">リアクティブ・エンジンを有効にするには、プロジェクトの<code>pom.xml</code>に次の依存関係を追加します(<router-link to="/about/managing-dependencies">「依存関係の管理」</router-link>を参照)。</span></p>
...
<h2 id="_usage"><span class="merged" id="all.JUOSC.24"  title="原文:: Usage">使用</span></h2>
<div class="section">
<p><span class="merged" id="all.35Vm78" title="原文 : The stream processing operator chain can be easily constructed by io.helidon.common.reactive.Multi, or io.helidon.common.reactive.Single for streams with single value.">ストリーム処理オペレータ・チェーンは、<code>io.helidon.common.reactive.Multi</code>、または単一の値のストリームの場合は<code>io.helidon.common.reactive.Single</code>で簡単に作成できます。</span></p>
...
<tbody>
<tr>
<td class=""><span class="merged" id="all.1P5J0j"  title="原文:: defer">遅延</span></td>
...
<td class=""><span class="merged" id="all.t48dd" title="原文 : Transform each upstream item with the supplied function and flatten the resulting CompletionStage results to downstream.">指定された関数を使用して各アップストリーム・アイテムを変換し、結果の<code>CompletionStage</code>結果をダウンストリームにフラット化します。</span></td>
</tr>
<tr>
...
<tr>
<td class=""><span class="merged" id="all.11TEte"  title="原文: flatMapOptional">flatMapOptional</span></td>
<td class=""><span class="merged" id="all.R5iIk" title="原文 : Transform each upstream item with the supplied function and flatten the resulting Optional to the downstream as item if present.">指定された関数を使用して各アップストリーム項目を変換し、結果の<code>Optional</code>をアイテムとしてダウンストリームにフラット化します(存在する場合)。</span></td>
</tr>
<tr>
<td class=""><span class="merged" id="all.o3RAJ"  title="原文: observeOn">observeOn</span></td>
<td class=""><span class="merged" id="all.4OjMYV" title="原文 : Re-emit the upstream’s signals to the downstream on the given executor’s thread using a default buffer size of 32 and errors skipping ahead of items.">デフォルトのバッファ・サイズである32を使用して、アップストリームのシグナルを指定されたエグゼキュータのスレッドのダウンストリームに再送信し、アイテムの前にエラーをスキップします。</span></td>
</tr>
<tr>
<td class=""><span class="merged" id="all.o3RAJ.1"  title="原文: observeOn">observeOn</span></td>
<td class=""><span class="merged" id="all.48nH4Z" title="原文 : Re-emit the upstream’s signals to the downstream on the given executor’s thread.">アップストリームのシグナルを、指定されたエグゼキュータのスレッドのダウンストリームに再送信します。</span></td>
</tr>
<tr>
<td class=""><span class="merged" id="all.4FFyRb"  title="原文: forEach">forEach</span></td>
<td class=""><span class="merged" id="all.3MR4Xj" title="原文 : Terminal stage, invokes provided consumer for every item in the stream with no backpressure.">ターミナル・ステージ。バック・プレッシャなしでストリーム内のすべてのアイテムに対して提供されたコンシューマを起動します。</span></td>
</tr>
<tr>
<td class=""><span class="merged" id="all.1MATKn"  title="原文: forEachCompletionStage">forEachCompletionStage</span></td>
<td class=""><span class="merged" id="all.2t0eph" title="原文 : Terminal stage, invokes provided function for every item in the stream with strict backpressure, requests another item only when previous operation is finished.">ターミナル・ステージは、厳密なバック・プレッシャでストリーム内のすべての項目に対して指定された関数を起動し、前の操作が終了したときにのみ別の項目をリクエストします。</span></td>
</tr>
<tr>
...
<tr>
<td class=""><span class="merged" id="all.vfpqc"  title="原文: ifEmpty">ifEmpty</span></td>
<td class=""><span class="merged" id="all.1Okd6E" title="原文 : Executes given java.lang.Runnable when stream is finished without value(empty stream).">value(empty stream)なしでストリームが終了したときに、指定された<code>java.lang.Runnable</code>を実行します。</span></td>
</tr>
<tr>
...
<td class=""><span class="merged" id="all.3NRik2" title="原文 : Signals a TimeoutException if the upstream doesn’t signal the next item, error or completion within the specified time.">アップストリームが指定された時間内に次のアイテム、エラーまたは完了を通知しない場合、<code>TimeoutException</code>にシグナルを送信します。</span></td>
</tr>
<tr>
<td class=""><span class="merged" id="all.2Ml17q.1"  title="原文:: timeout">timeout</span></td>
<td class=""><span class="merged" id="all.3TswBN" title="原文 : Switches to a fallback source if the upstream doesn’t signal the next item, error or completion within the specified time.">アップストリームが指定された時間内に次のアイテム、エラーまたは完了を通知しない場合、フォールバック・ソースに切り替えます。</span></td>
</tr>
<tr>
...
<h3 id="_operator_chains_composition"><span class="merged" id="all.2pIv5Y" title="原文 : Operator Chains Composition">オペレータ・チェーン構成</span></h3>
<div class="section">
