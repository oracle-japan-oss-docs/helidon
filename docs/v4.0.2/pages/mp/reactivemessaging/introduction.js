...
<ul class="ulist">
<li>
<p><span class="merged" id="all.1S1Ao6.23" title="原文 : Overview"><router-link @click.native="this.scrollFix('#_overview')" to="#_overview">概要</router-link></span></p>
...
<h2 id="_overview"><span class="merged" id="all.YrpRV.32"  title="原文:: Overview">概要</span></h2>
<div class="section">
...
<p><span class="merged" id="all.3Nhwei.spl1" title="原文 : Reactive messaging uses reactive streams as message channels so you can construct very effective pipelines for working with the messages or, if you prefer, you can continue to use older messaging methods.">リアクティブ・メッセージングでは、リアクティブ・ストリームがメッセージ・チャネルとして使用されるため、メッセージを操作するための非常に効果的なパイプラインを構築したり、必要に応じて、古いメッセージング・メソッドを引き続き使用できます。</span> <span class="merged" id="all.3Nhwei.spl2" title="原文 : Like the message-driven beans, MicroProfile Reactive Messaging uses CDI beans to produce, consume or process messages over Reactive Streams.">メッセージドリブンbeansと同様に、<a href="https://download.eclipse.org/microprofile/microprofile-reactive-messaging-3.0/microprofile-reactive-messaging-spec-3.0.html" target="_blank">「MicroProfileリアクティブ・メッセージング」</a>はCDI beansを使用して、リアクティブ・ストリームを介してメッセージを生成、消費または処理します。</span> <span class="merged" id="all.3Nhwei.spl3" title="原文 : These messaging beans are expected to be either ApplicationScoped or Dependent scoped.">これらのメッセージBeanは、<code>ApplicationScoped</code>または<code>Dependent</code>スコープである必要があります。</span> <span class="merged" id="all.3Nhwei.spl4" title="原文 : Messages are managed by methods annotated by @Incoming and @Outgoing and the invocation is always driven by message core - either at assembly time, or for every message coming from the stream.">メッセージは、<code>@Incoming</code>および<code>@Outgoing</code>によって注釈が付けられたメソッドによって管理され、呼出しは常にメッセージ・コアによって駆動されます - アセンブリ時、またはストリームからのすべてのメッセージに対して。</span> </p>
...
<div class="section">
<p><span class="merged" id="all.2F4QUy" title="原文 : To enable MicroProfile Reactive Messaging add the following dependency to your project’s pom.xml (see Managing Dependencies).">MicroProfileリアクティブ・メッセージングを有効にするには、プロジェクトの<code>pom.xml</code>に次の依存関係を追加します(<router-link to="/about/managing-dependencies">「依存関係の管理」</router-link>を参照)。</span></p>
...
<h2 id="_usage"><span class="merged" id="all.JUOSC.20"  title="原文:: Usage">使用</span></h2>
<div class="section">
<ul class="ulist">
<li>
...
<li>
<p><span class="merged" id="all.29NYvA" title="原文 : Message"><router-link @click.native="this.scrollFix('#_message')" to="#_message">Message</router-link></span></p>

</li>
<li>
<p><span class="merged" id="all.2wzShs" title="原文 : Acknowledgement"><router-link @click.native="this.scrollFix('#_acknowledgement')" to="#_acknowledgement">確認</router-link></span></p>

</li>
<li>
<p><span class="merged" id="all.2jexmN" title="原文 : Health Check"><router-link @click.native="this.scrollFix('#_health_check')" to="#_health_check">ヘルス・チェック</router-link></span></p>
...
<div class="section">
<p><span class="merged" id="all.Lwd6H.spl1" title="原文 : Consuming methods can be connected to the channel’s downstream to consume the message coming through the channel.">コンシューマ・メソッドは、チャネルのダウンストリームに接続して、チャネルを介して送信されるメッセージを消費できます。</span> <span class="merged" id="all.Lwd6H.spl2" title="原文 : The incoming annotation has one required attribute value that defines the channel name.">受信注釈には、チャネル名を定義する1つの必須属性<code>value</code>があります。</span> </p>
...
<li>
<p><span class="merged" id="all.1uTJRf" title="原文 : prepare reactive stream’s subscriber and connect it to the channel - invoked only once during the channel construction">リアクティブ・ストリーム・サブスクライバを準備し、チャネルに接続 - チャネル構築中に1回のみ起動</span></p>
...
<li>
<p><span class="merged" id="all.lNRJn" title="原文 : Flow.Publisher&lt;PAYLOAD&gt; - JDK’s flow publisher with unwrapped payload"><code>Flow.Publisher&lt;PAYLOAD></code> - ラップされていないペイロードを持つJDKフロー・パブリッシャ</span></p>

</li>
<li>
<p><span class="merged" id="all.1liNE6" title="原文 : Flow.Publisher&lt;Message&lt;PAYLOAD&gt;&gt; - JDK’s flow publisher with whole message"><code>Flow.Publisher&lt;Message&lt;PAYLOAD>></code> - メッセージ全体を含むJDKフロー・パブリッシャ</span></p>
...
<div class="section">
<p><span class="merged" id="all.49kdHb" title="原文 : The annotation has one required attribute value that defines the channel name.">注釈には、<a href="https://download.eclipse.org/microprofile/microprofile-reactive-messaging-3.0/microprofile-reactive-messaging-spec-3.0.html#_channel" target="_blank">channel</a>名を定義する必須属性<code>value</code>があります。</span></p>
...
<ul class="ulist">
<li>
<p><span class="merged" id="all.xSBes" title="原文 : produce exactly one message to the stream connected to the channel"><a href="https://download.eclipse.org/microprofile/microprofile-reactive-messaging-3.0/microprofile-reactive-messaging-spec-3.0.html#_channel" target="_blank">channel</a>に接続されたストリームにメッセージを生産</span></p>

</li>
<li>
<p><span class="merged" id="all.1Vsscr" title="原文 : prepare reactive stream’s publisher and connect it to the channel">リアクティブ・ストリームのパブリッシャを準備して<a href="https://download.eclipse.org/microprofile/microprofile-reactive-messaging-3.0/microprofile-reactive-messaging-spec-3.0.html#_channel" target="_blank">channel</a>に接続</span></p>
...
<td class=""><span class="merged" id="all.4JM9z7.16"  title="原文:: Description">説明</span></td>
</tr>
<tr>
...
<div class="section">
<p><span class="merged" id="all.2LZDoN" title="原文 : Such methods acts as processors, consuming messages from one channel and producing to another.">このような<a href="https://download.eclipse.org/microprofile/microprofile-reactive-messaging-3.0/microprofile-reactive-messaging-spec-3.0.html#_method_consuming_and_producing" target="_blank">methods</a>はプロセッサとして機能し、あるチャネルから別のチャネルにメッセージをコンシュームし、別のチャネルにメッセージを生産します。</span></p>
...
<li>
<p><span class="merged" id="all.iIKXH" title="原文 : prepare reactive stream’s processor and connect it between the channels">リアクティブ・ストリームのプロセッサを準備し、チャネル間で接続</span></p>
...
<div class="section">
<p><span class="merged" id="all.rQoUi.spl1" title="原文 : The Reactive Messaging Message class can be used to wrap or unwrap data items between methods and connectors.">リアクティブ・メッセージング<a href="https://download.eclipse.org/microprofile/microprofile-reactive-messaging-3.0/microprofile-reactive-messaging-spec-3.0.html#_message" target="_blank">Message</a>クラスを使用して、メソッドとコネクタ間のデータ・アイテムをラップまたはアン・ラップできます。</span> <span class="merged" id="all.rQoUi.spl2" title="原文 : The message wrapping and unwrapping can be performed explicitly by using org.eclipse.microprofile.reactive.messaging.Message#of(T) or implicitly through the messaging core.">メッセージのラップおよびアン・ラップは、<code>org.eclipse.microprofile.reactive.messaging.Message#of(T)</code>を使用して明示的に実行することも、メッセージング・コアを介して暗黙的に実行することもできます。</span> </p>
...
<h3 id="_acknowledgement"><span class="merged" id="all.2o4NbP"  title="原文:: Acknowledgement">受諾</span></h3>
<div class="section">
<p><span class="merged" id="all.2UnKT7.spl1" title="原文 : Messages carry a callback for reception acknowledgement (ack) and negative acknowledgement (nack).">メッセージには、受信確認(ack)および否定応答(nack)のコールバックがあります。</span> <span class="merged" id="all.2UnKT7.spl2" title="原文 : An acknowledgement in messaging methods is possible manually by org.eclipse.microprofile.reactive.messaging.Message#ack or automatically according explicit or implicit acknowledgement strategy by the messaging core.">メッセージング・メソッドでの確認は、<code>org.eclipse.microprofile.reactive.messaging.Message#ack</code>で手動で行うことも、メッセージ・コアによって明示的または暗黙的な確認戦略に従って自動的に行うこともできます。</span> <span class="merged" id="all.2UnKT7.spl3" title="原文 : Explicit strategy configuration is possible with @Acknowledgment annotation which has one required attribute value that expects the strategy type from enum org.eclipse.microprofile.reactive.messaging.Acknowledgment.Strategy.">明示的な戦略構成は、列挙<code>org.eclipse.microprofile.reactive.messaging.Acknowledgment.Strategy</code>からの戦略タイプを必要とする必須属性<code>value</code>を持つ<code>@Acknowledgment</code>注釈を使用して可能です。</span> <span class="merged" id="all.2UnKT7.spl4" title="原文 : More information about supported signatures and implicit automatic acknowledgement can be found in specification Message acknowledgement.">サポートされているシグネチャおよび暗黙的な自動確認の詳細は、仕様<a href="https://download.eclipse.org/microprofile/microprofile-reactive-messaging-3.0/microprofile-reactive-messaging-spec-3.0.html#_message_acknowledgement" target="_blank">「メッセージ確認」</a>を参照してください。</span> </p>
...
<h3 id="_health_check"><span class="merged" id="all.1G6No9.1"  title="原文:: Health Check">ヘルス・チェック</span></h3>
<div class="section">
...
<p class="admonition-inline"><span class="merged" id="all.6iYH3" title="原文 : Due to the nack support are exceptions thrown in messaging methods NOT translated to error and cancel signals implicitly anymore">ナック・サポートが原因で、メッセージ・メソッドでスローされる例外は、エラーに変換されず、暗黙的にシグナルを取り消します</span></p>
</div>
...
<li data-value="2"><span class="merged" id="all.3SGLBg" title="原文 : Name of the channel requesting publisher as it’s upstream from this connector">このコネクタからアップストリームであるため、パブリッシャをリクエストしているチャネルの名前</span></li>
</ul>
...
<ul class="ulist">
<li>
<p><span class="merged" id="all.4UyAy9" title="原文 : Helidon MicroProfile Reactive Messaging"><a href="https://helidon.io/docs/v4/apidocs/io.helidon.microprofile.messaging/module-summary.html" target="_blank">Helidon MicroProfileリアクティブ・メッセージング</a></span></p>

</li>
<li>
<p><span class="merged" id="all.3moJW9" title="原文 : MicroProfile Reactive Messaging Specification"><a href="https://download.eclipse.org/microprofile/microprofile-reactive-messaging-3.0/microprofile-reactive-messaging-spec-3.0.html" target="_blank">MicroProfileリアクティブ・メッセージング仕様</a></span></p>
