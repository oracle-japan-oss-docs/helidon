...
<ul class="ulist">
<li>
<p><span class="merged" id="all.1S1Ao6.38" title="原文 : Overview"><router-link @click.native="this.scrollFix('#_overview')" to="#_overview">概要</router-link></span></p>
...
<li>
<p><span class="merged" id="all.3grmMy.27" title="原文 : Usage"><router-link @click.native="this.scrollFix('#_usage')" to="#_usage">使用</router-link></span></p>
...
<li>
<p><span class="merged" id="all.2zAIy9.19" title="原文 : Reference"><router-link @click.native="this.scrollFix('#_reference')" to="#_reference">リファレンス</router-link></span></p>
...
<h2 id="_overview"><span class="merged" id="all.YrpRV.54"  title="原文:: Overview">概要</span></h2>
<div class="section">
<p><span class="merged" id="all.BG4bO" title="原文 : Helidon integrates with Tyrus to provide support for the Jakarta WebSocket API.">Helidonは、<a href="https://projects.eclipse.org/projects/ee4j.tyrus" target="_blank">Tyrus</a>と統合して<a href="https://jakarta.ee/specifications/websocket/2.1.0/websocket-spec-2.1.0.html" target="_blank">Jakarta WebSocket API</a>をサポートします。</span></p>
...
<div class="section">
<p><span class="merged" id="all.C1dTM" title="原文 : To enable Jakarta Websocket add the following dependency to your project’s pom.xml (see Managing Dependencies).">Jakarta Websocketを有効にするには、プロジェクトの<code>pom.xml</code>に次の依存関係を追加します(<router-link to="/about/managing-dependencies">「依存関係の管理」</router-link>を参照)。</span></p>
...
<h2 id="_usage"><span class="merged" id="all.JUOSC.34"  title="原文:: Usage">使用</span></h2>
<div class="section">
...
<p><span class="merged" id="all.2TAdNI" title="原文 : For more information see the example.">詳細は、<a href="https://github.com/oracle/helidon/tree/4.0.2/examples/webserver/websocket" target="_blank">example</a>を参照してください。</span></p>


<h3 id="_websocket_endpoints_on_different_ports"><span class="merged" id="all.HuWCB" title="原文 : WebSocket Endpoints on Different Ports">WebSocket異なるポートのエンドポイント</span></h3>
<div class="section">
<p><span class="merged" id="all.acnwv.spl1" title="原文 : The Helidon WebServer can listen on multiple ports or sockets.">Helidon WebServerは、複数のポートまたはソケットでリスニングできます。</span> <span class="merged" id="all.acnwv.spl2" title="原文 : This can be useful when APIs for different type of users need to be exposed (such as admin vs. non-admin users).">これは、様々なタイプのユーザーのAPIを公開する必要がある場合(管理ユーザーと非管理ユーザーなど)に役立ちます。</span> <span class="merged" id="all.acnwv.spl3" title="原文 : Just like for REST resources, it is possible to expose WebSocket applications on different ports provided that the routing paths are different --this is due to a constraint in Tyrus, given that it is simply unaware that the endpoints are bound to different ports in the Helidon WebServer.">RESTリソースの場合と同様に、WebSocketアプリケーションを異なるポート<em>「ルーティング・パスが異なる場合」</em>に公開できます。これは、エンドポイントがHelidon WebServer内の異なるポートにバインドされていることを認識していないため、Tyrusの制約が原因です。</span> </p>

<div class="admonition note">
<p class="admonition-inline"><span class="merged" id="all.Zfd7B.spl1" title="原文 : In practice, this implies that the value of @RoutingPath, or the equivalent entry in config, must be different across sockets to satisfy the restriction in Tyrus.">実際には、これは、Tyrusの制限を満たすために、<code>@RoutingPath</code>の値、またはconfig内の同等のエントリがソケット間で異なる必要があることを意味します。</span> <span class="merged" id="all.Zfd7B.spl2" title="原文 : An attempt to register two or more endpoints on the same path, even if they belong to applications registered on different ports, shall result in a jakarta.websocket.DeploymentException being thrown.">同じパスに2つ以上のエンドポイントを登録しようとすると、そのエンドポイントが別のポートに登録されているアプリケーションに属していても、<code>jakarta.websocket.DeploymentException</code>がスローされます。</span> </p>
</div>

<p><span class="merged" id="all.m406u" title="原文 : We can modify the MessageBoardApplication above and bind it to a non-default socket as follows:">前述の<code>MessageBoardApplication</code>を変更して、次のようにデフォルト以外のソケットにバインドできます:</span></p>
...
<p><span class="merged" id="all.1zKTKP" title="原文 : The value of the @RoutingName annotation must match that of a configured application socket as shown in the following application.yaml file:"><code>@RoutingName</code>注釈の値は、次の<code>application.yaml</code>ファイルに示すように、構成されたアプリケーション・ソケットの値と一致する必要があります:</span></p>
...
<p><span class="merged" id="all.3ohOAr" title="原文 : This example assumes that port 8888 is reserved for admin users and binds the MessageBoardApplication to it.">この例では、ポート8888が管理ユーザー用に予約されており、そのポートに<code>MessageBoardApplication</code>をバインドしていることを前提としています。</span></p>
...
<h2 id="_reference"><span class="merged" id="all.oKAT2.19"  title="原文:: Reference">リファレンス</span></h2>
<div class="section">
<ul class="ulist">
<li>
...
<li>
<p><span class="merged" id="all.1U1BPp" title="原文 : Helidon MicroProfile Tyrus Javadoc"><a href="/apidocs/io.helidon.microprofile.tyrus/module-summary.html" target="_blank">Helidon MicroProfile Tyrus Javadoc</a></span></p>
