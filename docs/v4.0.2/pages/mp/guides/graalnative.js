...
<div class="section">
<p><span class="merged" id="all.1ANIWk.spl1" title="原文 : Native images are ahead-of-time compiled Java code that result in a self contained native executable."><a href="https://www.graalvm.org/jdk21/reference-manual/native-image/" target="_blank">「ネイティブ・イメージ」</a>は、自己完結型のネイティブ実行可能ファイルになるコンパイル済Javaコードです。</span> <span class="merged" id="all.1ANIWk.spl2" title="原文 : When used appropriately native images have dramatically faster startup and lower runtime memory overhead compared to a Java VM.">適切にネイティブ・イメージを使用すると、Java VMに比べて起動が大幅に高速化され、ランタイム・メモリー・オーバーヘッドが削減されます。</span> </p>
...
<tr>
<td class=""><span class="merged" id="all.17vEm3.4" title="原文 : Java&#160;SE&#160;21 (Open&#160;JDK&#160;21)"><a href="https://www.oracle.com/technetwork/java/javase/downloads" target="_blank">Java SE 21</a> (<a href="http://jdk.java.net" target="_blank">JDK 21を開く</a>)</span></td>
<td class=""><span class="merged" id="all.3uVV7D.4" title="原文 : Helidon requires Java 21+.">HelidonにはJava 21+が必要です。</span></td>
</tr>
<tr>
<td class=""><span class="merged" id="all.UWTyd.4" title="原文 : Maven 3.8+"><a href="https://maven.apache.org/download.cgi" target="_blank">Maven 3.8+</a></span></td>
<td class=""><span class="merged" id="all.47RJEf.4" title="原文 : Helidon requires Maven 3.8+.">HelidonにはMaven 3.8+が必要です。</span></td>
</tr>
<tr>
<td class=""><span class="merged" id="all.3ND1mN.4"  title="原文:: Docker 18.09+"><a href="https://docs.docker.com/install/" target="_blank">Docker 18.09+</a></span></td>
...
<tr>
<td class=""><span class="merged" id="all.4Lo77h.4"  title="原文:: Kubectl 1.16.5+"><a href="https://kubernetes.io/docs/tasks/tools/install-kubectl/" target="_blank">Kubectl 1.16.5+</a></span></td>
<td class=""><span class="merged" id="all.1JBSOB.4" title="原文 : If you want to deploy to Kubernetes, you need kubectl and a Kubernetes cluster (you can install one on your desktop.">Kubernetesにデプロイする場合は、<code>kubectl</code>およびKubernetesクラスタが必要です (<router-link to="/about/kubernetes">「デスクトップにインストール」</router-link>できます)。</span></td>
</tr>
<tr>
<td class=""><span class="merged" id="all.4GuGHq" title="原文 : GraalVM for JDK 21"><a href="https://www.graalvm.org/release-notes/JDK_21/" target="_blank">JDK 21の場合はGraalVM</a></span></td>
...
<p><span class="merged" id="all.2gYPJc" title="原文 : Then verify:">次に、次を確認します:</span></p>
...
<p ><span class="merged" id="all.3Q6WM1.spl1" title="原文 : This uses the org.graalvm.buildtools:native-maven-plugin to perform the native compilation using your installed copy of GraalVM.">これは、<code>org.graalvm.buildtools:native-maven-plugin</code>を使用し、インストールしたGraalVMのコピーを使用してネイティブ・コンパイルを実行します。</span> <span class="merged" id="all.3Q6WM1.spl2" title="原文 : It might take a while to complete.">完了するまでに時間がかかる場合があります。</span> </p>
</div>
...
<p><span class="merged" id="all.2NHOSR.spl1" title="原文 : Yep, it starts fast.">はい、すぐに開始します。</span> <span class="merged" id="all.2NHOSR.spl2" title="原文 : You can exercise the application’s endpoints as before.">以前と同様に、アプリケーションのエンドポイントを実行できます。</span> </p>
...
<p><span class="merged" id="all.RH4Mw.spl1" title="原文 : Again, it starts fast.">ここでも、高速で起動します。</span> <span class="merged" id="all.RH4Mw.spl2" title="原文 : You can exercise the application’s endpoints as before.">以前と同様に、アプリケーションのエンドポイントを実行できます。</span> </p>
...
<p class="admonition-inline"><span class="merged" id="all.1FDiCj" title="原文 : When building Helidon using native-image, we check features on classpath, and warn if there is a problem or restriction of support">ネイティブ・イメージを使用してHelidonを構築する場合、クラスパス上の機能を確認し、サポートに問題や制限があるかどうかを警告</span></p>
</div>
