...
<tbody>
<tr>
<td class=""><span class="merged" id="all.3J4a7U.9" title="原文 : A Helidon MP Application">Helidon MPアプリケーション</span></td>
<td class=""><span class="merged" id="all.13muNj.9" title="原文 : You can use your own application or use the Helidon MP Quickstart to create a sample application.">独自のアプリケーションを使用するか、<router-link to="/mp/guides/quickstart">「Helidon MPクイックスタート」</router-link>を使用してサンプル・アプリケーションを作成できます。</span></td>
</tr>
<tr>
<td class=""><span class="merged" id="all.17vEm3.12" title="原文 : Java&#160;SE&#160;21 (Open&#160;JDK&#160;21)"><a href="https://www.oracle.com/technetwork/java/javase/downloads" target="_blank">Java SE 21</a> (<a href="http://jdk.java.net" target="_blank">JDK 21を開く</a>)</span></td>
<td class=""><span class="merged" id="all.3uVV7D.12" title="原文 : Helidon requires Java 21+.">HelidonにはJava 21+が必要です。</span></td>
</tr>
<tr>
<td class=""><span class="merged" id="all.UWTyd.12" title="原文 : Maven 3.8+"><a href="https://maven.apache.org/download.cgi" target="_blank">Maven 3.8+</a></span></td>
<td class=""><span class="merged" id="all.47RJEf.12" title="原文 : Helidon requires Maven 3.8+.">HelidonにはMaven 3.8+が必要です。</span></td>
</tr>
<tr>
<td class=""><span class="merged" id="all.3ND1mN.12"  title="原文:: Docker 18.09+"><a href="https://docs.docker.com/install/" target="_blank">Docker 18.09+</a></span></td>
<td class=""><span class="merged" id="all.aX1BT.12" title="原文 : You need Docker if you want to build and deploy Docker containers.">Dockerコンテナをビルドおよびデプロイする場合は、Dockerが必要です。</span></td>
</tr>
<tr>
<td class=""><span class="merged" id="all.4Lo77h.12"  title="原文:: Kubectl 1.16.5+"><a href="https://kubernetes.io/docs/tasks/tools/install-kubectl/" target="_blank">Kubectl 1.16.5+</a></span></td>
<td class=""><span class="merged" id="all.1JBSOB.12" title="原文 : If you want to deploy to Kubernetes, you need kubectl and a Kubernetes cluster (you can install one on your desktop.">Kubernetesにデプロイする場合は、<code>kubectl</code>およびKubernetesクラスタが必要です (<router-link to="/about/kubernetes">「デスクトップにインストール」</router-link>できます)。</span></td>
</tr>
</tbody>
</table>
</div>
...
<h3 id="_on_docker"><span class="merged" id="all.36l17N" title="原文 : On Docker">Dockerの場合</span></h3>
<div class="section">
...
<h3 id="_on_jdk"><span class="merged" id="all.3YAmaV" title="原文 : On JDK">JDKの場合</span></h3>
<div class="section">
<p><span class="merged" id="all.DbOGr.spl1" title="原文 : Download the last version of Keycloak from Keycloak website : https://www.keycloak.org/downloads In the table Server choose Standalone server distribution.">Keycloak webサイトから最後のバージョンのKeycloakをダウンロード : <a class="bare" href="https://www.keycloak.org/downloads" target="_blank">https://www.keycloak.org/downloads</a>表のサーバーで、スタンドアロン・サーバー・ディストリビューションを選択します。</span> <span class="merged" id="all.DbOGr.spl2" title="原文 : ZIP or Tar format are available, click on either to download Keycloak.">ZIPまたはTar形式が使用可能です。いずれかをクリックしてKeycloakをダウンロードしてください。</span> </p>
...
<h2 id="_start_keycloak"><span class="merged" id="all.1Wa2Zu" title="原文 : Start Keycloak">Keycloakの起動</span></h2>
<div class="section">
...
<div class="section">
<p><span class="merged" id="all.3mLZYh.spl1" title="原文 : You need to create an admin user because it does not come by default when installing Keycloak.">管理ユーザーは、Keycloakのインストール時にデフォルトでは提供されないため、作成する必要があります。</span> <span class="merged" id="all.3mLZYh.spl2" title="原文 : To do this, open http://localhost:8080/auth in your favorite browser.">これを行うには、お気に入りのブラウザで<a class="bare" href="http://localhost:8080/auth" target="_blank">http://localhost:8080/auth</a>を開きます。</span> </p>
...
<div class="section">
<p><span class="merged" id="all.3slDsp" title="原文 : To set up Keycloak properly, go to the admin console: http://localhost:8080/auth/admin">Keycloakを正しく設定するには、管理コンソールに移動 : <a class="bare" href="http://localhost:8080/auth/admin" target="_blank">http://localhost:8080/auth/admin</a></span></p>
...
<li>
<span class="merged" id="all.2Lp4ms" title="原文 : Open Keycloak admin console http://localhost:8080/auth/admin.">Keycloak管理コンソール<a class="bare" href="http://localhost:8080/auth/admin" target="_blank">http://localhost:8080/auth/admin</a>を開きます。</span>
...
<ol style="margin-left: 15px;">
<li>
<span class="merged" id="all.4RPgHl" title="原文 : Open the Keycloak admin console: http://localhost:8080/auth/admin">Keycloak管理コンソールを開く : <a class="bare" href="http://localhost:8080/auth/admin" target="_blank">http://localhost:8080/auth/admin</a></span>
...
<li>
<span class="merged" id="all.3axGYC" title="原文 : Click Save"><code>Save</code>をクリック</span>
...
<ol style="margin-left: 15px;">
<li>
<span class="merged" id="all.BrGVX" title="原文 : Open the Keycloak account console: http://localhost:8080/auth/realms/myRealm/account.">Keycloakアカウント・コンソールを開く : <a class="bare" href="http://localhost:8080/auth/realms/myRealm/account" target="_blank">http://localhost:8080/auth/realms/myRealm/account</a>。</span>
...
<ol style="margin-left: 15px;">
<li>
<span class="merged" id="all.Vm4eb" title="原文 : Open the Keycloak admin console: http://localhost:8080/auth/admin.">Keycloak管理コンソールを開く : <a class="bare" href="http://localhost:8080/auth/admin" target="_blank">http://localhost:8080/auth/admin</a>。</span>
...
<li>
<span class="merged" id="all.4Y7MZT" title="原文 : Update Valid Redirect URIs : http://localhost:7987/*"><code>Valid Redirect URIs</code>の更新 : <a class="bare" href="http://localhost:7987/*" target="_blank">http://localhost:7987/*</a></span>
...
<div class="section">
<p><span class="merged" id="all.2yv5GV.spl1" title="原文 : Use the Helidon MP Maven archetype to create a simple project.">Helidon MP Maven原型を使用して、単純なプロジェクトを作成します。</span> <span class="merged" id="all.2yv5GV.spl2" title="原文 : It will be used as an example to show how to set up Helidon.">Helidonの設定方法を示す例として使用されます。</span> <span class="merged" id="all.2yv5GV.spl3" title="原文 : Replace 4.0.2 by the latest helidon version."><code>4.0.2</code>を最新のhelidonバージョンに置き換えます。</span> <span class="merged" id="all.2yv5GV.spl4" title="原文 : It will download the quickstart project into the current directory.">クイックスタート・プロジェクトが現在のディレクトリにダウンロードされます。</span> </p>
...
<p><span class="merged" id="all.20EsxI" title="原文 : It is also needed to add the following line to the module-info.java file:">また、次の行を<code>module-info.java</code>ファイルに追加する必要があります:</span></p>

<markup
lang="java"
title="Add security annotations module requirement"
>requires io.helidon.security.annotations;</markup>

<p><span class="merged" id="all.3Y720w.spl1" title="原文 : When a client will send an HTTP GET request at the endpoint http://localhost:7987/greet, he will be redirected to keycloak.">クライアントがエンドポイント<code><a class="bare" href="http://localhost:7987/greet" target="_blank">http://localhost:7987/greet</a></code>でHTTP GETリクエストを送信すると、keycloakにリダイレクトされます。</span> <span class="merged" id="all.3Y720w.spl2" title="原文 : Keycloak will check if the client has the required authorisation to access this endpoint.">Keycloakは、このエンドポイントへのアクセスに必要な認可がクライアントにあるかどうかを確認します。</span> <span class="merged" id="all.3Y720w.spl3" title="原文 : If the client can log in successfully, keycloak redirect it to the wished endpoint.">クライアントが正常にログインできる場合、keycloakはそれを目的のエンドポイントにリダイレクトします。</span> <span class="merged" id="all.3Y720w.spl4" title="原文 : If the client cannot log in, or the required access data are incomplete, Keycloak refuses the access.">クライアントがログインできない場合、または必要なアクセス・データが不完全な場合、Keycloakはアクセスを拒否します。</span> </p>
...
<ol style="margin-left: 15px;">
<li>
<span class="merged" id="all.7Xc48" title="原文 : Open your favourite browser and try to access http://localhost:7987/greet/Michael.">お気に入りのブラウザを開き、<code><a class="bare" href="http://localhost:7987/greet/Michael" target="_blank">http://localhost:7987/greet/Michael</a></code>にアクセスしてみます。</span>
...
<li>
<span class="merged" id="all.4ZpsMN" title="原文 : Enter the following into URL : http://localhost:7987/greet.">URLに次のように入力 : <code><a class="bare" href="http://localhost:7987/greet" target="_blank">http://localhost:7987/greet</a></code>。</span>
...
<li>
<span class="merged" id="all.1Dqs7b" title="原文 : After successful log in, keycloak redirect you to the http://localhost:7987/greet endpoint and print Hello word.">ログインに成功すると、keycloakによって<code><a class="bare" href="http://localhost:7987/greet" target="_blank">http://localhost:7987/greet</a></code>エンドポイントにリダイレクトされ、Helloワードが出力されます。</span>
...
<p><span class="merged" id="all.1pUIp1" title="原文 : In the MainTest.java file, tests need to be modified to pass the application security when accessing /greet path with a GET method."><code>MainTest.java</code>ファイルで、<code>GET</code>メソッドを使用して<code>/greet</code>パスにアクセスするときに、アプリケーション・セキュリティに合格するようにテストを変更する必要があります。</span></p>

<p><span class="merged" id="all.4gRBpU.spl1" title="原文 : The server has now one security provider, basic authentication configured.">サーバーには、基本認証が構成された1つのセキュリティ・プロバイダがあります。</span> <span class="merged" id="all.4gRBpU.spl2" title="原文 : Next step is to create the test to check that the application is correctly protected.">次のステップでは、アプリケーションが正しく保護されていることを確認するためのテストを作成します。</span> </p>

<p><span class="merged" id="all.w7GUQ" title="原文 : Firstly, create new test method testHelloWorld">まず、新しいテスト・メソッド<code>testHelloWorld</code>を作成</span></p>

<markup
lang="java"
title="Add this method to the test class"
>@Test
void testHellowWorld() {

}</markup>

<p><span class="merged" id="all.1ow4TB" title="原文 : Now we can add the first test:">これで、最初のテストを追加できます:</span></p>

<markup
lang="java"
title="Add this code into <code>testHelloWorld</code> method:"
...
<p><span class="merged" id="all.2Y6hlG.spl1" title="原文 : This piece of code uses the JAX-RS client to access the application on /greet path with a GET method.">このコードでは、JAX-RSクライアントを使用して、<code>GET</code>メソッドを使用して<code>/greet</code>パス上のアプリケーションにアクセスします。</span> <span class="merged" id="all.2Y6hlG.spl2" title="原文 : The http basic authentication security provider protects this path, so the client should receive an HTTP 401 code for unauthorized.">http basic認証セキュリティ・プロバイダはこのパスを保護するため、クライアントは権限のないHTTP 401コードを受信する必要があります。</span> </p>
...
<markup
lang="java"
title="Change the testHelloWorld method:"
...
Message jsonMessage = target
        ...
        .get(Message.class);
assertThat(jsonMessage.getMessage(), is("Hello World!"));</markup>
...
<ol style="margin-left: 15px;">
<li>
<span class="merged" id="all.9TTcI" title="原文 : Open the Keycloak Console.">Keycloakコンソールを開きます。</span>
...
<p><span class="merged" id="all.1yLlsJ.spl1" title="原文 : For the first step, paste the following URL into your browser: http://localhost:8080/auth/realms/myRealm/protocol/openid-connect/auth?client_id=myClientID&amp;response_type=code.">最初のステップでは、次のURLをブラウザに貼り付けます : <code><a class="bare" href="http://localhost:8080/auth/realms/myRealm/protocol/openid-connect/auth?client_id=myClientID&amp;response_type=code" target="_blank">http://localhost:8080/auth/realms/myRealm/protocol/openid-connect/auth?client_id=myClientID&amp;response_type=code</a></code>。</span> <span class="merged" id="all.1yLlsJ.spl2" title="原文 : The first part of the url http:/../auth is the Keycloak endpoint to request an authorization code.">URL <code>http:/../auth</code>の最初の部分は、認可コードをリクエストするKeycloakエンドポイントです。</span> <span class="merged" id="all.1yLlsJ.spl3" title="原文 : Two query parameters are provided, the client id and the response type.">クライアントIDとレスポンス・タイプの2つの問合せパラメータが用意されています。</span> <span class="merged" id="all.1yLlsJ.spl4" title="原文 : Press enter and Keycloak responds with different URL containing a query parameter code.">Enterキーを押すと、問合せパラメータ<code>code</code>を含む別のURLでKeycloakが応答します。</span> <span class="merged" id="all.1yLlsJ.spl5" title="原文 : You successfully received the authorization code.">認可コードが正常に受信されました。</span> </p>

<p><span class="merged" id="all.vTCRi.spl1" title="原文 : In order to achieve the third step, we can use Postman to exchange the authorization code for tokens.">3番目のステップを達成するために、Postmanを使用してトークンに対する認可コードを交換できます。</span> <span class="merged" id="all.vTCRi.spl2" title="原文 : In Postman, select the Http POST method.">Postmanで、HTTP POSTメソッドを選択します。</span> <span class="merged" id="all.vTCRi.spl3" title="原文 : Keycloak endpoint to get token is the following: http://localhost:8080/auth/realms/myRealm/protocol/openid-connect/token.">トークンを取得するKeycloakエンドポイントは次のとおりです : <code><a class="bare" href="http://localhost:8080/auth/realms/myRealm/protocol/openid-connect/token" target="_blank">http://localhost:8080/auth/realms/myRealm/protocol/openid-connect/token</a></code>。</span> <span class="merged" id="all.vTCRi.spl4" title="原文 : In the body of the request, select x-www-form-urlencoded type.">リクエストの本文で、<code>x-www-form-urlencoded</code>タイプを選択します。</span> <span class="merged" id="all.vTCRi.spl5" title="原文 : Add the following data:">次のデータを追加します:</span> </p>
...
<div class="section">
<p><span class="merged" id="all.ewsoF.spl1" title="原文 : The Direct Access Grants flow is used by REST clients that want to request tokens on behalf of a user.">直接アクセス付与フローは、ユーザーにかわってトークンをリクエストするRESTクライアントで使用されます。</span> <span class="merged" id="all.ewsoF.spl2" title="原文 : To use Postman to make this request on behalf of myuser, select the GET method and enter this URL: http://localhost:7987/greet/.">Postmanを使用して<code>myuser</code>のかわりにこのリクエストを実行するには、GETメソッドを選択し、このURLを入力 : <code><a class="bare" href="http://localhost:7987/greet/" target="_blank">http://localhost:7987/greet/</a></code>。</span> <span class="merged" id="all.ewsoF.spl3" title="原文 : Under Authorization tab, select authorization type`OAuth 2.0`."><code>Authorization</code>タブで、認可タイプ`OAuth 2.0`を選択します。</span> <span class="merged" id="all.ewsoF.spl4" title="原文 : Under it, complete the sentence Add authorization data to with Request Headers, and complete the required fields.">下で、文<code>Add authorization data to</code>に<code>Request Headers</code>を入力し、必要なフィールドに入力します。</span> </p>
...
>import jakarta.annotation.security.RolesAllowed;</markup>
...
<p><span class="merged" id="all.3nSjjj.spl1" title="原文 : Now, only Jack has access to secure endpoint as he has an admin role.">Jackのみが<code>admin</code>ロールを持つため、セキュアなエンドポイントにアクセスできるようになりました。</span> <span class="merged" id="all.3nSjjj.spl2" title="原文 : John, as a simple user, can not access it.">ジョンは単純なユーザーなので、アクセスできません。</span> <span class="merged" id="all.3nSjjj.spl3" title="原文 : Once it is done, go to the tests to check the application behavior.">完了したら、テストに移動してアプリケーションの動作を確認します。</span> <span class="merged" id="all.3nSjjj.spl4" title="原文 : The test from previous section is still passing because jack has access.">jackにアクセス権があるため、前のセクションのテストに合格しています。</span> </p>
