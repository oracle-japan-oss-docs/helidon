...
<h2 id="_contents"><span class="merged" id="all.1Q34Zl.43"  title="原文:: Contents">目次</span></h2>
<div class="section">
<ul class="ulist">
<li>
...
<div class="section">
<p><span class="merged" id="all.Ytsx0.spl1" title="原文 : The cross-origin resource sharing (CORS) protocol helps developers control if and how REST resources served by their applications can be shared across origins."><a href="https://www.w3.org/TR/cors" target="_blank">「クロス・オリジン・リソース共有(CORS)プロトコル」</a>は、開発者がアプリケーションによって提供されるRESTリソースをオリジン間で共有できるかどうかおよび方法を制御するのに役立ちます。</span> <span class="merged" id="all.Ytsx0.spl2" title="原文 : Helidon SE includes an implementation of CORS that you can use to add CORS behavior to the services you develop.">Helidon SEには、開発するサービスにCORSの動作を追加するために使用できるCORSの実装が含まれています。</span> <span class="merged" id="all.Ytsx0.spl3" title="原文 : You can define your application&rsquo;s CORS behavior programmatically using the Helidon CORS API alone, or together with configuration.">アプリケーションのCORS動作は、Helidon CORS APIのみを使用してプログラムで定義することも、構成とともに定義することもできます。</span> </p>
...
<div class="section">
<p><span class="merged" id="all.XVNqn.1" title="原文 : To enable CORS add the following dependency to your project&rsquo;s pom.xml (see Managing Dependencies).">CORSを有効にするには、プロジェクトの<code>pom.xml</code>に次の依存関係を追加します(<router-link to="/about/managing-dependencies">「依存関係の管理」</router-link>を参照)。</span></p>
...
<li>
<span class="merged" id="all.1HAVF8" title="原文 : Create a CorsSupport instance corresponding to the resource.">リソースに対応する<a href="./apidocs/io.helidon.webserver.cors/io/helidon/webserver/cors/CorsSupport.html" target="_blank"><code>CorsSupport</code></a>インスタンスを作成します。</span>
...
<ol style="margin-left: 15px;">
<li>
<span class="merged" id="all.BprIQ.spl1" title="原文 : Create a CrossOriginConfig instance."><a href="./apidocs/io.helidon.webserver.cors/io/helidon/webserver/cors/CrossOriginConfig.html" target="_blank"><code>CrossOriginConfig</code></a>インスタンスを作成します。</span> <span class="merged" id="all.BprIQ.spl2" title="原文 :  The CrossOriginConfig Java class represents the details for a particular type of sharing, such as which origins are allowed to share via which HTTP methods, etc."><br><code>CrossOriginConfig</code> Javaクラスは、特定のタイプの共有の詳細(どのオリジンがどのHTTPメソッドを介して共有できるかなど)を表します。</span> 
...
<th><span class="merged" id="all.2ZwVVB.105"  title="原文:: type">type</span></th>
<th><span class="merged" id="all.4A9clD.1"  title="原文:: default">default</span></th>
<th><span class="merged" id="all.20ls8M.113"  title="原文:: description">説明</span></th>
...
<td class=""><span class="merged" id="all.2XHIO8.159"  title="原文:: boolean">boolean</span></td>
...
<td class=""><span class="merged" id="all.15f2Ma.40"  title="原文:: long">long</span></td>
...
<td class=""><span class="merged" id="all.2XHIO8.160"  title="原文:: boolean">boolean</span></td>
...
<div class="section">
<p><span class="merged" id="all.3WdyeS" title="原文 : The Helidon SE Quickstart application lets you change the greeting by sending a PUT request to the /greet/greeting resource."><a href="https://github.com/oracle/helidon/tree/master/examples/quickstarts/helidon-quickstart-se" target="_blank">「Helidon SE Quickstartアプリケーション」</a>では、<code>PUT</code>リクエストを<code>/greet/greeting</code>リソースに送信することで、応答メッセージを変更できます。</span></p>
...
<div class="section">
<p><span class="merged" id="all.14yctW.spl1" title="原文 : Support in Helidon for CORS configuration uses two closely-related cross-origin configuration formats: basic and mapped.">CORS構成に対するHelidonでのサポートでは、密接に関連する複数のクロス・オリジン構成形式が使用されます: basicおよびmapped。</span> <span class="merged" id="all.14yctW.spl2" title="原文 : Each format corresponds to a class in the Helidon CORS library.">各形式は、Helidon CORSライブラリのクラスに対応します。</span> <span class="merged" id="all.14yctW.spl3" title="原文 : The basic format corresponds to the CrossOriginConfig class, and the mapped format corresponds to the MappedCrossOriginConfig class.">基本フォーマットは<a href="./apidocs/io.helidon.webserver.cors/io/helidon/webserver/cors/CrossOriginConfig.html" target="_blank"><code>CrossOriginConfig</code></a>クラスに対応し、マップされたフォーマットは<a href="./apidocs/io.helidon.webserver.cors/io/helidon/webserver/cors/MappedCrossOriginConfig.html" target="_blank"><code>MappedCrossOriginConfig</code></a>クラスに対応します。</span> </p>
...
<p><span class="merged" id="all.18kKyd.1" title="原文 : Path patterns can be any expression accepted by the PathMatcher class.">パス・パターンには、<a href="./apidocs/io.helidon.webserver/io/helidon/webserver/PathMatcher.html" target="_blank"><code>PathMatcher</code></a>クラスで使用できる任意の式を指定できます。</span></p>
...
<div class="section">
<p><span class="merged" id="all.PWXal" title="原文 : For a complete example, see Helidon SE CORS Example.">完全な例については、<a href="https://github.com/oracle/helidon/tree/master/examples/cors" target="_blank">「Helidon SE CORSの例」</a>を参照してください。</span></p>
...
<div class="section">
<p><span class="merged" id="all.4QOCBh.spl1" title="原文 : Several built-in Helidon services-​health, metrics, and OpenAPI--have integrated CORS support.">組込みのHelidonサービス(<router-link to="/se/health">health</router-link>、<router-link to="/se/metrics/metrics">metrics</router-link>および<router-link to="/se/openapi">OpenAPI</router-link>)には、CORSサポートが統合されています。</span> <span class="merged" id="all.4QOCBh.spl2" title="原文 : You can include these services in your application and control how those resources can be shared across origins.">これらのサービスをアプリケーションに組み込み、これらのリソースをオリジン間で共有する方法を制御できます。</span> </p>
...
<tr>
<td class=""><span class="merged" id="all.LiTJt" title="原文 : OpenAPI"><router-link to="/se/openapi">OpenAPI</router-link></span></td>
...
<p><span class="merged" id="all.3NmGW4" title="原文 : The Helidon SE QuickStart example uses these services, so you can use that as a template for your own application, or use the example project itself to experiment with customizing the CORS behavior in the built-in services."><a href="https://github.com/oracle/helidon/tree/master/examples/quickstarts/helidon-quickstart-se" target="_blank">「Helidon SE QuickStartの例」</a>ではこれらのサービスが使用されるため、独自のアプリケーションのテンプレートとして使用することも、サンプル・プロジェクト自体を使用して組込みサービスのCORS動作のカスタマイズを試すこともできます。</span></p>
...
<p><span class="merged" id="all.QNDNc" title="原文 : The following excerpt shows changes to the Helidon SE QuickStart example which limit sharing of the /metrics endpoint to http://foo.com.">次の抜粋は、<code>http://foo.com</code>への<code>/metrics</code>エンドポイントの共有を制限する<a href="https://github.com/oracle/helidon/tree/master/examples/quickstarts/helidon-quickstart-se" target="_blank">「Helidon SE QuickStartの例」</a>に対する変更を示しています。</span></p>
