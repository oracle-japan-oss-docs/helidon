...
<dd slot="desc"><p><span class="merged" id="all.cx9QW" title="原文 : This guide describes how to create a sample Helidon {h1-prefix} project that can be used to run some basic examples using both built-in and custom metrics with Helidon.">このガイドでは、Helidonで組込みメトリクスとカスタム・メトリクスの両方を使用して、いくつかの基本的な例を実行するために使用できるサンプルのHelidon {h1-prefix} プロジェクトを作成する方法について説明します。</span></p>
</dd>
</dl>
</v-card-text>
</v-card>
...
<tr>
<td class=""><span class="merged" id="all.2sRt9f.19" title="原文 : Java&#160;SE&#160;17 (Open&#160;JDK&#160;17)"><a href="https://www.oracle.com/technetwork/java/javase/downloads" target="_blank">Java&#160;SE&#160;17</a> (<a href="http://jdk.java.net" target="_blank">Open&#160;JDK&#160;17</a>)</span></td>
<td class=""><span class="merged" id="all.3aBChT.19" title="原文 : Helidon requires Java 17+.">HelidonにはJava 17以上が必要です。</span></td>
</tr>
<tr>
<td class=""><span class="merged" id="all.1DYqqk.20" title="原文 : Maven 3.6.1+"><a href="https://maven.apache.org/download.cgi" target="_blank">Maven 3.6.1+</a></span></td>
<td class=""><span class="merged" id="all.3woDk0.19" title="原文 : Helidon requires Maven 3.6.1+.">Helidonには、Maven 3.6.1+が必要です。</span></td>
</tr>
<tr>
<td class=""><span class="merged" id="all.3ND1mN.19"  title="原文:: Docker 18.09+"><a href="https://docs.docker.com/install/" target="_blank">Docker 18.09+</a></span></td>
...
<tr>
<td class=""><span class="merged" id="all.4Lo77h.19"  title="原文:: Kubectl 1.16.5+"><a href="https://kubernetes.io/docs/tasks/tools/install-kubectl/" target="_blank">Kubectl 1.16.5+</a></span></td>
<td class=""><span class="merged" id="all.1JBSOB.19" title="原文 : If you want to deploy to Kubernetes, you need kubectl and a Kubernetes cluster (you can install one on your desktop.">Kubernetesにデプロイする場合は、<code>kubectl</code>およびKubernetesクラスタが必要です (<router-link to="/about/kubernetes">「デスクトップにインストール」</router-link>できます)。</span></td>
</tr>
<tr>
...
<div class="admonition note">
<p class="admonition-inline"><span class="merged" id="all.g31Oi.1.spl1" title="原文 : You cannot get the individual fields of a metric.">メトリックの個々のフィールドは取得できません。</span> <span class="merged" id="all.g31Oi.1.spl2" title="原文 : For example, you cannot target http://localhost:8080/metrics/vendor/requests.meter.count.">たとえば、<a class="bare" href="http://localhost:8080/metrics/vendor/requests.meter.count" target="_blank">http://localhost:8080/metrics/vendor/requests.meter.count</a>はターゲットにできません。</span> </p>
</div>
...
<ul class="ulist">
<li>
<p><span class="merged" id="all.3cthqo.1" title="原文 : Helidon thread pools ">Helidonスレッド・プール<br></span>
</p>
<p><span class="merged" id="all.UTEUN.1.spl1" title="原文 : Helidon uses these thread pools for its own internal work, and your application can also use Helidon-managed thread pools if it needs to do work asynchronously.">Helidonは、独自の内部作業にこれらのスレッド・プールを使用し、非同期で作業する必要がある場合は、アプリケーションでHelidon-managedスレッド・プールを使用することもできます。</span> <span class="merged" id="all.UTEUN.1.spl2" title="原文 : (See this example.)">(<a href="https://github.com/oracle/helidon/tree/master/examples/webserver/threadpool" target="_blank">「この例」</a>を参照。)</span> <span class="merged" id="all.UTEUN.1.spl3" title="原文 : The metrics in this group show information about the thread pools which can help you assess how efficiently they are utilized.">このグループのメトリクスは、スレッド・プールに関する情報を表示します。スレッド・プールは、その使用効率を評価するのに役立ちます。</span> <span class="merged" id="all.UTEUN.1.spl4" title="原文 : Helidon uses tags to distinguish the metrics which describe different thread pools.">Helidonは、タグを使用して、様々なスレッド・プールを記述するメトリクスを区別します。</span> <span class="merged" id="all.UTEUN.1.spl5" title="原文 : In some cases the specific metrics exposed depend on the particular type of thread pool.">場合によっては、表示される特定のメトリクスは、特定のタイプのスレッド・プールによって異なります。</span> </p>

</li>
<li>
<p><span class="merged" id="all.2ATFij.1" title="原文 : basic key performance indicators ">基本的なキー・パフォーマンス・インジケータ<br></span>
</p>
...
<ul class="colist">
<li data-value="1"><span class="merged" id="all.REKjJ" title="原文 : Create a MetricsSettings instance (via its Builder) with the metrics subsystem disabled.">メトリクス・サブシステムが無効になっている<a href="./apidocs/io.helidon.metrics.api/io/helidon/metrics/api/MetricsSettings.html" target="_blank"><code>MetricsSettings</a></code>インスタンス(<a href="./apidocs/io.helidon.metrics.api/io/helidon/metrics/api/MetricsSettings.Builder.html" target="_blank"><code>Builder</code></a>を介して)を作成します。</span></li>
<li data-value="2"><span class="merged" id="all.3PzJ8O" title="原文 : Get a MetricsSupport service (usable in setting routing rules) that responds to the /metrics endpoint with 404 and an explanatory message."><code>404</code>および説明メッセージを含む<code>/metrics</code>エンドポイントに応答する<a href="./apidocs/io.helidon.metrics.serviceapi/io/helidon/metrics/serviceapi/MetricsSupport.html" target="_blank"><code>MetricsSupport</a></code>サービス(ルーティング・ルールの設定で使用可能)を取得します。</span></li>
<li data-value="3"><span class="merged" id="all.37XKVg" title="原文 : Get a RegistryFactory instance that provides MetricRegistry instances which register no-op metric objects (counters, timers, etc.).">no-opメトリック・オブジェクト(カウンタ、タイマーなど)を登録する<code>MetricRegistry</code>インスタンスを提供する<a href="./apidocs/io.helidon.metrics.api/io/helidon/metrics/api/RegistryFactory.html" target="_blank"><code>RegistryFactory</a></code>インスタンスを取得します。</span></li>
</ul>
...
<ul class="colist">
<li data-value="1"><span class="merged" id="all.2n8f2U" title="原文 : Create a ComponentMetricsSettings instance (via its Builder) indicating that metrics usage should be disabled.">メトリクスの使用を無効にすることを示す<a href="./apidocs/io.helidon.metrics.api/io/helidon/metrics/api/ComponentMetricsSettings.html" target="_blank"><code>ComponentMetricsSettings</a></code>インスタンス(<a href="./apidocs/io.helidon.metrics.api/io/helidon/metrics/api/ComponentMetricsSettings.Builder.html" target="_blank"><code>Builder</code></a>を介して)を作成します。</span></li>
...
<div class="section">
<p><span class="merged" id="all.3ZRsGn.2.spl1" title="原文 : You can be even more selective.">もっと選択的になることができます。</span> <span class="merged" id="all.3ZRsGn.2.spl2" title="原文 : Within a registry type you can configure up to two regular expression patterns:">レジストリ・タイプ内では、最大2つの正規表現パターンを構成できます:</span> </p>

<ul class="ulist">
<li>
<p><span class="merged" id="all.1zsYLp.2" title="原文 : one matching metric names to exclude, and"><em>「除外」</em>に一致する1つのメトリック名、および</span></p>

</li>
<li>
<p><span class="merged" id="all.3Aies9.2" title="原文 : one matching metric names to include."><em>「含める」</em>一致する1つのメトリック名。</span></p>

</li>
</ul>

<p><span class="merged" id="all.1ZlNQz.2" title="原文 : Helidon updates and reports a metric only if two conditions hold:">Helidonは、2つの条件が保持されている場合にのみメトリックを更新およびレポートします:</span></p>

<ul class="ulist">
<li>
<p><span class="merged" id="all.4G7F05.2" title="原文 : the metric name does not match the exclude regex pattern (if you define one), and">メトリック名は、<code>exclude</code>正規表現パターン(定義した場合)と一致<em>「しません」</em>、および</span></p>

</li>
<li>
<p><span class="merged" id="all.2EEvyx.2"  title="原文:: either">また</span></p>
<ul class="ulist">
<li>
<p><span class="merged" id="all.3zYc4V.2" title="原文 : there is no include regex pattern, or"><code>include</code>正規表現パターンがないか、</span></p>

</li>
<li>
<p><span class="merged" id="all.2WKtx9.2" title="原文 : the metric name matches the include pattern.">メトリック名は<code>include</code>パターンと一致します。</span></p>
...
<p ><p><span class="merged" id="all.3RoVe3.2" title="原文 : Make sure any include regex pattern you specify matches all the metric names you want to capture.">指定した<code>include</code>正規表現パターンが、取得するメトリック名<em>「すべて」</em>と一致していることを確認します。</span></p>
</p>
</div>

<p><span class="merged" id="all.2vtS5v.2" title="原文 : Suppose your application creates and updates a group of metrics with names such as myapp.xxx.queries, myapp.xxx.creates, myapp.xxx.updates, and myapp.xxx.deletes where xxx can be either supplier or customer.">アプリケーションで、<code>myapp.xxx.queries</code>, <code>myapp.xxx.creates</code>, <code>myapp.xxx.updates</code>などの名前を持つメトリクスのグループを作成および更新し、<code>myapp.xxx.deletes</code> (<code>xxx</code>は<code>supplier</code>または<code>customer</code>のいずれか)を実行するとします。</span></p>
...
<ul class="ulist">
<li>
<p><span class="merged" id="all.2u5woT" title="原文 : RegistryFilterSettings"><a href="./apidocs/io.helidon.metrics.api/io/helidon/metrics/api/RegistryFilterSettings.html" target="_blank"><code>RegistryFilterSettings</code></a></span></p>

</li>
<li>
<p><span class="merged" id="all.3ASwK6" title="原文 : RegistrySettings"><a href="./apidocs/io.helidon.metrics.api/io/helidon/metrics/api/RegistrySettings.html" target="_blank"><code>RegistrySettings</code></a></span></p>

</li>
<li>
<p><span class="merged" id="all.hZde6" title="原文 : MetricsSettings"><a href="./apidocs/io.helidon.metrics.api/io/helidon/metrics/api/MetricsSettings.html" target="_blank"><code>MetricsSettings</code></a></span></p>

</li>
</ul>

<p><span class="merged" id="all.2P9R7B" title="原文 : and using the resulting MetricsSettings to retrieve a suitable RegistryFactory.">結果の<code>MetricsSettings</code>を使用して、適切な<a href="./apidocs/io.helidon.metrics.api/io/helidon/metrics/api/RegistryFactory.html" target="_blank"><code>RegistryFactory</code></a>を取得します。</span></p>
...
<div class="section">
<p><span class="merged" id="all.3sOZXl.2" title="原文 : Any time you include the Helidon metrics module in your application, Helidon tracks two basic performance indicator metrics:">アプリケーションにHelidonメトリクス・モジュールを含めるたびに、Helidonでは、次の2つの基本的なパフォーマンス・メトリクスが追跡されます:</span></p>

<ul class="ulist">
<li>
<p><span class="merged" id="all.3AcZDp.2" title="原文 : a Counter of all requests received (requests.count), and">受信したすべてのリクエストの<code>Counter</code> (<code>requests.count</code>)、および</span></p>

</li>
<li>
<p><span class="merged" id="all.153KWy.2" title="原文 : a Meter of all requests received (requests.meter).">受信したすべてのリクエストの<code>Meter</code> (<code>requests.meter</code>)。</span></p>
...
<ul class="ulist">
<li>
<p><span class="merged" id="all.JrarW.2" title="原文 : current number of requests in-flight - a ConcurrentGauge (requests.inFlight) of requests currently being processed">現在進行中のリクエスト数 - 現在処理中のリクエストの<code>ConcurrentGauge</code> (<code>requests.inFlight</code>)</span></p>

</li>
<li>
<p><span class="merged" id="all.2cyom6.2" title="原文 : long-running requests - a Meter (requests.longRunning) measuring the rate at which Helidon processes requests which take at least a given amount of time to complete; configurable, defaults to 10000 milliseconds (10 seconds)">長時間実行リクエスト - Helidonが完了までに少なくとも指定された時間を要するリクエストを処理する速度を測定する<code>Meter</code> (<code>requests.longRunning</code>)です。構成可能で、デフォルトは10000ミリ秒(10秒)です</span></p>

</li>
<li>
<p><span class="merged" id="all.2iOF4J.2" title="原文 : load - a Meter (requests.load) measuring the rate at which requests are worked on (as opposed to received)">load - リクエストの処理速度を測定する<code>Meter</code> (<code>requests.load</code>) (受信したものとは対照的)</span></p>

</li>
<li>
<p><span class="merged" id="all.1QFwn1.2" title="原文 : deferred - a Meter (requests.deferred) measuring the rate at which a request&rsquo;s processing is delayed after Helidon receives the request">延期 - Helidonがリクエストを受信した後にリクエスト処理が遅延する割合を測定する<code>Meter</code> (<code>requests.deferred</code>)</span></p>

</li>
</ul>

<p><span class="merged" id="all.oRutk.2" title="原文 : You can enable and control these metrics using configuration:">構成を使用して、次のメトリクスを有効化および制御できます:</span></p>
...
<ul class="colist">
<li data-value="1"><span class="merged" id="all.1FIfju" title="原文 : Create a KeyPerformanceIndicatorMetricsSettings instance (via its Builder) with non-default values.">デフォルト以外の値を持つ<a href="./apidocs/io.helidon.metrics.api/io/helidon/metrics/api/KeyPerformanceIndicatorMetricsSettings.html" target="_blank"><code>KeyPerformanceIndicatorMetricsSettings</a></code>インスタンス(<a href="./apidocs/io.helidon.metrics.api/io/helidon/metrics/api/KeyPerformanceIndicatorMetricsSettings.Builder.html" target="_blank"><code>Builder</code></a>を介して)を作成します。</span></li>
<li data-value="2"><span class="merged" id="all.2EI2p9" title="原文 : Create a MetricsSettings instance reflecting the KPI settings.">KPI設定を反映した<a href="./apidocs/io.helidon.metrics.api/io/helidon/metrics/api/MetricsSettings.html" target="_blank"><code>MetricsSettings</a></code>インスタンスを作成します。</span></li>
</ul>
...
import jakarta.json.JsonObject;
import org.eclipse.microprofile.metrics.MetricRegistry; <span class="conum" data-value="1" />
import org.eclipse.microprofile.metrics.Timer; <span class="conum" data-value="1" />
...
>{
  "cardTimer": {
    ...
    "elapsedTime": 26683406, <span class="conum" data-value="2" />
    "meanRate": 0.05669258258076838,
    "oneMinRate": 0,
    "fiveMinRate": 0,
    "fifteenMinRate": 0,
    "min": 26683406, <span class="conum" data-value="2" />
    "max": 26683406,
    "mean": 26683406,
    "stddev": 0,
    "p50": 26683406,
...
<ul class="colist">
<li data-value="1"><span class="merged" id="all.2JDUOH" title="原文 : The first several fields (except for elapsedTime) are the same ones used by Meter.">最初のいくつかのフィールド(<code>elapsedTime</code>を除く)は、<code>Meter</code>で使用されるものと同じです。</span></li>
<li data-value="2"><span class="merged" id="all.3uwvMR.spl1" title="原文 : The elapsedTime field and the rest starting with min are the Timer fields that measure the duration of the getDefaultMessageHandler method."><code>elapsedTime</code>フィールドおよび<code>min</code>で始まる残りのフィールドは、<code>getDefaultMessageHandler</code>メソッドの期間を測定する<code>Timer</code>フィールドです。</span> <span class="merged" id="all.3uwvMR.spl2" title="原文 : Some of these values will change each time you invoke the /cards endpoint.">これらの値の一部は、<code>/cards</code>エンドポイントを呼び出すたびに変更されます。</span> </li>
</ul>
...
<div class="section">
<p><span class="merged" id="all.34VxAG.spl1" title="原文 : The SimpleTimer metric counts invocations and accumulates duration (in seconds)."><code>SimpleTimer</code>メトリックは、呼出しをカウントし、期間(秒単位)を累積します。</span> <span class="merged" id="all.34VxAG.spl2" title="原文 : It also records the minimum and maximum values sampled during the previous complete minute.">また、前の完了分間にサンプリングされた最小値と最大値を記録します。</span> <span class="merged" id="all.34VxAG.spl3" title="原文 : In the following example, a SimpleTimer metric is used to count and measure the duration of a method&rsquo;s execution.">次の例では、<code>SimpleTimer</code>メトリックを使用して、メソッドの実行時間をカウントおよび測定します。</span> <span class="merged" id="all.34VxAG.spl4" title="原文 : Whenever the REST /cards endpoint is called, the SimpleTimer updates its count and total elapsed time.">REST <code>/cards</code>エンドポイントがコールされるたびに、<code>SimpleTimer</code>はその数と合計経過時間を更新します。</span> </p>
...
>{
  "cardSimpleTimer": {
    "count": 1, <span class="conum" data-value="1" />
    "elapsedTime": 13455720, <span class="conum" data-value="2" />
    "maxTimeDuration": null, <span class="conum" data-value="3" />
...
<li data-value="3"><span class="merged" id="all.1DTxqM.spl1" title="原文 : Simple timers report the minimum and maximum durations recorded over the preceding complete minute.">簡易タイマーは、前述の完了分間に記録された最小期間および最大期間をレポートします。</span> <span class="merged" id="all.1DTxqM.spl2" title="原文 : The JSON output contains null for these values if the simple timer recorded no observations during the preceding complete minute.">前の完了分間に単純なタイマーが観測を記録しなかった場合、JSON出力にはこれらの値の<code>null</code>が含まれます。</span> </li>
</ul>

<p><span class="merged" id="all.45JUKh" title="原文 : If you retrieve the metrics again shortly after the current minute has completed, the simple timer reports useful values.">現在の分が完了した直後にメトリクスを再取得すると、単純なタイマーは有用な値をレポートします。</span></p>

<markup
lang="json"
title="JSON response after the current minute completes:"
>{
  "cardSimpleTimer": {
    "count": 1,
...
<p><span class="merged" id="all.11GTyV" title="原文 : If you again wait until the next complete minute without accessing the greeting card endpoint again and rerun the metrics query, the simple timer reports null values again because there was no activity in the preceding complete minute.">グリーティング・カード・エンドポイントに再度アクセスせずに<em>「次」</em>が完了するまでの時間を再度待機し、メトリクス問合せを再実行すると、前の完了分にはアクティビティがなかったため、単純なタイマーはnull値を再度レポートします。</span></p>
...
<ul class="ulist">
<li>
<p><span class="merged" id="all.Bbpbc.2" title="原文 : MicroProfile Metrics specification"><a href="https://download.eclipse.org/microprofile/microprofile-metrics-4.0/microprofile-metrics-spec-4.0.html" target="_blank">MicroProfileメトリクス仕様</a></span></p>

</li>
<li>
<p><span class="merged" id="all.17OsJ4.1" title="原文 : MicroProfile Metrics Javadoc"><a href="https://download.eclipse.org/microprofile/microprofile-metrics-4.0/apidocs" target="_blank">MicroProfileメトリクスのJavadoc</a></span></p>

</li>
<li>
<p><span class="merged" id="all.1cU8n2.5" title="原文 : Helidon Javadoc"><a href="./apidocs/index.html?overview-summary.html" target="_blank">Helidon Javadoc</a></span></p>
