...
<div class="section">
<p><span class="merged" id="all.3B9fGx.spl1" title="原文 : The config system records when it loads each configuration into memory.">config systemは、各構成をメモリーにロードするタイミングを記録します。</span> <span class="merged" id="all.3B9fGx.spl2" title="原文 : Your application can retrieve it by invoking the timestamp method:">アプリケーションでは、<a href="./apidocs/io.helidon.config/io/helidon/config/Config.html#timestamp--" target="_blank">timestampメソッド</a>を起動して取得できます:</span> </p>
...
<div class="section">
<p><span class="merged" id="all.23KoZX.spl1" title="原文 : The config system maintains a Config.Context for each Config node.">構成システムでは、<code>Config</code>ノードごとに<a href="./apidocs/io.helidon.config/io/helidon/config/Config.Context.html" target="_blank"><code>Config.Context</code></a>が保持されます。</span> <span class="merged" id="all.23KoZX.spl2" title="原文 : Your application can retrieve the context by invoking the Config.context() method and then use it for these operations:">アプリケーションでは、<code>Config.context()</code>メソッドを起動してコンテキストを取得し、次の操作に使用できます:</span> </p>
...
<th><span class="merged" id="all.JUOSC.42"  title="原文:: Usage">使用方法</span></th>
</tr>
</thead>
<tbody>
<tr>
...
<p ><p><span class="merged" id="all.1Jp8J1.spl1" title="原文 : This section describes the Config.changes() method.">この項では、<code>Config.changes()</code>のメソッドについて説明します。</span> <span class="merged" id="all.1Jp8J1.spl2" title="原文 : It is marked as deprecated because it returns an io.helidon.reactive.Flow.Publisher object."><code>io.helidon.reactive.Flow.Publisher</code>オブジェクトを返すため、非推奨としてマークされています。</span> <span class="merged" id="all.1Jp8J1.spl3" title="原文 : In a future Helidon release that requires Java 11 or later this method will be un-deprecated and changed - or a similar method will be added - so that the return type is java.util.concurrent.Flow.Publisher instead.">Java 11以降を必要とする将来のHelidonリリースでは、このメソッドは非推奨になって変更されるか、または同様のメソッドが追加されるため、戻り型は<code>java.util.concurrent.Flow.Publisher</code>になります。</span> </p>
...
<div class="section">
<p><span class="merged" id="all.3rg8Jp.spl1" title="原文 : When the application creates a config source, it can set up change detection for that source.">アプリケーションは、構成ソースを作成するときに、そのソースの変更検出を設定できます。</span> <span class="merged" id="all.3rg8Jp.spl2" title="原文 : This is called polling in the Helidon API but specific change detection algorithms might not use actual polling.">これはHelidon APIでは<em>「ポーリング」</em>と呼ばれますが、特定の変更検出アルゴリズムでは実際のポーリングが使用されない場合があります。</span> <span class="merged" id="all.3rg8Jp.spl3" title="原文 : You choose a specific PollingStrategy for each config source you want to monitor.">モニターする構成ソースごとに特定の<a href="./apidocs/io.helidon.config/io/helidon/config/spi/PollingStrategy.html" target="_blank"><code>PollingStrategy</code></a>を選択します。</span> <span class="merged" id="all.3rg8Jp.spl4" title="原文 : See the section on polling strategies in the config extensions doc page for more information.">詳細は、構成拡張ドキュメント・ページの<router-link :to="{path: '/se/config/extensions', hash: '#Config-SPI-PollingStrategy'}">「ポーリング計画」</router-link>に関する項を参照してください。</span> </p>

<p><span class="merged" id="all.3AvLKb" title="原文 : The config system provides some built-in polling strategies, exposed as these methods on the PollingStrategies class:">構成システムには、<a href="./apidocs/io.helidon.config/io/helidon/config/PollingStrategies.html" target="_blank"><code>PollingStrategies</code></a>クラスで次のメソッドとして公開されている組込みポーリング戦略がいくつか用意されています:</span></p>
...
<p><span class="merged" id="all.3q4knF.spl1" title="原文 : Each Config instance exposes the Config.changes() method which returns a Flow.Publisher&lt;Config&gt;.">各<code>Config</code>インスタンスは、<code>Flow.Publisher&lt;Config></code>を返す<a href="./apidocs/io.helidon.config/io/helidon/config/Config.html#changes--" target="_blank"><code>Config.changes()</code></a>メソッドを公開します。</span> <span class="merged" id="all.3q4knF.spl2" title="原文 : Your application can invoke this method, then invoke subscribe on the returned Flow.Publisher, passing your own Flow.Subscriber implementation.">アプリケーションでこのメソッドを呼び出してから、返された<code>Flow.Publisher</code>で<code>subscribe</code>を呼び出し、独自の<code>Flow.Subscriber</code>実装を渡すことができます。</span> <span class="merged" id="all.3q4knF.spl3" title="原文 : The config system will invoke your subscriber&rsquo;s methods as appropriate, most notably calling onNext whenever it detects a change in one of the underlying config sources for the config node of interest.">構成システムは、目的の構成ノードの基礎となる構成ソースのいずれかで変更が検出されるたびに、必要に応じてサブスクライバのメソッド(特に<code>onNext</code>をコールする)を起動します。</span> </p>
