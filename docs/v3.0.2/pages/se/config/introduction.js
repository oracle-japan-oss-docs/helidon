...
<h2 id="_contents"><span class="merged" id="all.1Q34Zl.39"  title="原文:: Contents">目次</span></h2>
<div class="section">
<ul class="ulist">
<li>
...
<li>
<p><span class="merged" id="all.26FILD.22" title="原文 : Configuration"><router-link @click.native="this.scrollFix('#_configuration')" to="#_configuration">構成</router-link></span></p>
...
<div class="section">
<p><span class="merged" id="all.1twj18" title="原文 : To enable Config add the following dependency to your project&rsquo;s pom.xml (see Managing Dependencies).">構成を有効にするには、プロジェクトの<code>pom.xml</code>に次の依存関係を追加します(<router-link to="/about/managing-dependencies">「依存関係の管理」</router-link>を参照)。</span></p>
...
<h2 id="_usage"><span class="merged" id="all.JUOSC.41"  title="原文:: Usage">使用方法</span></h2>
<div class="section">
...
<p><span class="merged" id="all.1lBOd1" title="原文 : See the JavaDoc for the ConfigSources class for a complete list of the built-in config source types and how to use them.">組込み構成ソース・タイプの完全なリストおよびその使用方法は、<a href="./apidocs/io.helidon.config/io/helidon/config/ConfigSources.html" target="_blank"><code>ConfigSources</code></a>クラスのJavaDocを参照してください。</span></p>
...
<p><span class="merged" id="all.3UIWSi.spl1" title="原文 : The config system includes several built-in parsers, such as for the Java properties, YAML, JSON, and HOCON formats.">構成システムには、Javaプロパティ、YAML、JSON、HOCON形式などの組込みパーサーがいくつか含まれています。</span> <span class="merged" id="all.3UIWSi.spl2" title="原文 : See this section for how to change your pom.xml to make parsers for those formats available to your application."><code>pom.xml</code>を変更して、これらのフォーマットのパーサーをアプリケーションで使用できるようにする方法は、<router-link @click.native="this.scrollFix('#built-in-formats')" to="#built-in-formats">「このセクション」</router-link>を参照してください。</span> <span class="merged" id="all.3UIWSi.spl3" title="原文 : Then your application can invoke the config builder&rsquo;s addParser method so that builder will use the parsers you choose.">その後、選択したパーサーがビルダーで使用されるように、アプリケーションで<a href="./apidocs/io.helidon.config/io/helidon/config/Config.Builder.html#addParser-io.helidon.config.spi.ConfigParser-" target="_blank">「構成ビルダー<code>addParser</code>」</a>メソッドを起動できます。</span> </p>

<p><span class="merged" id="all.3E2im6.spl1" title="原文 : You can extend the system with custom parsers of your own.">独自のカスタム・パーサーを使用してシステムを拡張できます。</span> <span class="merged" id="all.3E2im6.spl2" title="原文 : Implement the ConfigParser interface, then construct a Config.Builder using the addParser method, passing an instance of your customer parser."><a href="./apidocs/io.helidon.config/io/helidon/config/spi/ConfigParser.html" target="_blank"><code>ConfigParser</code></a>インタフェースを実装し、<code>addParser</code>メソッドを使用して<code>Config.Builder</code>を構築し、カスタマ・パーサーのインスタンスを渡します。</span> <span class="merged" id="all.3E2im6.spl3" title="原文 : Invoke one of the sources methods to include a source that uses the custom format and then build the Config object.">いずれかの<code>sources</code>メソッドを呼び出して、カスタム書式を使用するソースを含めてから、<code>Config</code>オブジェクトをビルドします。</span> </p>
...
<p><span class="merged" id="all.1rcWy8.spl1" title="原文 : When your application prepares a Config.Builder it sets what ConfigSources and ConfigParsers the builder should use in constructing the resulting Config object.">アプリケーションで<code>Config.Builder</code>を準備する際、ビルダーが結果の<code>Config</code>オブジェクトをビルドする際に使用する<code>ConfigSource</code>および<code>ConfigParser</code>を設定します。</span> <span class="merged" id="all.1rcWy8.spl2" title="原文 : The JavaDoc explains how to use the Config.Builder.">JavaDocでは、<a href="./apidocs/io.helidon.config/io/helidon/config/Config.Builder.html" target="_blank"><code>Config.Builder</code></a>の使用方法について説明します。</span> </p>
...
<p><span class="merged" id="all.uef5s.spl1" title="原文 : The configuration has the same internal representation in Config.">構成は、<code>Config</code>で同じ内部表現を持ちます。</span> <span class="merged" id="all.uef5s.spl2" title="原文 : Once created, the Config object provides many methods the application can use to retrieve config data as various Java types."><code>Config</code>オブジェクトを作成すると、アプリケーションで様々なJavaタイプとして構成データを取得するために使用できる多くのメソッドが提供されます。</span> <span class="merged" id="all.uef5s.spl3" title="原文 : See the Config JavaDoc for complete details.">詳細は、<a href="./apidocs/io.helidon.config/io/helidon/config/Config.html" target="_blank"><code>Config</code></a> JavaDocを参照してください。</span> </p>
...
<p><span class="merged" id="all.QsGlL" title="原文 : See the RetryPolicies JavaDoc for complete details on these built-in retry policies.">これらの組込みの再試行ポリシーの詳細は、<a href="./apidocs/io.helidon.config/io/helidon/config/RetryPolicies.html" target="_blank"><code>RetryPolicies</code></a> JavaDocを参照してください。</span></p>

<p><span class="merged" id="all.3hbYqM.spl1" title="原文 : You can devise your own policy.">独自のポリシーを考案できます。</span> <span class="merged" id="all.3hbYqM.spl2" title="原文 : Implement the RetryPolicy interface."><a href="./apidocs/io.helidon.config/io/helidon/config/spi/RetryPolicy.html" target="_blank"><code>RetryPolicy</code></a>インタフェースを実装します。</span> <span class="merged" id="all.3hbYqM.spl3" title="原文 : Then pass an instance of your policy implementation to the config source builder&rsquo;s retryPolicy method.">次に、ポリシー実装のインスタンスを構成ソース・ビルダーの<code>retryPolicy</code>メソッドに渡します。</span> </p>
...
<p><span class="merged" id="all.2VKPlS" title="原文 : The mutability documentation explains this in detail, and the PollingStrategies JavaDoc describes the built-in implementations."><router-link to="/se/config/mutability-support">「可変性」</router-link>ドキュメントではこの詳細について説明し、<a href="./apidocs/io.helidon.config/io/helidon/config/PollingStrategies.html" target="_blank"><code>PollingStrategies</code></a> JavaDocでは組込み実装について説明します。</span></p>

<p><span class="merged" id="all.spZTX.spl1" title="原文 : You can, of course, write your own by implementing the PollingStrategy interface.">もちろん、<a href="./apidocs/io.helidon.config/io/helidon/config/spi/PollingStrategy.html" target="_blank"><code>PollingStrategy</code></a>インタフェースを実装することで、独自に記述できます。</span> <span class="merged" id="all.spZTX.spl2" title="原文 : On a config source builder invoke pollingStrategy with an instance of your custom strategy and then invoke build to create the ConfigSource.">構成ソース・ビルダーで、カスタム戦略のインスタンスを使用して<code>pollingStrategy</code>を起動し、<code>build</code>を起動して<code>ConfigSource</code>を作成します。</span> </p>
...
<th><span class="merged" id="all.3Wy1iS.1"  title="原文:: Notes">ノート</span></th>
</tr>
</thead>
<tbody>
<tr>
...
<p><span class="merged" id="all.2L2qd6.spl1" title="原文 : You can also extend the config system to handle other types of sources by implementing the ConfigSource interface."><a href="./apidocs/io.helidon.config/io/helidon/config/spi/ConfigSource.html" target="_blank"><code>ConfigSource</code></a>インタフェースを実装することで、他のタイプのソースを処理するように構成システムを拡張することもできます。</span> <span class="merged" id="all.2L2qd6.spl2" title="原文 : See the extensions&apos; documentation for complete information.">詳細は、<router-link to="/se/config/extensions">extensions'</router-link>のドキュメントを参照してください。</span> </p>
