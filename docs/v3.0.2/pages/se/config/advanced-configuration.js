...
<div class="section">
<p><span class="merged" id="all.2AzrFs.spl1" title="原文 : The config system provides several ways to create a Config tree from data already in memory.">configシステムには、すでにメモリー内にあるデータから<code>Config</code>ツリーを作成する方法がいくつか用意されています。</span> <span class="merged" id="all.2AzrFs.spl2" title="原文 : See the ConfigSources javadoc for further details.">詳細は、<a href="./apidocs/io.helidon.config/io/helidon/config/ConfigSources.html" target="_blank"><code>ConfigSources</code> javadoc</a>を参照してください。</span> <span class="merged" id="all.2AzrFs.spl3" title="原文 : The numerous variants of the from method construct ConfigSource or Builder&lt;ConfigSource&gt; instances."><code>from</code>メソッドの様々なバリアントによって、<code>ConfigSource</code>または<code>Builder&lt;ConfigSource></code>インスタンスが構成されます。</span> </p>
...
<p><span class="merged" id="all.kSLMf.spl1" title="原文 : The config system lets you assign a prefix to all keys from a given source using the ConfigSources.prefixed method.">configシステムでは、<a href="./apidocs/io.helidon.config/io/helidon/config/ConfigSources.html#prefixed-java.lang.String-java.util.function.Supplier-" target="_blank"><code>ConfigSources.prefixed</code></a>メソッドを使用して、特定のソースのすべてのキーにプレフィクスを割り当てることができます。</span> <span class="merged" id="all.kSLMf.spl2" title="原文 : The following example shows two YAML files as config sources and the code to load each with a different prefix into a single Config tree:">次の例は、構成ソースとしての2つのYAMLファイルと、それぞれ異なるプレフィクスを持つコードを単一の<code>Config</code>ツリーにロードする方法を示しています:</span> </p>
...
<p><span class="merged" id="all.40nN4q.spl1" title="原文 : Each CompositeConfigSource&apos;s merging strategy actually controls this behavior.">実際には、各<code>CompositeConfigSource</code>の<em>「マージ戦略」</em>がこの動作を制御します。</span> <span class="merged" id="all.40nN4q.spl2" title="原文 : The config system provides the FallbackMergingStrategy which implements the default, &quot;first wins&quot; algorithm.">構成システムには、デフォルトの「最初に優先」アルゴリズムを実装する<code>FallbackMergingStrategy</code>が用意されています。</span> <span class="merged" id="all.40nN4q.spl3" title="原文 : You can write your own implementation of ConfigSources.MergingStrategy and use it instead to provide a different algorithm."><a href="./apidocs/io.helidon.config/io/helidon/config/ConfigSources.MergingStrategy.html" target="_blank"><code>ConfigSources.MergingStrategy</code></a>の独自の実装を作成し、かわりにそれを使用して別のアルゴリズムを提供できます。</span> </p>
...
<p><span class="merged" id="all.bjI1m" title="原文 : The built-in value-resolving filter enables the token substitution described below.">組込み<a href="./apidocs/io.helidon.config/io/helidon/config/ConfigFilters.html#valueResolving--" target="_blank">value-resolving</a>フィルタを使用すると、次に説明するトークンの置換が可能になります。</span></p>

<p><span class="merged" id="all.1MuVyD" title="原文 : See the ConfigFilter JavaDoc for more information.">詳細は、<a href="./apidocs/io.helidon.config/io/helidon/config/spi/ConfigFilter.html" target="_blank"><code>ConfigFilter</code></a> JavaDocを参照してください。</span></p>
...
<p><span class="merged" id="all.2tOUpv" title="原文 : See the OverrideSource JavaDoc for more detail.">詳細は、<a href="./apidocs/io.helidon.config/io/helidon/config/spi/OverrideSource.html" target="_blank"><code>OverrideSource</a></code> JavaDocを参照してください。</span></p>
...
<div class="section">
<p><span class="merged" id="all.3HUiiE.spl1" title="原文 : The two methods PollingStrategies.regular(Duration) and PollingStrategies.watch(Path) return builders for their respective strategies.">この2つのメソッドでは、<code>PollingStrategies.regular(Duration)</code>と<code>PollingStrategies.watch(Path)</code>がそれぞれの戦略のビルダーを返します。</span> <span class="merged" id="all.3HUiiE.spl2" title="原文 : Both builders expose the executor method which your application can invoke, passing a java.util.concurrent.ScheduledExecutorService instance it requires for the polling work.">どちらのビルダーでも、アプリケーションが起動できる<code>executor</code>メソッドが公開され、ポーリング作業に必要な<code>java.util.concurrent.ScheduledExecutorService</code>インスタンスが渡されます。</span> <span class="merged" id="all.3HUiiE.spl3" title="原文 : By default, each polling strategy instance uses a separate thread pool executor.">デフォルトでは、各ポーリング戦略インスタンスは個別のスレッド・プール・エグゼキュータを使用します。</span> </p>
...
<div class="section">
<p><span class="merged" id="all.zV9Pf.spl1" title="原文 : Recall that when a polling strategy detects a change in a source, it informs interested parties of the changes.">ポーリング戦略でソースの変更が検出されると、その変更がパーティに通知されることを思い出してください。</span> <span class="merged" id="all.zV9Pf.spl2" title="原文 : By default, each Config.Builder arranges for the resulting Config tree to use a shared executor that reuses available threads from a pool, creating new threads as needed.">デフォルトでは、各<code>Config.Builder</code>は、結果の<code>Config</code>ツリーに、プールから使用可能なスレッドを再利用する共有エグゼキュータを使用するように配置し、必要に応じて新しいスレッドを作成します。</span> <span class="merged" id="all.zV9Pf.spl3" title="原文 : The same executor is used for actually reloading the source.">実際にソースをリロードする場合は、同じエグゼキュータが使用されます。</span> </p>
...
<div class="section">
<p><span class="merged" id="all.2OGhY0.spl1" title="原文 : When your application supplies multiple sources to a config builder, as with Config.create(Supplier&lt;ConfigSource&gt;…​) and Config.create(List&lt;Supplier&lt;ConfigSource&gt;&gt;), the config system automatically uses a composite config source which aggregates the separate sources but also listens for changes to any of the individual sources, so it can delegate the change notification."><code>Config.create(Supplier&lt;ConfigSource>…​)</code>および<code>Config.create(List&lt;Supplier&lt;ConfigSource>>)</code>のように、アプリケーションが複数のソースを構成ビルダーに提供すると、構成システムは、個別のソースを集計する<em>「コンポジット構成ソース」</em>を自動的に使用します。また、個々のソースに対する変更をリスニングするため、変更通知を委任できます。</span> <span class="merged" id="all.2OGhY0.spl2" title="原文 : For this change detection and notification the config system, by default, uses an executor with a dedicated thread pool that is shared across all Config instances.">この変更検出および通知のために、構成システムはデフォルトで、すべての<code>Config</code>インスタンス間で共有される専用のスレッド・プールを持つエグゼキュータを使用します。</span> </p>

<p><span class="merged" id="all.3K6AhO.spl1" title="原文 : Your application can invoke the builder&rsquo;s changesExecutor method to use a different ScheduledExecutorService instance.">アプリケーションでは、別の<code>ScheduledExecutorService</code>インスタンスを使用するためにビルダーの<code>changesExecutor</code>メソッドを起動できます。</span> <span class="merged" id="all.3K6AhO.spl2" title="原文 : The builder returned by the from methods mentioned above is a CompositeBuilder which extends Config.Builder.">前述の<code>from</code>メソッドによって返されるビルダーは、<code>Config.Builder</code>を拡張する<a href="./apidocs/io.helidon.config/io/helidon/config/ConfigSources.CompositeBuilder.html" target="_blank">CompositeBuilder</a>です。</span> </p>

<p><span class="merged" id="all.4Purgr.spl1" title="原文 : Because a composite source might yield more numerous change events - because of the multiple underlying sources - your application can specify debounce timeout for the composite source by invoking the CompositeBuilder.changesDebounce(Duration) method.">コンポジット・ソースでは、基礎となるソースが複数あるため、アプリケーションは<code>CompositeBuilder.changesDebounce(Duration)</code>メソッドを起動してコンポジット・ソースの<em>「デバウンス・タイムアウト」</em>を指定できます。</span> <span class="merged" id="all.4Purgr.spl2" title="原文 : The composite source aggregates multiple change events within this debounce timeout period into a single event and broadcasts that one instead.">コンポジット・ソースは、この<em>「デバウンス・タイムアウト」</em>期間内の複数の変更イベントを1つのイベントに集計し、かわりにそのイベントをブロードキャストします。</span> <span class="merged" id="all.4Purgr.spl3" title="原文 : Next, it reloads the sources at that time, not necessarily in response to every single change in any source.">次に、その時点でソースがリロードされます。これは、ソースのすべての1つの変更にレスポンスしているとはかぎりません。</span> <span class="merged" id="all.4Purgr.spl4"  title="原文: The default is 100 milliseconds.">デフォルトは<code>100</code>ミリ秒です。</span> </p>
...
<p><span class="merged" id="all.1Dc4TM.spl1" title="原文 : Your application can specify a non-default Executor for a tree to use for accepting and propagating those events by invoking the changesExecutor method on the Config.Builder.">アプリケーションでは、<code>Config.Builder</code>で<code>changesExecutor</code>メソッドを起動することで、これらのイベントの受入れおよび伝播に使用するツリーにデフォルト以外の<code>Executor</code>を指定できます。</span> <span class="merged" id="all.1Dc4TM.spl2" title="原文 : Each source subscriber has a dedicated buffer for holding changes events.">各ソース・サブスクライバには、変更イベントを保持するための専用バッファがあります。</span> <span class="merged" id="all.1Dc4TM.spl3" title="原文 : This defaults to 256, but you can tailor this value as needed.">このデフォルト値は256ですが、必要に応じてこの値を調整できます。</span> </p>
...
<div class="section">
<p><span class="merged" id="all.2aq55s.spl1" title="原文 : You can control which executor a retry policy should use for its work.">再試行ポリシーで作業に使用するエグゼキュータを制御できます。</span> <span class="merged" id="all.2aq55s.spl2" title="原文 : The RetryPolicies.repeat(int retries) method returns a RetryPolicies.Builder."><code>RetryPolicies.repeat(int retries)</code>メソッドは、<a href="./apidocs/io.helidon.config/io/helidon/config/RetryPolicies.Builder.html" target="_blank">RetryPolicies.Builder</a>を返します。</span> <span class="merged" id="all.2aq55s.spl3" title="原文 : Your application can invoke the retry policy builder&rsquo;s executor method to specify which ScheduledExecutorService instance it should use to schedule and execute delayed retries.">アプリケーションでは、再試行ポリシー・ビルダーの<code>executor</code>メソッドを起動して、遅延再試行のスケジュールおよび実行に使用する<code>ScheduledExecutorService</code>インスタンスを指定できます。</span> <span class="merged" id="all.2aq55s.spl4" title="原文 : By default, the config system uses a separate thread pool executor for each retry policy instance.">デフォルトでは、構成システムは再試行ポリシー・インスタンスごとに個別のスレッド・プール・エグゼキュータを使用します。</span> </p>
