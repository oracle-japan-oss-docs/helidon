...
<div class="section">
<p><span class="merged" id="all.3cPTme.spl1" title="原文 : The Config class itself provides many conversions to Java types."><a href="./apidocs/io.helidon.config/io/helidon/config/Config.html" target="_blank"><code>Config</code></a>クラス自体は、Java型への多数の変換を提供します。</span> <span class="merged" id="all.3cPTme.spl2" title="原文 : See the JavaDoc for the complete list.">完全なリストは、JavaDocを参照してください。</span> </p>
...
<p><span class="merged" id="all.2pipcW.spl1" title="原文 : The numerous conversions defined on the Config class for other types (integers, doubles, etc.) will satisfy many of your application&rsquo;s needs.">他の型(integer、doubleなど)に対して<code>Config</code>クラスで定義されている多数の変換は、アプリケーションのニーズの多くを満たします。</span> <span class="merged" id="all.2pipcW.spl2" title="原文 : The ConfigMappers class includes other related mappings from String (rather than from Config) to Java types (described in the JavaDoc)."><a href="./apidocs/io.helidon.config/io/helidon/config/ConfigMappers.html" target="_blank"><code>ConfigMappers</code></a>クラスには、<code>String</code> (<code>Config</code>ではなく)からJavaタイプ(JavaDocで説明されている)へのその他の関連マッピングが含まれています。</span> </p>
...
<div class="section">
<p><span class="merged" id="all.4IzEk9.spl1" title="原文 : The Config class exposes several methods for mapping a structured config node to a Java List or Map."><code>Config</code>クラスは、構造化構成ノードをJava <code>List</code>または<code>Map</code>にマッピングするためのいくつかのメソッドを公開しています。</span> <span class="merged" id="all.4IzEk9.spl2" title="原文 : The JavaDoc contains complete details, but briefly your application can convert a structured Config node into:"><a href="./apidocs/io.helidon.config/io/helidon/config/Config.html" target="_blank">JavaDoc</a>には完全な詳細が含まれていますが、アプリケーションで構造化<code>Config</code>ノードを次のように簡単に変換できます:</span> </p>
...
<li>
<span class="merged" id="all.4Axyf3.spl1" title="原文 : implementing ConfigMapperProvider so it returns an instance of your mapper (see the JavaDoc for complete information) and creating or editing the file io.helidon.config.spi.ConfigMapperProvider so it contains a line with the fully-qualified class name of your ConfigMapperProvider.">マッパーのインスタンスを返すように<a href="./apidocs/io.helidon.config/io/helidon/config/spi/ConfigMapperProvider.html" target="_blank"><code>ConfigMapperProvider</code></a>を実装し(詳細はJavaDocを参照)、<code>ConfigMapperProvider</code>の完全修飾クラス名の行が含まれるように<code>io.helidon.config.spi.ConfigMapperProvider</code>ファイルを作成または編集します。</span> <span class="merged" id="all.4Axyf3.spl2" title="原文 : The config system will use the Java service loader to find and invoke all ConfigMapperProvider classes listed and add the mappers they provide to each Config.Builder automatically.">構成システムは、Javaサービス・ローダーを使用して、リストされているすべての<code>ConfigMapperProvider</code>クラスを検索して呼び出し、提供されているマッパーを各<code>Config.Builder</code>に自動的に追加します。</span> 
...
<p><span class="merged" id="all.3BmiEf" title="原文 : Use the Value annotation to control some of JavaBean processing for a given property."><a href="./apidocs/io.helidon.config/io/helidon/config/Value.html" target="_blank"><code>Value</code></a>注釈を使用して、特定のプロパティのJavaBean処理の一部を制御します。</span></p>
...
<th><span class="merged" id="all.JUOSC.43"  title="原文:: Usage">使用方法</span></th>
</tr>
</thead>
<tbody>
<tr>
...
<p><span class="merged" id="all.3TzVBx" title="原文 : To exclude a bean property from the config system bean processing annotate it with Config.Transient.">Beanプロパティを構成システムから除外するには、Beanプロパティに<a href="./apidocs/io.helidon.config/io/helidon/config/Config.Transient.html" target="_blank"><code>Config.Transient</code></a>の注釈を付けます。</span></p>
