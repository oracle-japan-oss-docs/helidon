...
<p><span class="merged" id="all.4LXqVx.spl1" title="原文 : If your application creates a Config.Builder with parser services disabled (see disableParserServices then that builder will not find the Java services for the various parsers and so will be unable to match the file type or media type of sources with the corresponding parser automatically.">アプリケーションがパーサー・サービスを<em>「無効」</em>にして<code>Config.Builder</code>を作成する場合(<a href="./apidocs/io.helidon.config/io/helidon/config/Config.Builder.html#disableParserServices--" target="_blank"><code>disableParserServices</code></a>を参照)、そのビルダーは様々なパーサーのJavaサービスを検出しないため、ソースのファイル・タイプまたはメディア・タイプを対応するパーサーと自動的に照合できません。</span> <span class="merged" id="all.4LXqVx.spl2" title="原文 : So if you want to use automatic type matching with a given builder, do not invoke Config.Builder.disableParserServices().">そのため、特定のビルダーで自動型照合を使用する場合は、<code>Config.Builder.disableParserServices()</code>を起動しないでください。</span> </p>
...
<h4 id="_maven_coordinates"><span class="merged" id="all.317oeS.35"  title="原文:: Maven Coordinates">Maven連携</span></h4>
<div class="section">
...
<li data-value="2"><span class="merged" id="all.34njv2.spl1" title="原文 : The config system will parse the resource my-config on the runtime classpath using the YAML parser instance created by the YamlConfigParserBuilder.">構成システムは、<a href="./apidocs/io.helidon.config/io/helidon/config/yaml/YamlConfigParserBuilder.html" target="_blank"><code>YamlConfigParserBuilder</code></a>によって作成されたYAMLパーサー・インスタンスを使用して、ランタイム・クラスパスのリソース<code>my-config</code>を解析します。</span> <span class="merged" id="all.34njv2.spl2" title="原文 : The buildDefault() method creates a config parser with default behavior."><code>buildDefault()</code>メソッドは、デフォルトの動作で構成パーサーを作成します。</span> </li>
</ul>
...
<h4 id="_maven_coordinates_2"><span class="merged" id="all.317oeS.36"  title="原文:: Maven Coordinates">Maven連携</span></h4>
<div class="section">
...
<li data-value="2"><span class="merged" id="all.2hCNjz.spl1" title="原文 : The config system will parse the resource my-config using the HOCON parser created by the HoconConfigParserBuilder.">構成システムは、<a href="./apidocs/io.helidon.config/io/helidon/config/hocon/HoconConfigParserBuilder.html" target="_blank">HoconConfigParserBuilder</a>によって作成されたHOCONパーサーを使用してリソース<code>my-config</code>を解析します。</span> <span class="merged" id="all.2hCNjz.spl2" title="原文 : The buildDefault() method creates a config parser with default behavior."><code>buildDefault()</code>メソッドは、デフォルトの動作で構成パーサーを作成します。</span> </li>
</ul>
...
<p><span class="merged" id="all.3JOKZy" title="原文 : You can also specify ConfigResolveOptions using the HoconConfigParserBuilder.resolveOptions method."><code>HoconConfigParserBuilder.resolveOptions</code>メソッドを使用して<a href="https://github.com/lightbend/config/blob/master/config/src/main/java/com/typesafe/config/ConfigResolveOptions.java" target="_blank"><code>ConfigResolveOptions</code></a>を指定することもできます。</span></p>
...
<h4 id="_maven_coordinates_3"><span class="merged" id="all.317oeS.37"  title="原文:: Maven Coordinates">Maven連携</span></h4>
<div class="section">
...
<div class="section">
<p><span class="merged" id="all.2iLOSH" title="原文 : To read configuration from an Etcd source, your application uses the EtcdConfigSourceBuilder.">Etcdソースから構成を読み取るには、アプリケーションで<a href="./apidocs/io.helidon.config/io/helidon/config/etcd/EtcdConfigSourceBuilder.html" target="_blank"><code>EtcdConfigSourceBuilder</code></a>を使用します。</span></p>
...
<p><span class="merged" id="all.LNiP" title="原文 : The EtcdConfigSourceBuilder class extends AbstractParsableConfigSource.Builder and so supports the usual settings on config sources."><code>EtcdConfigSourceBuilder</code>クラスは<a href="./apidocs/io.helidon.config/io/helidon/config/spi/AbstractParsableConfigSource.Builder.html" target="_blank"><code>AbstractParsableConfigSource.Builder</code></a>を拡張するため、構成ソースの通常の設定をサポートしています。</span></p>
...
<div class="section">
<p><span class="merged" id="all.3QCS0S.spl1" title="原文 : The config system can load information about config sources from meta-configuration rather than requiring your application to construct the builder.">構成システムは、アプリケーションでビルダーを構築する必要なく、<router-link :to="{path: '/se/config/advanced-configuration', hash: '#Config-Advanced-Config-MetaConfig'}">meta-configuration</router-link>から構成ソースに関する情報をロードできます。</span> <span class="merged" id="all.3QCS0S.spl2" title="原文 : To read meta-configuration from an Etcd source set the following required properties for the source:">Etcdソースからメタ構成を読み取るには、ソースに次の必須プロパティを設定します:</span> </p>
...
<p><span class="merged" id="all.2RrwnN" title="原文 : Other optional properties are inherited from AbstractParsableConfigSource.Builder. (see javadoc)">その他のオプションの<code>properties</code>は、<code>AbstractParsableConfigSource.Builder</code>から継承されます(<a href="./apidocs/io.helidon.config/io/helidon/config/spi/AbstractParsableConfigSource.Builder.html#init-io.helidon.config.Config-" target="_blank">javadoc</a>を参照)</span></p>
...
<h4 id="_maven_coordinates_4"><span class="merged" id="all.317oeS.38"  title="原文:: Maven Coordinates">Maven連携</span></h4>
<div class="section">
...
<div class="section">
<p><span class="merged" id="all.1kvzkj" title="原文 : To read configuration from a git source, your application uses the GitConfigSourceBuilder.">gitソースから構成を読み取るには、アプリケーションで<a href="./apidocs/io.helidon.config/io/helidon/config/git/GitConfigSourceBuilder.html" target="_blank"><code>GitConfigSourceBuilder</code></a>を使用します。</span></p>
...
<p><span class="merged" id="all.3m9T0A" title="原文 : The GitConfigSourceBuilder supports the usual source builder properties because it extends AbstractParsableConfigSource.Builder."><code>GitConfigSourceBuilder</code>は、<a href="./apidocs/io.helidon.config/io/helidon/config/spi/AbstractParsableConfigSource.Builder.html" target="_blank"><code>AbstractParsableConfigSource.Builder</code></a>を拡張するため、通常のソース・ビルダー・プロパティをサポートしています。</span></p>
...
<p><span class="merged" id="all.1Fx69k.spl1" title="原文 : You can also implement your own polling strategy by implementing PollingStrategy."><a href="./apidocs/io.helidon.config/io/helidon/config/spi/PollingStrategy.html" target="_blank"><code>PollingStrategy</code></a>を実装して、独自のポーリング戦略を実装することもできます。</span> <span class="merged" id="all.1Fx69k.spl2" title="原文 : See the mutability support and polling strategy discussions."><router-link to="/se/config/mutability-support">「可変性のサポート」</router-link>および<router-link :to="{path: '/se/config/extensions', hash: '#Config-SPI-PollingStrategy'}">「ポーリング計画」</router-link>のディスカッションを参照してください。</span> </p>
...
<p><span class="merged" id="all.1IVJK4.spl1" title="原文 : The meta-configuration must set the path and one of uri or directory.">メタ構成では、<code>path</code>および<code>uri</code>または<code>directory</code>のいずれかを設定する必要があります。</span> <span class="merged" id="all.1IVJK4.spl2" title="原文 : Other optional properties are inherited from AbstractParsableConfigSource.Builder (see javadoc)">その他のオプションの<code>properties</code>は、<code>AbstractParsableConfigSource.Builder</code>から継承されます(<a href="./apidocs/io.helidon.config/io/helidon/config/spi/AbstractParsableConfigSource.Builder.html#init-io.helidon.config.Config-" target="_blank">javadoc</a>を参照)</span> </p>
