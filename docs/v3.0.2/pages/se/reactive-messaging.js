...
<div class="section">
<p><span class="merged" id="all.1FhYR7" title="原文 : To enable Reactive Messaging add the following dependency to your project&rsquo;s pom.xml (see Managing Dependencies).">リアクティブ・メッセージングを有効にするには、プロジェクトの<code>pom.xml</code>に次の依存関係を追加します(<router-link to="/about/managing-dependencies">「依存関係の管理」</router-link>を参照)。</span></p>
...
<h2 id="_usage"><span class="merged" id="all.JUOSC.57"  title="原文:: Usage">使用方法</span></h2>
<div class="section">
...
<p><span class="merged" id="all.2kPwss.spl1" title="原文 : In Helidon there is a system of connectors, emitters and means to orchestrate these tasks called Reactive Messaging.">Helidonには、コネクタ、エミッタ、および<strong>「リアクティブ・メッセージング」</strong>と呼ばれるこれらのタスクを編成する手段のシステムがあります。</span> <span class="merged" id="all.2kPwss.spl2" title="原文 : It&rsquo;s basically an API for connecting and configuring connectors and emitters with your reactive streams through Channels.">基本的には、<router-link @click.native="this.scrollFix('#_channel')" to="#_channel">Channels</router-link>を介して、リアクティブ・ストリームでコネクタおよびエミッタを接続および構成するためのAPIです。</span> </p>
...
<div class="section">
<p><span class="merged" id="all.4QZOW6.spl1" title="原文 : An explicit config for channel&rsquo;s publisher is possible with Channel.Builder#publisherConfig(Config config) and for a subscriber with the Channel.Builder#subscriberConfig(Config config)."><code>Channel.Builder#publisherConfig(Config config)</code>および<code>Channel.Builder#subscriberConfig(Config config)</code>を持つサブスクライバに対して、チャネルのパブリッシャの明示的な構成が可能です。</span> <span class="merged" id="all.4QZOW6.spl2" title="原文 : The supplied Helidon Config is merged with the mandatory attributes and any implicit configuration found.">指定された<router-link to="/se/config/introduction">「Helidon構成」</router-link>は、必須属性および見つかった暗黙的な構成とマージされます。</span> <span class="merged" id="all.4QZOW6.spl3" title="原文 : The resulting configuration is then served to the Connector.">結果の構成はコネクタに提供されます。</span> </p>
...
<ul class="colist">
<li data-value="1"><span class="merged" id="all.4e457X.1" title="原文 : Kafka client consumer&rsquo;s property auto.offset.reset configuration for from-kafka channel only"><code>from-kafka</code>チャネルのKafkaクライアント・コンシューマのプロパティauto.offset.reset構成のみ</span></li>
<li data-value="2"><span class="merged" id="all.tggkr" title="原文 : Kafka client&rsquo;s property bootstrap.servers configuration for all channels using the connector">コネクタを使用するすべてのチャネルのKafkaクライアントのプロパティ<a href="{kafka-client-base-url}#consumerconfigs_bootstrap.servers" target="_blank">bootstrap.servers</a>構成</span></li>
</ul>
...
<h5 id="_reactive_jms_connector"><span class="merged" id="all.2u6yav.1" title="原文 : Reactive JMS Connector">リアクティブJMSコネクタ</span></h5>
<div class="section">
...
<ul class="ulist">
<li>
<p><span class="merged" id="all.44DzRK.2" title="原文 : https://github.com/oracle/helidon/tree/master/examples/messaging"><a class="bare" href="https://github.com/oracle/helidon/tree/master/examples/messaging" target="_blank">https://github.com/oracle/helidon/tree/master/examples/messaging</a></span></p>
...
<ul class="ulist">
<li>
<p><span class="merged" id="all.4apDrJ" title="原文 : MicroProfile Reactive Messaging Specification"><a href="{microprofile-reactive-messaging-spec-url}" target="_blank">MicroProfileリアクティブ・メッセージング仕様</a></span></p>
...
<li>
<p><span class="merged" id="all.1rqUyx" title="原文 : Helidon Messaging Examples"><a href="{helidon-github-tree-url}/examples/messaging" target="_blank">Helidonメッセージングの例</a></span></p>
