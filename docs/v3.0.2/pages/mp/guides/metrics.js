...
<tr>
<td class=""><span class="merged" id="all.2sRt9f.7" title="原文 : Java&#160;SE&#160;17 (Open&#160;JDK&#160;17)"><a href="https://www.oracle.com/technetwork/java/javase/downloads" target="_blank">Java&#160;SE&#160;17</a> (<a href="http://jdk.java.net" target="_blank">Open&#160;JDK&#160;17</a>)</span></td>
<td class=""><span class="merged" id="all.3aBChT.7" title="原文 : Helidon requires Java 17+.">HelidonにはJava 17以上が必要です。</span></td>
</tr>
<tr>
<td class=""><span class="merged" id="all.1DYqqk.8" title="原文 : Maven 3.6.1+"><a href="https://maven.apache.org/download.cgi" target="_blank">Maven 3.6.1+</a></span></td>
...
<div class="admonition note">
<p class="admonition-inline"><span class="merged" id="all.g31Oi.spl1" title="原文 : You cannot get the individual fields of a metric.">メトリックの個々のフィールドは取得できません。</span> <span class="merged" id="all.g31Oi.spl2" title="原文 : For example, you cannot target http://localhost:8080/metrics/vendor/requests.meter.count.">たとえば、<a class="bare" href="http://localhost:8080/metrics/vendor/requests.meter.count" target="_blank">http://localhost:8080/metrics/vendor/requests.meter.count</a>はターゲットにできません。</span> </p>
</div>
...
<ul class="ulist">
<li>
<p><span class="merged" id="all.3cthqo" title="原文 : Helidon thread pools ">Helidonスレッド・プール<br></span>
</p>
<p><span class="merged" id="all.UTEUN.spl1" title="原文 : Helidon uses these thread pools for its own internal work, and your application can also use Helidon-managed thread pools if it needs to do work asynchronously.">Helidonは、独自の内部作業にこれらのスレッド・プールを使用し、非同期で作業する必要がある場合は、アプリケーションでHelidon-managedスレッド・プールを使用することもできます。</span> <span class="merged" id="all.UTEUN.spl2" title="原文 : (See this example.)">(<a href="https://github.com/oracle/helidon/tree/master/examples/webserver/threadpool" target="_blank">「この例」</a>を参照。)</span> <span class="merged" id="all.UTEUN.spl3" title="原文 : The metrics in this group show information about the thread pools which can help you assess how efficiently they are utilized.">このグループのメトリクスは、スレッド・プールに関する情報を表示します。スレッド・プールは、その使用効率を評価するのに役立ちます。</span> <span class="merged" id="all.UTEUN.spl4" title="原文 : Helidon uses tags to distinguish the metrics which describe different thread pools.">Helidonは、タグを使用して、様々なスレッド・プールを記述するメトリクスを区別します。</span> <span class="merged" id="all.UTEUN.spl5" title="原文 : In some cases the specific metrics exposed depend on the particular type of thread pool.">場合によっては、表示される特定のメトリクスは、特定のタイプのスレッド・プールによって異なります。</span> </p>

</li>
<li>
<p><span class="merged" id="all.2ATFij" title="原文 : basic key performance indicators ">基本的なキー・パフォーマンス・インジケータ<br></span>
</p>
...
<div class="section">
<p><span class="merged" id="all.3swngc.spl1" title="原文 : Helidon implements the optional family of metrics, all with the name REST.request, as described in the MicroProfile Metrics specification.">Helidonは、<a href="https://download.eclipse.org/microprofile/microprofile-metrics-4.0/microprofile-metrics-spec-4.0.html#_optional_rest" target="_blank">「MicroProfileメトリクス仕様」</a>で説明されているように、オプションのメトリクス・ファミリをすべて<code>REST.request</code>という名前で実装します。</span> <span class="merged" id="all.3swngc.spl2" title="原文 : Each instance is a SimpleTimer with tags class and method identifying exactly which REST endpoint Java method that instance measures.">各インスタンスは、インスタンスが測定するRESTエンドポイントJavaメソッドを正確に識別するタグ<code>class</code>および<code>method</code>を含む<code>SimpleTimer</code>です。</span> </p>
...
<ul class="ulist">
<li>
<p><span class="merged" id="all.Bbpbc" title="原文 : MicroProfile Metrics specification"><a href="https://download.eclipse.org/microprofile/microprofile-metrics-4.0/microprofile-metrics-spec-4.0.html" target="_blank">MicroProfileメトリクス仕様</a></span></p>

</li>
<li>
<p><span class="merged" id="all.17OsJ4" title="原文 : MicroProfile Metrics Javadoc"><a href="https://download.eclipse.org/microprofile/microprofile-metrics-4.0/apidocs" target="_blank">MicroProfileメトリクスのJavadoc</a></span></p>

</li>
<li>
<p><span class="merged" id="all.2rtIah" title="原文 : Helidon Javadoc at ./apidocs/index.html?overview-summary.html">Helidon Javadoc at ./apidocs/index.html?overview-summary.html</span></p>
