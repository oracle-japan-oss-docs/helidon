...
<div class="section">
<p><span class="merged" id="all.49jk6h.spl1" title="原文 : Helidon provides a MicroProfile server implementation (io.helidon.microprofile.server) that encapsulates the Helidon WebServer.">Helidonは、Helidon WebServerをカプセル化するMicroProfileサーバー実装(<code>io.helidon.microprofile.server</code>)を提供します。</span> <span class="merged" id="all.49jk6h.spl2" title="原文 : You can either instantiate the server directly as is done in the Helidon MP Quickstart example or use its built-in main as shown below."><router-link to="/mp/guides/quickstart">「Helidon MPクイックスタートの例」</router-link>の場合と同様にサーバーを直接インスタンス化するか、次に示すように組込み<code>main</code>を使用できます。</span> </p>


<h3 id="_maven_coordinates"><span class="merged" id="all.317oeS.12"  title="原文:: Maven Coordinates">Maven連携</span></h3>
<div class="section">
<p><span class="merged" id="all.1MQGLn.spl1" title="原文 : The Managing Dependencies page describes how you should declare dependency management for Helidon applications."><router-link to="/about/managing-dependencies">「依存関係の管理」</router-link>ページでは、Helidonアプリケーションの依存関係管理を宣言する方法について説明します。</span> <span class="merged" id="all.1MQGLn.spl2" title="原文 : Then declare the following dependency in your project:">次に、プロジェクトで次の依存関係を宣言します:</span> </p>
