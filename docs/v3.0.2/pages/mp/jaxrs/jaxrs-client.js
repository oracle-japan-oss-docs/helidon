...
<div class="section">
<p><span class="merged" id="all.1mdC2F" title="原文 : To enable Jakarta REST Client add the following dependency to your project&rsquo;s pom.xml (see Managing Dependencies).">Jakarta RESTクライアントを有効にするには、プロジェクトの<code>pom.xml</code>に次の依存関係を追加します(<router-link to="/about/managing-dependencies">「依存関係の管理」</router-link>を参照)。</span></p>
...
<p><span class="merged" id="all.1NwsaI" title="原文 : In the snippet above, the call to target returns a WebTarget, and the call to request returns an Invocation.Builder; finally, the call to get returns the Response that results from accessing the service resource.">上のスニペットでは、<code>target</code>のコールは<code>WebTarget</code>を返し、<code>request</code>のコールは<code>Invocation.Builder</code>を返します。最後に、<code>get</code>のコールは、サービス・リソースにアクセスした結果となる<code>Response</code>を返します。</span></p>
...
<p><span class="merged" id="all.1edCOE.spl1" title="原文 : Configuration can be specified at the Client or WebTarget level, as both types implement Configurable&lt;T&gt;.">両方のタイプが<code>Configurable&lt;T></code>を実装するため、構成は<code>Client</code>レベルまたは<code>WebTarget</code>レベルで指定できます。</span> <span class="merged" id="all.1edCOE.spl2" title="原文 : This enables common configuration to be inherited by a WebTarget created from a Client instance.">これにより、<code>Client</code>インスタンスから作成された<code>WebTarget</code>によって共通構成を継承できます。</span> <span class="merged" id="all.1edCOE.spl3" title="原文 : In either case, methods such as register can be used to register providers such as filters and exception mappers.">どちらの場合も、<code>register</code>などのメソッドを使用して、フィルタや例外マッパーなどのプロバイダを登録できます。</span> </p>
...
<p><span class="merged" id="all.2DY6Eh" title="原文 : Jakarta REST Client Specification."><a href="https://jakarta.ee/specifications/restful-ws/3.0/jakarta-restful-ws-spec-3.0.html#client_api" target="_blank">「Jakarta RESTクライアント仕様」</a>。</span></p>
...
<div class="section">
<p><span class="merged" id="all.Y6kIq" title="原文 : For additional information, see the Jakarta REST Javadocs.">詳細は、<a href="https://jakarta.ee/specifications/restful-ws/3.0/apidocs" target="_blank">Jakarta REST Javadocs</a>を参照してください。</span></p>
...
<ul class="ulist">
<li>
<p><span class="merged" id="all.4VxQGX" title="原文 : Jakarta REST Client Specification"><a href="https://jakarta.ee/specifications/restful-ws/3.0/jakarta-restful-ws-spec-3.0.html#client_api" target="_blank">Jakarta RESTクライアント仕様</a></span></p>
